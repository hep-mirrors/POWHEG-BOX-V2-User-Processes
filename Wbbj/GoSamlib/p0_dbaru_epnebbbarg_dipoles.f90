module     p0_dbaru_epnebbbarg_dipoles
   use p0_dbaru_epnebbbarg_config, only: ki
   use p0_dbaru_epnebbbarg_color, only: gammaF, gammaA, &
       & CF, CA, numcs, KF, KA, &
       & T1T2, &
       & T1T5, &
       & T1T6, &
       ...
       & T5T7, &
       & T6T7
   use p0_dbaru_epnebbbarg_kinematics, only: num_legs, dotproduct, &
       lambda
   implicit none
...
contains
   ...
   function I_ff(mu_sq, vec, I, J)
      ...
      s_jk = 2.0_ki * abs(dotproduct(vec(5,:), vec(6,:)))
      ...
      if (flag) then
              matrix(:,:) = T5T6(:,:)/CF
              term = V_SING(s_jk, real(mB, ki), real(mB, ki))
              term(1) = term(1) + log_s * term(2)
              term =  CF * term + GAMMA_F(real(mB, ki))
      ...
      s_jk = 2.0_ki * abs(dotproduct(vec(5,:), vec(7,:)))
      log_s = log(mu_sq / s_jk)
      !log_s = 0.0_ki
      ...
      if (flag) then
              matrix(:,:) = T5T7(:,:)/CF
              term = V_SING(s_jk, real(mB, ki), real(0, ki))
              term(1) = term(1) + log_s * term(2)
              term =  CF * term + GAMMA_F(real(mB, ki))
              I_ff(:,:,1) = I_ff(:,:,1) + term(1) * matrix(:,:)
              I_ff(:,:,2) = I_ff(:,:,2) + term(2) * matrix(:,:)
      ...
      if (flag) then
              matrix(:,:) = T5T7(:,:)/CA
              term = V_SING(s_jk, real(0, ki), real(mB, ki))
              term(1) = term(1) + log_s * term(2)
              term = CA * term + GAMMA_A()
              I_ff(:,:,1) = I_ff(:,:,1) + term(1) * matrix(:,:)
              I_ff(:,:,2) = I_ff(:,:,2) + term(2) * matrix(:,:)
      end if

      s_jk = 2.0_ki * abs(dotproduct(vec(6,:), vec(7,:)))
      log_s = log(mu_sq / s_jk)
      !log_s = 0.0_ki
   ...
   function I_if(mu_sq, vec, I, J)
      ...
      s_ja = 2.0_ki * abs(dotproduct(vec(1,:), vec(5,:)))
      log_s = log(mu_sq / s_ja)
      !log_s = 0.0_ki

      ! T5.T1/T5^2
      if(present(I) .and. present(J)) then
         flag = (I .eq. 5) .and. (J .eq. 1)
      else
         flag = .true.
      end if
      ...
      if (flag) then
              matrix(:,:) = T1T5(:,:)/CF
              term = V_SING(s_ja, mB, 0.0_ki)
              term(1) = term(1) + log_s * term(2)
              term = CF * term + GAMMA_F(mB)
      ...
      s_ja = 2.0_ki * abs(dotproduct(vec(1,:), vec(7,:)))
      log_s = log(mu_sq / s_ja)
      !log_s = 0.0_ki

      ! T7.T1/T7^2
      if(present(I) .and. present(J)) then
         flag = (I .eq. 7) .and. (J .eq. 1)
      else
         flag = .true.
      end if
      ...
      s_ja = 2.0_ki * abs(dotproduct(vec(2,:), vec(5,:)))
      log_s = log(mu_sq / s_ja)
      !log_s = 0.0_ki

      ! T5.T2/T5^2
      if(present(I) .and. present(J)) then
         flag = (I .eq. 5) .and. (J .eq. 2)
      else
         flag = .true.
      end if
      ...
      s_ja = 2.0_ki * abs(dotproduct(vec(2,:), vec(7,:)))
      log_s = log(mu_sq / s_ja)
      !log_s = 0.0_ki

      ! T7.T2/T7^2
      if(present(I) .and. present(J)) then
         flag = (I .eq. 7) .and. (J .eq. 2)
      else
         flag = .true.
      end if
   ...
   function I_ff_qed(mu_sq, vec, I, J)
      ...
      if (flag) then
              ...
              term = V_SING_QED(s_jk, real(0, ki), real(0, ki))
              term(1) = (term(1) + log_s * term(2) ) * ncs*(-3.0_ki)*(-0.0_ki)/9.0_ki
              term(2) = term(2)* ncs*(-3.0_ki)*(-0.0_ki)/9.0_ki
      ...
      s_jk = 2.0_ki * abs(dotproduct(vec(3,:), vec(5,:)))
      log_s = log(mu_sq / s_jk)
      !log_s = 0.0_ki
      ...
      if (flag) then
              ...
              term = V_SING_QED(s_jk, real(0, ki), real(mB, ki))
              term(1) = (term(1) + log_s * term(2) ) * ncs*(-3.0_ki)*(1.0_ki)/9.0_ki
              term(2) = term(2)* ncs*(-3.0_ki)*(1.0_ki)/9.0_ki
              I_ff_qed(:,:,1) = I_ff_qed(:,:,1) + term(1) * matrix(:,:)
              I_ff_qed(:,:,2) = I_ff_qed(:,:,2) + term(2) * matrix(:,:)
      end if
      ...
      s_jk = 2.0_ki * abs(dotproduct(vec(3,:), vec(6,:)))
      log_s = log(mu_sq / s_jk)
      !log_s = 0.0_ki
      ...
      s_jk = 2.0_ki * abs(dotproduct(vec(3,:), vec(7,:)))
      log_s = log(mu_sq / s_jk)
      !log_s = 0.0_ki
      ...
      s_jk = 2.0_ki * abs(dotproduct(vec(4,:), vec(5,:)))
      log_s = log(mu_sq / s_jk)
      !log_s = 0.0_ki
      ...
      if (flag) then
              ...
              term = V_SING_QED(s_jk, real(0, ki), real(mB, ki))
              term(1) = (term(1) + log_s * term(2) ) * ncs*(-0.0_ki)*(1.0_ki)/9.0_ki
              term(2) = term(2)* ncs*(-0.0_ki)*(1.0_ki)/9.0_ki
              I_ff_qed(:,:,1) = I_ff_qed(:,:,1) + term(1) * matrix(:,:)
              I_ff_qed(:,:,2) = I_ff_qed(:,:,2) + term(2) * matrix(:,:)
      end if
      ...
      s_jk = 2.0_ki * abs(dotproduct(vec(4,:), vec(6,:)))
      log_s = log(mu_sq / s_jk)
      !log_s = 0.0_ki
      ...
      s_jk = 2.0_ki * abs(dotproduct(vec(4,:), vec(7,:)))
      log_s = log(mu_sq / s_jk)
      !log_s = 0.0_ki
      ...
      if (flag) then
              ...
              term = V_SING_QED(s_jk, real(mB, ki), real(mB, ki))
              term(1) = (term(1) + log_s * term(2) ) * ncs*(1.0_ki)*(-1.0_ki)/9.0_ki
              term(2) = term(2)* ncs*(1.0_ki)*(-1.0_ki)/9.0_ki
              I_ff_qed(:,:,1) = I_ff_qed(:,:,1) + term(1) * matrix(:,:)
              I_ff_qed(:,:,2) = I_ff_qed(:,:,2) + term(2) * matrix(:,:)
      end if
      ...
      if (flag) then
              ...
              term = V_SING_QED(s_jk, real(mB, ki), real(0, ki))
              term(1) = (term(1) + log_s * term(2) ) * ncs*(1.0_ki)*(0.0_ki)/9.0_ki
              term(2) = term(2)* ncs*(1.0_ki)*(0.0_ki)/9.0_ki
              I_ff_qed(:,:,1) = I_ff_qed(:,:,1) + term(1) * matrix(:,:)
              I_ff_qed(:,:,2) = I_ff_qed(:,:,2) + term(2) * matrix(:,:)
      end if
   ...
   function I_if_qed(mu_sq, vec, I, J)
      ...
      if (flag) then
              ncs=1.0_ki
              term = V_SING_QED(s_ja, 0.0_ki, 0.0_ki)
              term=term*ncs*(1.0_ki)*(-3.0_ki/9.0_ki)
              term(1) = term(1) + log_s * term(2)
      ...
      if (flag) then
              ncs=1.0_ki
              term = V_SING_QED(s_ja, 0.0_ki, 0.0_ki)
              term=term*ncs*(1.0_ki)*(-0.0_ki/9.0_ki)
              term(1) = term(1) + log_s * term(2)
      ...
      if (flag) then
              ncs=1.0_ki
              term = V_SING_QED(s_ja, mB, 0.0_ki)
              term=term*ncs*(1.0_ki)*(1.0_ki/9.0_ki)
              term(1) = term(1) + log_s * term(2)
      ...
      if (flag) then
              ncs=1.0_ki
              term = V_SING_QED(s_ja, 0.0_ki, 0.0_ki)
              term=term*ncs*(2.0_ki)*(-3.0_ki/9.0_ki)
              term(1) = term(1) + log_s * term(2)
      ...
      if (flag) then
              ncs=1.0_ki
              term = V_SING_QED(s_ja, 0.0_ki, 0.0_ki)
              term=term*ncs*(2.0_ki)*(-0.0_ki/9.0_ki)
              term(1) = term(1) + log_s * term(2)
      ...
      if (flag) then
              ncs=1.0_ki
              term = V_SING_QED(s_ja, mB, 0.0_ki)
              term=term*ncs*(2.0_ki)*(1.0_ki/9.0_ki)
              term(1) = term(1) + log_s * term(2)
   ...
   function I_ii_qed(mu_sq, vec, I, J)
      ...
      if (flag) then
              ...
              term(1) = ncs*(1.0_ki)*(2.0_ki)/9.0_ki
              term(2) = ncs*(1.0_ki)*(2.0_ki)/9.0_ki
...
end module p0_dbaru_epnebbbarg_dipoles
