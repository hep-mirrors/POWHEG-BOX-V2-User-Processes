...
namespace ninja {
  ...
  // Set the default infrared threshold used to detect unstable
  // kinematics to be used when not specified for an Amplitude object.
  inline void setDefaultInfraredThreshold(Real threshold)
  {
    Options::ir_threshold = threshold;
  }
  ...
  public:
    ...
    Amplitude()
      : result(0.,0.,0.),
        ...
        cut_constr(),
        scale(1.),
        ir_threshold(Options::ir_threshold),
        n(0), rank(0), 
        min_cut(0),
        return_val(SUCCESS),
        use_mu_exp(true) {}
    ...
    // set infrared threshold used to detect unstable kinematics.  If
    // an unstable kinematics is detected in the 'evaluate' method,
    // UNSTABLE_KINEMATICS is returned and the result is left
    // unchanged.
    void setInfraredThreshold(Real threshold)
    {
  ...
  private:
    ...
    {
      if (taxicab_norm(z) < ir_threshold) {
