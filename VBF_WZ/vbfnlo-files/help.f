...
common block for D_ij etc. information via bcd_fill_c
      ...
      common /bcd_qqv_c/ sc,tc,q1sqc,q2sqc, D0c,Dijc, 
     1                   Teps1c,Teps2c,Tbornc,Tg1c,Tg2c
C   local variables
      integer mu, nu, isig
      double precision s,t,mom(0:3),dotp
      external dotp
...
c          print*,'box gauge check counting ratio:',real(gcount)/real(bcount)
c        endif
...
c momenta are outgoing
      ...
      qbe(2) = dimag(jbe(5))
      qbe(3) = dimag(jbe(4))
c compute Born amplitudes contracted with momentum 
...
c
     ...
     3     mbc(3,isig),   mvc(3,isig))
      ...
      if (.false.) then
         ...
         enddo
         do j = 1,3
            print*,' j = ',j,' tree ratio for isig = -1: ',
     &           mbc(j,-1)/mb(j,-1)
         enddo
      endif
