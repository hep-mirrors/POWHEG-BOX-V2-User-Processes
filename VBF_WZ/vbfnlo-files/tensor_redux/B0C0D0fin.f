...
c          N_ep = i/(4 pi)^2 (4 pi)^ep Gamma(1+ep) (musq)^(-ep)
      ...
      function D0fin(s,t,qq1sq,qq2sq,musq)
      implicit none
      complex * 16 D0fin
...
c      complex*16  D0t
      ...
      if (musq.lt.0) then
         write(*,*) 'ERROR in D0fin: mu^2 MUST be a positive number'
         stop
      endif

c      write(*,*) 'D0 with args',s,t,qq1sq,qq2sq,musq

c      d0fin = D0t(s,t,qq1sq,qq2sq)
c      return
...
c exchange who is positive with the one that is negative         
     ...
     #     (q1sq.lt.0).and.(q2sq.gt.0)) then
         ...
         if (r1.lt.1.d0) then
            ...
            ris(2) = -(l1+l2) - I*pi
            ris(1) = -2*(
     #           dilog(r1) + l1*log(1.d0-r1) + 
     #           dilog(r2) + l2*l1mr2) + 1.d0/2*(l1**2+l2**2)+l1*l2
     #           + I*pi*(-2*l1mr2+l2+l1)
     #           + 1.d0/2*lsomu2**2 - lsomu2*ris(2)
         ...
         elseif (r1.ge.1.d0) then
            ...
            ris(2) = -(l1+l2) - I*pi
            ris(1) = -2*(
     #           -dilog(q1sq/t)-1.d0/2*l1**2+pi**2/3+l1*log(r1-1.d0)
     #           +dilog(r2) + l2*l1mr2) + 1.d0/2*(l1**2+l2**2)+l1*l2
     #           + I*pi*(-2*l1mr2+l2+l1)
     #           + 1.d0/2*lsomu2**2 - lsomu2*ris(2)
            ris(2) = ris(2) - lsomu2
     ...
     #        (q1sq.lt.0).and.(q2sq.gt.0)) then
         ...
         if (r2.lt.1.d0) then
            ...
            ris(1) = -2*(
     #           dilog(r1) + l1*l1mr1 + 
     #           dilog(r2) + l2*log(1-r2))+1.d0/2.d0*(l1**2+l2**2)+l1*l2
     #           + I*pi*( 2*l1mr1-l1-l2)
     #           + 1.d0/2*lsomu2**2 - lsomu2*ris(2)
            ris(2) = ris(2) - lsomu2
     ...
     #           + 1.d0/2*lsomu2**2 - lsomu2*ris(2)
         ...
         write(*,*) 'ERROR in the D0fin function. Cannot be called with
     #        these arguments'
