...
c     delta(p2^2-s2) delta(p3^2-s3)
     ...
     . mass2,width2,mass3,width3
      ...
      if (s3min .gt. s3max) then 
         write(*,*) 'in phi1_2m_nobw.f:'
         write(*,*) 's3min , s3max', s3min, s3max 
         s3max = s3min 
         wt = 0d0 
      endif
      w3=s3max-s3min
      ...
      write(6,*) 'lambda in phi1_2m_nobw',lambda
      write(6,*) 's1 in phi1_2m_nobw',s1
      write(6,*) 's2 in phi1_2m_nobw',s2
      write(6,*) 's3 in phi1_2m_nobw',s3
      write(6,*) 'm1 in phi1_2m_nobw',m1
      write(6,*) 'm2 in phi1_2m_nobw',m2
      write(6,*) 'm3 in phi1_2m_nobw',m3
      write(6,*) 'x3 in phi1_2m_nobw',x3
      write(6,*) 'mass3 in phi1_2m_nobw',mass3
      wt = 0d0 
      endif
      lambda=dsqrt(lambda)
      wt=wt0*w3*lambda/s1
