...
c
      ...
      include 'global.inc'
c
      real*8 p(0:3,np), v(0:3,nv), q12(0:4,3), q34(0:4,3), qsq
      integer Lmax, L, mu
c
...
c
	 ...
	 mursq(2,L) = qsq
      ...
      do mu = 0,3
         q12(mu,1) = p(mu,1)-p(mu,5)-p(mu,3)
         q34(mu,1) = p(mu,2)-p(mu,5)-p(mu,4)         
c adapted for POWHEG:
            q12(mu,2) = q12(mu,1)
            q12(mu,3) = p(mu,1)-p(mu,3)
            q34(mu,2) = p(mu,2)-p(mu,4)
            q34(mu,3) = q34(mu,1)
      enddo
      do L = 1,Lmax
