...
c Madevent parameters
     ...
     #         rat,cth2,ttenmnr,cmenmnr,krtop2,kpair,tten,cmen
      ...
      real * 8 dotprod,random,fgg,fqq,fqg,ggborn,qqborn,powheginput
      external dotprod,random,fgg,fqq,fqg,ggborn,qqborn,powheginput
...
c spin summmed and averaged amplitude squared for W decay
      ...
      elseif(ini.eq.0) then
         if(powheginput('#topdecaymode').eq.-1d6) then
            ini=2
            return
         endif
         if(powheginput('topdecaymode').eq.0d0) then
            ini=2
            return
         endif
         ini=1
...
c compute mnr invariants
      ...
      xm2=tmass**2
      if(nup.eq.5) then
         tk=-2*dotprod(pp(1,1),pp(1,5))
         uk=-2*dotprod(pp(1,2),pp(1,5))
         q2q=-2*dotprod(pp(1,2),pp(1,4))
...
c cth2 is not used by fgg,fqg,fqq, unless we are very near the collinear limit
         cth2=0
         if(proc.eq.'gg')  then
            resmnr = fgg(s,x,y,xm2,q1q,q2q,w1h,w2h,cth2)/(4*tk*uk)
         elseif(proc.eq.'qg') then
            resmnr = fqg(s,x,y,xm2,q1q,q2q,w1h,w2h,cth2)/(4*tk*uk)
         elseif(proc.eq.'qa') then
            resmnr = fqq(s,x,y,xm2,q1q,q2q,w1h,w2h,cth2)/(4*tk*uk)
         else
            write(*,*)' error: ttdecay, invalid process 1',proc
            stop
         endif
      elseif(nup.eq.4) then
         if(proc.eq.'gg') then
            resmnr=ggborn(s,q1q,xm2)
         elseif(proc.eq.'qa') then
            resmnr=qqborn(s,q1q,xm2)
         else
            write(*,*)' error: ttdecay, invalid process 2'
            stop
         endif
      endif
...
c krtop: relative top momentum (i.e. in the top-antitop CM)
      ...
      call twvirts(krtop2,kpair,tpvirt2,wpvirt2,tmvirt2,wmvirt2)
...
c check mom.cons.
      call momcons(1,pup(1,3),3,ppss(1,3))
      call momcons(1,pup(1,4),3,ppss(1,6))
      call momcons(2,ppss,nup-4+6,ppss(1,3))
...
c         
      if(nup.eq.4) then
         if(proc.eq.'qa') then
            call uub_ttb(pme,res)
         elseif(proc.eq.'gg') then
            call gg_ttb(pme,res)
         endif
      else
         if(proc.eq.'qa') then
            call uub_ttbg(pme,res)
         elseif(proc.eq.'gg') then
            call gg_ttbg(pme,res)
         elseif(proc.eq.'qg') then
            call ug_ttbu(pme,res)
         endif
      endif
c /(2*s)
...
c Put finite masses to W decay products
      call momcons(2,pp,nup-4+6,pp(1,3))
      call truemasses(pp(1,3),mdecwp1,mdecwp2)
      call truemasses(pp(1,6),mdecwm1,mdecwm2)
      call momcons(2,pp,nup-4+6,pp(1,3))
...
c
      ...
      subroutine truemasses(decprod,m1,m2)
      implicit none
...
c compute the momentum of the two body decay products, from mass m-> m1,m2
      ...
      subroutine twvirts(krtop2,kpair,
...
c due to t and tbar going off the pole mass. The scheme adopted here is that
...
c are taken from the MEcoupl.inc common blocks.
     ...
     #         kb0,wpkl0,wmkl0,tten,cmen,cmen0,ran(5)
      real * 8  pdfcorr,decmom
      external pdfcorr,decmom
      integer j
      real * 8 pi
      parameter (pi=3.141592653589793d0)
      real * 8 virt2,mass,gam
c generate squared virtuality according to Breit-Wigner and random number ran(j)
      virt2(mass,gam,j)=gam*mass*tan(pi/2*(1-2*ran(j)))+mass**2
      bmass2=bmass**2
      tmass2=tmass**2
      wmass2=wmass**2
...
c generate random directed b momentum in t rest frame
      call rn3vec(decprod(1,3),pb)
      decprod(4,3)=eb
c 1st W decay product in W rest frame
      pl=wvirt/2
      call rn3vec(decprod(1,1),pl)
      decprod(4,1)=pl
...
c (its momentum is opposite to the b momentum)
      ...
      call boostm(beta,2,decprod(1,1),decprod(1,1))
...
c equals the sum of n2 momenta in p2
      ...
      subroutine pickwdecays(iwp1,mdecwp1,iwp2,mdecwp2,
...
c on first run look for decay mode in powheginput
     ...
     1                 .or.(j.le.3.and.k.le.3) ) ) then
         ...
         mass(14)=0
         mass(16)=0
         mass(1)=powheginput('tdec/dmass')
         mass(2)=powheginput('tdec/umass')
         mass(3)=powheginput('tdec/smass')
...
c
c through the following parameters, given by powheginput
c
...
c
      ...
      if(ini.eq.1) return
      ini=1
      wmass=powheginput('tdec/wmass')
      wwidth=powheginput('tdec/wwidth')
      zmass=0
      zwidth=0
      tmass=powheginput('qmass')
      twidth=powheginput('tdec/twidth')
      bmass=powheginput('tdec/bmass')
      sin2w=0.23
      alpha=1
      ee2=4*pi*alpha
c
