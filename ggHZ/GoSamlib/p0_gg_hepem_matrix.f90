module     p0_gg_hepem_matrix
   ...
   use p0_gg_hepem_model, only: Nf, NC, sqrt2, init_functions
   use p0_gg_hepem_color, only: TR, CA, CF, numcs, &
     & incolors, init_color
   use p0_gg_hepem_amplitudeh0, only: samplitudeh0l1 => samplitude, &
        &   finite_renormalisation0 => finite_renormalisation
   use p0_gg_hepem_amplitudeh4, only: samplitudeh4l1 => samplitude, &
        &   finite_renormalisation4 => finite_renormalisation
   use p0_gg_hepem_dipoles, only: insertion_operator
...
contains
   ...
   subroutine     samplitude(vecs, scale2, amp, ok, h)
      ...
      if(spprec1 .lt. PSP_chk_threshold1 .and. spprec1 .gt. -10000 .or. &
         ...
         if(PSP_rescue) then
            ...
            call samplitudel01(vecs, scale2, amp, rat2, ok, h)
            if((amp(3)-irp(2)) .ne. 0.0_ki) then
               spprec2 = -int(log10(abs(amp(3)/amp(2))))
            else
               spprec2 = 16
            endif
            if((amp(2)-rat2) .ne. 0.0_ki) then
            ...
            if(spprec2 .le. PSP_chk_threshold2 .and. spprec2 .gt. -10000 .or. &
               ...
               if(PSP_verbosity .ge. 1) then
                  ...
                  if(spprec2 .le. PSP_chk_threshold2 .and. spprec2 .gt. -10000) then
                     write(42,'(4x,A15,A11,A18,A3)') "<process name='", &
                          &   "p0_gg_hepem", "' problem='sinpole","'/>"
                  else if(abs(kfac) > PSP_chk_kfactor) then
                     write(42,'(4x,A15,A11,A18,A3)') "<process name='", &
                          &   "p0_gg_hepem", "' problem='kfactor","'/>"
                  end if
   ...
   subroutine     samplitudel01(vecs, scale2, amp, rat2, ok, h)
      ...
      use p0_gg_hepem_dipoles, only: pi
      implicit none
      real(ki), dimension(5, 4), intent(in) :: vecs
      real(ki), intent(in) :: scale2
   ...
   function     samplitudel0(vecs, h) result(amp)
      use p0_gg_hepem_config, only: logfile
      use p0_gg_hepem_kinematics, only: init_event
      implicit none
      real(ki), dimension(5, 4), intent(in) :: vecs
      integer, optional, intent(in) :: h
      real(ki) :: amp, heli_amp
      complex(ki), dimension(numcs) :: color_vector
      logical, dimension(0:15) :: eval_heli
   ...
   pure subroutine color_correlated_lo(color_vector,res)
      use p0_gg_hepem_color, only: T1T1, &
      & T1T2, &
      & T2T2
      implicit none
   ...
   pure function square_0l_0l_sc(color_vector1, color_vector2) result(amp)
      use p0_gg_hepem_color, only: cmat => CC
      implicit none
