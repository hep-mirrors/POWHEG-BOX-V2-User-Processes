...
c     First determine virtualities of the Higgs
      smin=ph_Hmass2low
      smax=ph_Hmass2high
      mllminsq=ph_Zmass2low
      mllmaxsq=ph_Zmass2high
c
      z=xborn(1)
c
...
c     running mass
      if (.not.higgsfixedwidth) then
         ...
         BW_fixed=ph_hmass*ph_hwidth /
     $        ((s-ph_Hmass2)**2 + (ph_hmass*ph_hwidth)**2)
         BW_running= (s*ph_Hwidth/ph_Hmass) /
     $        ((s-ph_Hmass2)**2+(s*ph_Hwidth/ph_Hmass)**2)
