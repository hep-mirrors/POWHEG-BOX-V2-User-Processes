...
C
C###############################################################################
      subroutine MG5IXXXXX(p, fmass, nhel, nsf ,fi)
c
...
c         write(stdo,*)
c     &        ' helas-error : p(0:3) in MG5IXXXXX is zero momentum'
c      endif
c      if ( p(0).le.rZero ) then
c         write(stdo,*)
c     &        ' helas-error : p(0:3) in MG5IXXXXX has non-positive energy'
c         write(stdo,*)
c     &        '             : p(0) = ',p(0)
c      endif
c      p2 = (p(0)-pp)*(p(0)+pp)
...
c         write(stdo,*)
c     &        ' helas-error : p(0:3) in MG5IXXXXX has inappropriate mass'
c         write(stdo,*)
c     &        '             : p**2 = ',p2,' : fmass**2 = ',fmass**2
c      endif
c      if (abs(nhel).ne.1) then
c         write(stdo,*) ' helas-error : nhel in MG5IXXXXX is not -1,1'
c         write(stdo,*) '             : nhel = ',nhel
c      endif
c      if (abs(nsf).ne.1) then
c         write(stdo,*) ' helas-error : nsf in MG5IXXXXX is not -1,1'
...
c Identical to MG5IXXXXX, except that fi returns only the spinor (without the momentum)
      implicit none
...
c
      ...
      subroutine MG5OXXXXX(p,fmass,nhel,nsf , fo)
c
...
c      if ( abs(p(0))+pp.eq.rZero ) then
c         write(stdo,*)
c     &        ' helas-error : p(0:3) in MG5OXXXXX is zero momentum'
c      endif
c      if ( p(0).le.rZero ) then
c         write(stdo,*)
c     &        ' helas-error : p(0:3) in MG5OXXXXX has non-positive energy'
c         write(stdo,*)
c     &        '         : p(0) = ',p(0)
c      endif
c      p2 = (p(0)-pp)*(p(0)+pp)
c      if ( abs(p2-fmass**2).gt.p(0)**2*epsi ) then
c         write(stdo,*)
c     &        ' helas-error : p(0:3) in MG5OXXXXX has inappropriate mass'
c         write(stdo,*)
c     &        '             : p**2 = ',p2,' : fmass**2 = ',fmass**2
c      endif
c      if ( abs(nhel).ne.1 ) then
c         write(stdo,*) ' helas-error : nhel in MG5OXXXXX is not -1,1'
c         write(stdo,*) '             : nhel = ',nhel
c      endif
c      if ( abs(nsf).ne.1 ) then
c         write(stdo,*) ' helas-error : nsf in MG5OXXXXX is not -1,1'
c         write(stdo,*) '             : nsf = ',nsf
c      endif
c#endif
...
c Identical to MG5OXXXXX, except that fo returns only the spinor (without the momentum)
      implicit none
...
c construct eps0
     ...
     &                                + rTwo*e0(i)*e0(j) )
      ...
      subroutine MG5VXXXXX(p,vmass,nhel,nsv , vc)
c
...
c      if ( abs(p(0))+pp.eq.rZero ) then
c         write(stdo,*)
c     &        ' helas-error : p(0:3) in MG5VXXXXX is zero momentum'
c      endif
c      if ( p(0).le.rZero ) then
c         write(stdo,*)
c     &        ' helas-error : p(0:3) in MG5VXXXXX has non-positive energy'
c         write(stdo,*)
c     &        '             : p(0) = ',p(0)
c      endif
c      p2 = (p(0)+pp)*(p(0)-pp)
...
c         write(stdo,*)
c     &        ' helas-error : p(0:3) in MG5VXXXXX has inappropriate mass'
c         write(stdo,*)
c     &        '             : p**2 = ',p2,' : vmass**2 = ',vmass**2
c      endif
c      if ( vmass.ne.rZero ) then
c         if ( abs(nhel).gt.1 ) then
c            write(stdo,*) ' helas-error : nhel in MG5VXXXXX is not -1,0,1'
c            write(stdo,*) '             : nhel = ',nhel
c         endif
c      else
c         if ( abs(nhel).ne.1 ) then
c            write(stdo,*) ' helas-error : nhel in MG5VXXXXX is not -1,1'
c            write(stdo,*) '             : nhel = ',nhel
c         endif
c      endif
...
c
      ...
      subroutine MG5boostm(p,q,m, pboost)
c
c This subroutine performs the Lorentz boost of a four-momentum.  The
c momentum p is assumed to be given in the rest frame of q.  pboost is
c the momentum p boosted to the frame in which q is given.  q must be a
