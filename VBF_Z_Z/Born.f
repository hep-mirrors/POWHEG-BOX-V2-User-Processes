...
c     
      ...
      call compborn_zzjj_ew(p,bflav,born) 
...
c==================================================
c
      subroutine compborn_zzjj_ew(pin,bflav,born)
      implicit none
c
      include 'nlegborn.h'
      include 'pwhg_math.h'
...
c lepton momenta:
      do mu = 0,3             ! kinematics for 4-lepton final state
         pbar(mu,5) = v(mu,2) ! l-
         pbar(mu,6) = v(mu,1) ! l+
         pbar(mu,7) = v(mu,3) ! ll- or v
         pbar(mu,8) = v(mu,4) ! ll+ or v~
      enddo
      if (bos.eq.22) then
      fsign(5) = -1
      fsign(6) = 1
...
c use value of bflav to select appropriate uucc, uuss etc.
      ...
      call qqzzqq_channel(pbar,fsign,0,1,k,bos,res)
