...
c     fat_jet = powheginput('#fat_jet')
         higgs_analysis = powheginput("higgs_analysis")
...
c     if (fat_jet.eq.1) then 
c     write(*,*) 'Calling analysis_slp_fat'
c     else
      ...
      if (decmode_slp) then
c            if (fat_jet.eq.1) then 
c               call init_hist_slp_fat
c            else   
         call init_hist_slp
      elseif (decmode_lnu) then
         call init_hist_lnu
      elseif (decmode_lep.and.(higgs_analysis.eq.1)) then
         call init_hist_lep_higgs
      else
         call init_hist_lep
      endif   
...
c            else   
      ...
      elseif (decmode_lnu) then
         call analysis_lnu(dsig0)
      elseif (decmode_lep.and.(higgs_analysis.eq.1)) then
         call analysis_lep_higgs(dsig0)
      else
