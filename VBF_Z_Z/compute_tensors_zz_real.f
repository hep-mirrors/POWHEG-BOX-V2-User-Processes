     ...
     &     qu_real(2)**2-qu_real(3)**2
      ...
      if(decmode_lep) then
         ...
         do h = 1,4,1
            ...
            CALL J_Z_BOSON_e(lep_real(1,h),lem_real(1,h),GC_50,
     &           GC_59,MZ,WZ,ze_real(1,h)) !Zl
            CALL J_A_BOSON(lep_real(1,h),lem_real(1,h),GC_4,
     &           ZERO ,ZERO  ,ae_real(1,h)) !Al
            CALL J_Z_BOSON_e(lup_real(1,h),lum_real(1,h),GC_50,
     &           GC_59,MZ,WZ,zu_real(1,h)) !Zl
            CALL J_A_BOSON(lup_real(1,h),lum_real(1,h),GC_4,
...
c     for V-> llll:	
     ...
     &           call vto4l(v,h,azz_real(0,h),zzz_real(0,h),ttype)	
         ...
         do h = 1,3,2 !The remaining two helicities are set to zero at initialization
            ie = sign(1,2-h)
            iu = -1
     ...
     &           ZERO ,ZERO  ,ae_real(1,h)) !Al
            ...
            CALL J_Z_BOSON_v(lup_real(1,h),lum_real(1,h),-GC_62,
...
c     for V-> llll:	
     ...
     &           GC_59,MZ,WZ,ze_real(1,h)) !Zl
            CALL J_A_BOSON(lep_real(1,h),lem_real(1,h),GC_4,
     &           ZERO ,ZERO  ,ae_real(1,h)) !Al
            CALL J_Z_BOSON_d(lup_real(1,h),lum_real(1,h),GC_50,
     &           GC_58,MZ,WZ,zu_real(1,h)) !Zd
            CALL J_A_BOSON(lup_real(1,h),lum_real(1,h),-GC_1,
     &           ZERO ,ZERO  ,au_real(1,h)) !Ad
...
c     
C     for W+W-
               call wwtozd(q12(0,j),q34(0,j),v,h,wwzz5_real(0,0,j,h),ttype) !q12 = W+
               call wwtozd(q34(0,j),q12(0,j),v,h,wwzz6_real(0,0,j,h),ttype) !q12 = W-
...
c     CCl tensors for CC (k=5)
                  call WWtoll(2,1,h,q34(0,j),v,CCee_real(0,0,1,j,h),ttype) !V on upper
                  call WWtoll(1,1,h,q12(0,j),v,CCee_real(0,0,2,j,h),ttype) !V on lower
                  call WWtodd(2,h,q34(0,j),v,CCuu_real(0,0,1,j,h),ttype) !V on upper
                  call WWtodd(1,h,q12(0,j),v,CCuu_real(0,0,2,j,h),ttype) !V on lower
                  
C     for W-W+ --> e+ e- or mu+ mu-
c     CCl for CC (k=6)	
                  call WWtoll(1,1,h,q34(0,j),v,CCee6_real(0,0,1,j,h),ttype) !V on upper
                  call WWtoll(2,1,h,q12(0,j),v,CCee6_real(0,0,2,j,h),ttype) !V on lower
                  call WWtodd(1,h,q34(0,j),v,CCuu6_real(0,0,1,j,h),ttype) !V on upper
                  call WWtodd(2,h,q12(0,j),v,CCuu6_real(0,0,2,j,h),ttype) !V on lower
...
c     for V-> llll:	
            if (.not.higgs_only) 
     &           call vto4ld(v,h,azz_real(0,h),zzz_real(0,h),ttype)	
     ...
     &           GC_59,MZ,WZ,ze_real(1,h)) !Zl
            CALL J_A_BOSON(lep_real(1,h),lem_real(1,h),GC_4,
     &           ZERO ,ZERO  ,ae_real(1,h)) !Al
            CALL J_Z_BOSON_d(lup_real(1,h),lum_real(1,h),GC_51,
     &           GC_58,MZ,WZ,zu_real(1,h)) !Zd
            CALL J_A_BOSON(lup_real(1,h),lum_real(1,h),-GC_2,
     &           ZERO ,ZERO  ,au_real(1,h)) !Ad
...
c     
C     for W+W-
               call wwtozu(q12(0,j),q34(0,j),v,h,wwzz5_real(0,0,j,h),ttype) !q12 = W+
               call wwtozu(q34(0,j),q12(0,j),v,h,wwzz6_real(0,0,j,h),ttype) !q12 = W-
...
c     CCl tensors for CC (k=5)
                  call WWtoll(2,1,h,q34(0,j),v,CCee_real(0,0,1,j,h),ttype) !V on upper
                  call WWtoll(1,1,h,q12(0,j),v,CCee_real(0,0,2,j,h),ttype) !V on lower
                  call WWtouu(2,h,q34(0,j),v,CCuu_real(0,0,1,j,h),ttype) !V on upper
                  call WWtouu(1,h,q12(0,j),v,CCuu_real(0,0,2,j,h),ttype) !V on lower
                  
C     for W-W+ --> e+ e- or mu+ mu-
c     CCl for CC (k=6)	
                  call WWtoll(1,1,h,q34(0,j),v,CCee6_real(0,0,1,j,h),ttype) !V on upper
                  call WWtoll(2,1,h,q12(0,j),v,CCee6_real(0,0,2,j,h),ttype) !V on lower
                  call WWtouu(1,h,q34(0,j),v,CCuu6_real(0,0,1,j,h),ttype) !V on upper
                  call WWtouu(2,h,q12(0,j),v,CCuu6_real(0,0,2,j,h),ttype) !V on lower
...
c     for V-> llll:	
            if (.not.higgs_only) 
     &           call vto4lu(v,h,azz_real(0,h),zzz_real(0,h),ttype)	
            
         enddo                  !h  
      else
         print*, 'Wrong decay mode in compute_tensor_zz_real.f'
      endif
      return
      end
