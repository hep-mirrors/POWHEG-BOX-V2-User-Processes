...
c================================================
      ...
      if (firsttime) then
         ...
         fakevirt = 1 !First point ALWAYS run with fakevirt. This ensures that smartsig wokrs properly
      ...
      if (decmode_slp) then
         col_dec = 3d0
         if ((abs(vdecaymodeZ2).eq.107).or.(abs(vdecaymodeZ2).eq.108)) then
            had_sum = 2d0
         endif
      endif   
      ...
      if ( pzsum.ne.pid) then ! new PS point -> compute tensors
         call compute_tensors_zz(p) 
         pid = 0d0
         do j = 1,nlegs
      ...
      ttype = 1
      call provide_tensors_zz(ttype)
      call compborn_zzjj_ew(p,vflav,born) 
...
c
         ...
         ttype = 1
         call provide_tensors_zz(ttype)
         call compvirt_zzjj_ew(p,vflav,virtual) 
...
c
      subroutine compvirt_zzjj_ew(pin,bflav,virtual)
      implicit none
c
      include 'nlegborn.h'
      include 'pwhg_math.h'
...
c use value of bflav to select appropriate uucc, uuss etc.
      ...
      pent = 0d0
c triangles:
      call qqzzqq_channel(pbar,fsign,4,1,k,bos,tri)
      if (.not.higgs_only) then
c boxes:
      call qqzzqq_channel(pbar,fsign,-4,1,k,bos,box)
c pentagons:
      call qqzzqq_channel(pbar,fsign,-5,1,k,bos,pent)
      endif
c
      res = tri+box+pent
c
