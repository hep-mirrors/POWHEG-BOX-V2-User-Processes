...
c     consider 4 different combinations of lepton helicities:
         do h = 1,4,1
            ...
            CALL J_Z_BOSON_e(lep_born(1,h),lem_born(1,h),GC_50,
     &           GC_59,MZ,WZ,ze_born(1,h)) !Zl
            CALL J_A_BOSON(lep_born(1,h),lem_born(1,h),GC_4,
     &           ZERO ,ZERO  ,ae_born(1,h)) !Al
            CALL J_Z_BOSON_e(lup_born(1,h),lum_born(1,h),GC_50,
     &           GC_59,MZ,WZ,zu_born(1,h)) !Zl
            CALL J_A_BOSON(lup_born(1,h),lum_born(1,h),GC_4,
...
c     
      ...
      elseif(decmode_lnu) then
         do h = 1,3,2 !The remaining two helicity configuration is set to zero at initialization
     ...
     &           ZERO ,ZERO  ,ae_born(1,h)) !Al
            ...
            CALL J_Z_BOSON_v(lup_born(1,h),lum_born(1,h),-GC_62,
...
c     
         ...
         enddo                  !h
      else if(decmode_slp.and.(mod(vdecaymodeZ2,2).eq.1)) then
c     consider 4 different combinations of lepton helicities:
         do h = 1,4,1
            ...
            CALL IXXXXX_5(v(0,1),ZERO ,+ie,-1,lep_born(1,h)) !e+	
            CALL OXXXXX_5(v(0,2),ZERO ,-ie, 1,lem_born(1,h)) !e- 
            CALL IXXXXX_5(v(0,3),ZERO ,+iu,-1,lup_born(1,h)) !bbar	
            CALL OXXXXX_5(v(0,4),ZERO ,-iu, 1,lum_born(1,h)) !b 
            
            CALL J_Z_BOSON_e(lep_born(1,h),lem_born(1,h),GC_50,
     &           GC_59,MZ,WZ,ze_born(1,h)) !Zl
            CALL J_A_BOSON(lep_born(1,h),lem_born(1,h),GC_4,
     &           ZERO ,ZERO  ,ae_born(1,h)) !Al
            CALL J_Z_BOSON_d(lup_born(1,h),lum_born(1,h),GC_50,
     &           GC_58,MZ,WZ,zu_born(1,h)) !Zd
            CALL J_A_BOSON(lup_born(1,h),lum_born(1,h),-GC_1,
     &           ZERO ,ZERO  ,au_born(1,h)) !Ad
...
c     leptonic tensors
            j = 1
C     for AA,AZ,ZA,ZZ -> e+ e- b~ b :
            call vvtozd(q12(0,j),q34(0,j),v,h,
     #           aazz_born(0,0,j,h),azzz_born(0,0,j,h),
     #           zazz_born(0,0,j,h),zzzz_born(0,0,j,h),ttype)
            
C     for W+W-
            call wwtozd(q12(0,j),q34(0,j),v,h,wwzz5_born(0,0,j,h),ttype) ! q12 = W+
            call wwtozd(q34(0,j),q12(0,j),v,h,wwzz6_born(0,0,j,h),ttype) ! q12 = W-
            
            if (.not.higgs_only) then
               
C     for V1V2 --> e+ e- or  V1V2 -->  b~ b (Vi=A,Z)
c     NCl tensors for NC process (k=1...4)
     ...
     #              aaee_born(0,0,2,j,h),azee_born(0,0,2,j,h),
     #              zaee_born(0,0,2,j,h),zzee_born(0,0,2,j,h),ttype) ! V on lower_born
               call VVtoDD(2,h,q34(0,j),v,
     #              aauu_born(0,0,1,j,h),azuu_born(0,0,1,j,h),
     #              zauu_born(0,0,1,j,h),zzuu_born(0,0,1,j,h),ttype) ! V on upper    
               call VVtoDD(1,h,q12(0,j),v,
     #              aauu_born(0,0,2,j,h),azuu_born(0,0,2,j,h),
     #              zauu_born(0,0,2,j,h),zzuu_born(0,0,2,j,h),ttype) ! V on lower
...
c     CCl tensors for CC (k=5)
               ...
               call WWtoll(2,1,h,q34(0,j),v,CCee_born(0,0,1,j,h),ttype) ! V on upper
               call WWtoll(1,1,h,q12(0,j),v,CCee_born(0,0,2,j,h),ttype) ! V on lower
               call WWtoDD(2,h,q34(0,j),v,CCuu_born(0,0,1,j,h),ttype) ! V on upper
               call WWtoDD(1,h,q12(0,j),v,CCuu_born(0,0,2,j,h),ttype) ! V on lower
c     
C     for W-W+ --> q+ q- 
c     CCl for CC (k=6)
c     
               call WWtoll(1,1,h,q34(0,j),v,CCee6_born(0,0,1,j,h),ttype) ! V on upper
               call WWtoll(2,1,h,q12(0,j),v,CCee6_born(0,0,2,j,h),ttype) ! V on lower
               call WWtoDD(1,h,q34(0,j),v,CCuu6_born(0,0,1,j,h),ttype) ! V on upper
               call WWtoDD(2,h,q12(0,j),v,CCuu6_born(0,0,2,j,h),ttype) ! V on lower
c     
c     for V-> llb~b
c     
               call vto4ld(v,h,azz_born(0,h),zzz_born(0,h),ttype)

            endif               !not higgsonly
            
         enddo                  !h
      else if(decmode_slp.and.(mod(vdecaymodeZ2,2).eq.0)) then
c     consider 4 different combinations of lepton helicities:
         do h = 1,4,1
            ...
            CALL IXXXXX_5(v(0,1),ZERO ,+ie,-1,lep_born(1,h)) !e+	
            CALL OXXXXX_5(v(0,2),ZERO ,-ie, 1,lem_born(1,h)) !e- 
            CALL IXXXXX_5(v(0,3),ZERO ,+iu,-1,lup_born(1,h)) !tbar	
            CALL OXXXXX_5(v(0,4),ZERO ,-iu, 1,lum_born(1,h)) !t 
            
            CALL J_Z_BOSON_e(lep_born(1,h),lem_born(1,h),GC_50,
     &           GC_59,MZ,WZ,ze_born(1,h)) !Zl
            CALL J_A_BOSON(lep_born(1,h),lem_born(1,h),GC_4,
     &           ZERO ,ZERO  ,ae_born(1,h)) !Al
            CALL J_Z_BOSON_u(lup_born(1,h),lum_born(1,h),GC_51,
     &           GC_58,MZ,WZ,zu_born(1,h)) !Zu
            CALL J_A_BOSON(lup_born(1,h),lum_born(1,h),-GC_2,
     &           ZERO ,ZERO  ,au_born(1,h)) !Au
...
c     leptonic tensors
            j = 1
C     for AA,AZ,ZA,ZZ -> e+ e- t~ t :
            call vvtozu(q12(0,j),q34(0,j),v,h,
     #           aazz_born(0,0,j,h),azzz_born(0,0,j,h),
     #           zazz_born(0,0,j,h),zzzz_born(0,0,j,h),ttype)
...
C     for W+W-
            ...
            call wwtozu(q12(0,j),q34(0,j),v,h,wwzz5_born(0,0,j,h),ttype) ! q12 = W+
            call wwtozu(q34(0,j),q12(0,j),v,h,wwzz6_born(0,0,j,h),ttype) ! q12 = W-
            
            if (.not.higgs_only) then
               
C     for V1V2 --> e+ e- or  V1V2 -->  t~ t (Vi=A,Z)
c     NCl tensors for NC process (k=1...4)
c     
               call VVtoll(2,1,h,q34(0,j),v,
     #              aaee_born(0,0,1,j,h),azee_born(0,0,1,j,h),
     ...
     #              aaee_born(0,0,2,j,h),azee_born(0,0,2,j,h),
     #              zaee_born(0,0,2,j,h),zzee_born(0,0,2,j,h),ttype) ! V on lower_born
               call VVtoUU(2,h,q34(0,j),v,
     #              aauu_born(0,0,1,j,h),azuu_born(0,0,1,j,h),
     #              zauu_born(0,0,1,j,h),zzuu_born(0,0,1,j,h),ttype) ! V on upper    
               call VVtoUU(1,h,q12(0,j),v,
     #              aauu_born(0,0,2,j,h),azuu_born(0,0,2,j,h),
     #              zauu_born(0,0,2,j,h),zzuu_born(0,0,2,j,h),ttype) ! V on lower
c     
C     for W+W- --> e+ e- or  W+W- -->  t~ t
c     CCl tensors for CC (k=5)
               
               call WWtoll(2,1,h,q34(0,j),v,CCee_born(0,0,1,j,h),ttype) ! V on upper
               call WWtoll(1,1,h,q12(0,j),v,CCee_born(0,0,2,j,h),ttype) ! V on lower
               call WWtoUU(2,h,q34(0,j),v,CCuu_born(0,0,1,j,h),ttype) ! V on upper
               call WWtoUU(1,h,q12(0,j),v,CCuu_born(0,0,2,j,h),ttype) ! V on lower
c     
C     for W-W+ --> e+ e- or t~ t
c     CCl for CC (k=6)
c     
               call WWtoll(1,1,h,q34(0,j),v,CCee6_born(0,0,1,j,h),ttype) ! V on upper
               call WWtoll(2,1,h,q12(0,j),v,CCee6_born(0,0,2,j,h),ttype) ! V on lower
               call WWtoUU(1,h,q34(0,j),v,CCuu6_born(0,0,1,j,h),ttype) ! V on upper
               call WWtoUU(2,h,q12(0,j),v,CCuu6_born(0,0,2,j,h),ttype) ! V on lower
               
c     for V-> llt~t
c     
               call vto4lu(v,h,azz_born(0,h),zzz_born(0,h),ttype)
               
            endif               !not higgsonly
            
         enddo                  !h
      else
         print*, "Wrong decaymode"
      endif                     !decaymode_lep
