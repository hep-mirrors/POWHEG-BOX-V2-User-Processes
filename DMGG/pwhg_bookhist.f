...
C Call PWHGBOOK(N,'TITLE',DEL,XMIN,XMAX) for each histogram to be booked.
...
C Example:
C      call pwhgbook(2,'pt distribution',1.,10,70)
...
C
C FILLING:
C When it's time, call PWHGFILL(N,X,Y); this will add Y (real*4) to the bin 
C in which X (real*4) happens to be, within histogram N. 
C
C PLAYING AROUND:
C At the end of the day you may want to sum, divide, cancel, etc.etc.
C various histograms (bin by bin). Then you call PWHGOPERA(I,'O',J,K,X,Y). 
...
C
C FINAL ACCOUNTING:
C Now we can finalize our histograms; PWHGFINAL(N) will calculate the integral
C of the histogram N, the mean value of the X variable and its RMS.
C If we now want to renormalize the hist's, we can call PWHGNORM(N,X), which
C will normalize the integral to X  -- CAUTION: do not call PWHGNORM before
C PWHGFINAL, it will blow up.
C
C OUTPUT:
C To get a .dat file containing the values of the histograms, together with
C some information (like integral, mean values, etc.etc.) call PWHGPRINT(N),
C for each hist N that you want in the .dat file. Before the call to PWHGPRINT
...
C PWHGTOP(N,M,'X','Y','SCALE'). The points of the plot will be taken from histogram
...
C itself. Before calling PWHGTOP,
...
C
   ...
   1  BOOK(I)=' NO'
      DO 2 I=1,10
   2  BOOK2(I)=' NO'
      call pwhgresetmultitop
      END  


      subroutine pwhggettitle(N,TIT)
      implicit none
      character *(*) TIT
      integer n
      include 'pwhg_book.h'
      ...
      SUBROUTINE PWHGBOOK(N,TIT,DEL,XMIN,XMAX)
      implicit none
      include 'pwhg_book.h'
      integer n,i
      ...
      DATA ONEP/1.00001d0/
      IF(N.GT.NMB) THEN
	 CALL PWHGWARN('PWHGBOOK')
         WRITE(*,*) 'PWHGBOOK: no more then ',NMB,' histograms'
         STOP
      ENDIF
      NHIST = MAX(N,NHIST)
...
C
      ...
      SUBROUTINE PWHGINTEG(NIN,NOUT,IDIR,IPOW)
...
C of a given distribution. Overscores and underscores are included.
      ...
      I = (IDIR + 3)/2
      IF(NOUT.NE.NIN) THEN
      	CALL PWHGBOOK(NOUT,TITLE(NIN)//C(I), 
     &                HDEL(NIN),HMIN(NIN),HMAX(NIN))
      ENDIF
      IF(IDIR.EQ.1) THEN
         HIST(NOUT,1) = PWHGSUMPOW(HIST(NIN,1),USCORE(NIN),IPOW)
         IHIS(NOUT,1) = IHIS(NIN,1) + IUSCORE(NIN)
         XHIS(NOUT,1) = XHIS(NIN,1) + HDEL(NIN)/2
         DO L=2,M                      
            HIST(NOUT,L) = PWHGSUMPOW(HIST(NIN,L),HIST(NOUT,L-1),IPOW)
            IHIS(NOUT,L) = IHIS(NIN,L) + IHIS(NOUT,L-1) 
            XHIS(NOUT,L) = XHIS(NIN,L) + HDEL(NIN)/2
         ENDDO
         OSCORE(NOUT) = PWHGSUMPOW(OSCORE(NIN),HIST(NIN,M),IPOW)
         IOSCORE(NOUT) = IOSCORE(NIN) + IHIS(NIN,M)
      ELSEIF(IDIR.EQ.-1) THEN
         HIST(NOUT,M) = PWHGSUMPOW(HIST(NIN,M),OSCORE(NIN),IPOW)
         ...
         DO L=M-1,1,-1                        
            ...
            XHIS(NOUT,L) = XHIS(NIN,L) - HDEL(NIN)/2
         ENDDO
         USCORE(NOUT) = PWHGSUMPOW(USCORE(NIN),HIST(NIN,1),IPOW)
         IUSCORE(NOUT) = IUSCORE(NIN)+IHIS(NIN,1)
      ELSE                                 
         CALL PWHGWARN('PWHGINTEG')
         WRITE(*,*) 'Wrong idir in pwhginteg: OPERATION NOT PERFORMED'
         STOP
      ENDIF
      END

      FUNCTION PWHGSUMPOW(X,Y,IPOW)
      implicit none
      real * 8 x,y,pwhgsumpow
      integer ipow
      IF(IPOW.EQ.1) THEN
         PWHGSUMPOW = X + Y
      ELSEIF(IPOW.EQ.2) THEN
         PWHGSUMPOW = SQRT(X**2+Y**2)
      ELSEIF(IPOW.EQ.0) THEN
         CALL PWHGWARN('PWHGSUMPOW')
         WRITE(*,*)'Error: IPOW=0 not allowed in PWHGSUMPOW'
         STOP
      ELSE
         PWHGSUMPOW = (X**IPOW+Y**IPOW)**(1./IPOW)
      ENDIF
      END

      SUBROUTINE PWHGOPERA(I,OPER,J,K,X,Y)
      implicit none
      include 'pwhg_book.h'
      integer i,j,k,l
...
c Per marcare un istogramma
      ...
      BOOK(J) = NAME
      RETURN
      ENTRY pwhgGETTAG(J,TAG)
      TAG = BOOK(J)
      END

      SUBROUTINE PWHGFINAL(N)
      implicit none
      integer n
...
c      CALL TIME(CTIME)
   ...
   20 FORMAT(' ENTRIES=',I5,4X,'UNDERSCORE=',I5,4x,'OVERSCORE=',I5,//)
      ...
      SUBROUTINE PWHGTOP(N,M,BTIT,LTIT,SCALE)
      implicit none
      include 'pwhg_book.h'
...
c      CALL TIME(CTIME)
  ...
  400 FORMAT('   NEW PLOT')
      ...
      SUBROUTINE PWHGMNRTOP(N,M,BTIT,LTIT,SCALE)
      implicit none
      include 'pwhg_book.h'
      integer imon,iday,iyear,ini,n,m,j,itime(3)
      CHARACTER CTIME*8
...
c         CALL TIME(CTIME)
  ...
  400 FORMAT('   NEW PLOT')
      ...
      SUBROUTINE PWHGBOOK2(N,TIT,DELX,XMIN,XMAX,DELY,YMIN,YMAX)
...
C-- setup y-boundaries
   ...
   3  CONTINUE
      ...
      SUBROUTINE PWHGFILL2(N,X,Y,WGT)
      implicit real * 8 (a-h,o-z)
      include 'pwhg_book.h'
C-- find x-coordinate
...
C-- find y-coordinate
     ...
     *                                             THEN        
      ...
      SUBROUTINE PWHGOPERA2(I,OPER,J,K,X,Y)
      implicit real * 8 (a-h,o-z)
      include 'pwhg_book.h'
      CHARACTER OPER*1
...
c$$$      DO ND=1,NDOTS
c$$$      X=XHIS2(N,I)+(2.*RANdom()-1.)*HDEL2(1,N)
c$$$      Y=YHIS2(N,J)+(2.*RANdom()-1.)*HDEL2(2,N)
c$$$      WRITE(99,*) X,Y
c$$$      ENDDO
c$$$      NTOT=NTOT+NDOTS
c$$$  3   CONTINUE       
...
106   FORMAT(2X,'SET ORD X Y ',/,2(F8.4,1X),/,2(F8.4,1X),/,
     ...
     *       2X,'JOIN TEXT')                                    
      WRITE(99,107) BTIT,LTIT,SCALE,HMIN(NH),HMAX(NH),FMIN,FMAX
107   FORMAT(                                           
     &' TITLE BOTTOM ','"',A,'"',/1X,
     &' TITLE LEFT ','"',A,'"',/1X,
     &' SET SCALE Y ',A,/1X,
...
C
      ENTRY PWHGTFILL(NH,NE,N,M,BTIT,LTIT,SCA)
      IP=IP+1                             
      IF(IP.GT.4) IP=1
...
C This may happen if PWHGFINAL is called prior to PWHGCOPY
      IF(BOOK(NIN)(1:1).EQ.'R'.AND.HMIN(NIN).EQ.HMAX(NIN)) RETURN
      IF(BOOK(NOUT)(1:1).NE.'Y'.AND.BOOK(NOUT)(1:1).NE.'R') 
     &  CALL PWHGBOOK(NOUT,TITLE(NIN)(2:),HDEL(NIN),HMIN(NIN),HMAX(NIN))
...
c     !ER:
      open(unit=99,file=trim(adjustl(filename))//'.top')
ccccccccccccccccccccccccccccccccccccccccccccc
