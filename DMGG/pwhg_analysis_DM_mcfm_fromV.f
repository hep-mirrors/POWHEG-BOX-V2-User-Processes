...
c arrays to reconstruct jets
     ...
     $     ATLAS_min_dphi_j1j2,ATLAS_max_dphi_j1j2
      ...
      integer idlep,idnu,iddm,idhiggs,idm(2),ndm
      save channel,idlep,idnu,iddm,idhiggs

      if(ini) then
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
C FIRST SELECT THE CHANNEL
         if((abs(powheginput('decaymode')).eq.1).or.
     $        (abs(powheginput('decaymode')).eq.2)) channel='DM'
...
c$$$c     find DM particles...
c$$$      nlep=0
c$$$      if(WHCPRG.eq.'NLO   '.or.WHCPRG.eq.'LHE   '
c$$$     $     .or.WHCPRG.eq.'PYTHIA') then
c$$$         do ihep=1,nhep
c$$$            if(isthep(ihep).eq.1) then
c$$$               if(abs(idhep(ihep)).eq.idlep) then
c$$$                  nlep=nlep+1
c$$$                  ilep(nlep)=ihep
c$$$               endif
c$$$            endif
c$$$         enddo
c$$$         if(nlep.ne.2) then
c$$$            write(*,*) 'Problems with DM particles X: ',nlep
c$$$            write(*,*) 'SKIP EVENT'
c$$$            goto 999            ! reject event
c$$$         endif
c$$$      elseif (WHCPRG.eq.'HERWIG') then
c$$$         write(*,*) 'HERWIG analysis not implemented'
c$$$         call exit(1)
c$$$      else
c$$$         write(*,*) 'Wrong WHCPRG: ',WHCPRG
c$$$         call exit(1)
c$$$      endif
