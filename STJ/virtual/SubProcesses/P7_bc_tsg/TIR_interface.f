      SUBROUTINE ML5_7_TIRLOOP(I_SQSO,I_LOOPGROUP,I_LIB,NLOOPLINE,PL
     $ ,M2L,RANK,RES,STABLE)
C     
...
C     CONVERT TO MADLOOP CONVENTION
      CALL ML5_7_CONVERT_IREGI_COEFFS(RANK,PJCOEFS,TIRCOEFS)
      CASE(7)
C     COLLIER
...
C     ENDIF
      ...
      SUBROUTINE ML5_7_SWITCH_ORDER(CTMODE,NLOOPLINE,PL,PDEN,M2L)
      IMPLICIT NONE
      ...
      SUBROUTINE ML5_7_MP_SWITCH_ORDER(CTMODE,NLOOPLINE,PL,PDEN,M2L)
      IMPLICIT NONE
...
C     The initialization below is for CT v1.9.2+
      ...
      SUBROUTINE ML5_7_CHOOSE_LOOPLIB(LIBINDEX,NLOOPLINE,RANK
...
C      so that CutTools limitations can be correctly implemented
     ...
     $ ,.FALSE.,.FALSE.,.FALSE.,.FALSE.,.FALSE.,.FALSE.,.FALSE./
      DATA (HAS_AN_HEFT_VERTEX(I),I=    19,    21) /.FALSE.,.FALSE.
     $ ,.FALSE./
C     
C     GLOBAL VARIABLES
C     
...
C       DP EVALUATION
     ...
     $   ,HAS_AN_HEFT_VERTEX(LOOP_ID),MAX_SPIN_CONNECTED_TO_LOOP,LPASS)
      ...
      SUBROUTINE ML5_7_CLEAR_TIR_CACHE()
...
C      kept as it might be called by the MC).
      ...
      SUBROUTINE ML5_7_CONVERT_IREGI_COEFFS(RANK,IREGICOEFS,TIRCOEFS)
C     GLOABLE VARIABLES
