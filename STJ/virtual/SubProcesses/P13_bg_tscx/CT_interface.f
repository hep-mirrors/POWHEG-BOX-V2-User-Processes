...
C     ===========================================
      SUBROUTINE ML5_13_CTLOOP(NLOOPLINE,PL,M2L,RANK,RES,STABLE)
C     
...
C     
      EXTERNAL ML5_13_LOOPNUM
      EXTERNAL ML5_13_MPLOOPNUM
C     
C     GLOBAL VARIABLES
C     
      INCLUDE 'coupl.inc'
      ...
      COMMON/ML5_13_LOOP/ID,SQSOINDEX,R
...
C     CONVERT THE MOMENTA FLOWING IN THE LOOP LINES TO CT CONVENTIONS
      ...
      CALL CTSXCUT(CTMODE,LSCALE,MU_R,NLOOPLINE,ML5_13_LOOPNUM
     $ ,ML5_13_MPLOOPNUM,RANK,PCT,M2LCT,RES,ACC,R1,STABLE)
...
C     CALL CTSINIT(THRS,LOOPLIB)
      ...
      SUBROUTINE ML5_13_BUILD_KINEMATIC_MATRIX(NLOOPLINE,P_LOOP,M2L
     $ ,S_MAT)
C     
...
C            component
     ...
     $          + ABS(P_LOOP(J,K))
      ...
      SUBROUTINE ML5_13_MP_BUILD_KINEMATIC_MATRIX(NLOOPLINE,P_LOOP,M2L
     $ ,S_MAT)
C     
C     Helper function that compute the loop kinematic matrix with
C      proper thresholds
...
C            component
     ...
     $          + ABS(P_LOOP(J,K))
      ...
      SUBROUTINE ML5_13_LOOP_4(W1, W2, W3, W4, M1, M2, M3, M4,  RANK,
...
C       Choose the correct loop library
        CALL ML5_13_CHOOSE_LOOPLIB(LIBINDEX,NLOOPLINE,RANK
     $   ,COMPLEX_MASS,ID,DOING_QP,I_LIB)
        IF(MLREDUCTIONLIB(I_LIB).EQ.1)THEN
C         CutTools is used
          CALL ML5_13_CTLOOP(NLOOPLINE,PL,M2L,RANK,LOOPRES(1
     $     ,SQUAREDSOINDEX,LOOPNUM),S(SQUAREDSOINDEX,LOOPNUM))
        ELSE
C         Tensor Integral Reduction is used 
          CALL ML5_13_TIRLOOP(SQUAREDSOINDEX,LOOPNUM,I_LIB,NLOOPLINE
     ...
     $     ,S(SQUAREDSOINDEX,LOOPNUM))
      ...
      SUBROUTINE ML5_13_LOOP_3(W1, W2, W3, M1, M2, M3,  RANK,
     $  SQUAREDSOINDEX, LOOPNUM)
      INTEGER    NEXTERNAL
      PARAMETER (NEXTERNAL=5)
      INTEGER    NLOOPLINE
...
C         Tensor Integral Reduction is used 
     ...
     $     ,S(SQUAREDSOINDEX,LOOPNUM))
      ...
      SUBROUTINE ML5_13_LOOP_2(W1, W2, M1, M2,  RANK, SQUAREDSOINDEX,
     $  LOOPNUM)
      INTEGER    NEXTERNAL
      PARAMETER (NEXTERNAL=5)
      INTEGER    NLOOPLINE
