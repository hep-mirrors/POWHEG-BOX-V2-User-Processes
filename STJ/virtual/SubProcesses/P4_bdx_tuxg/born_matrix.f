      SUBROUTINE ML5_4_SMATRIX(P,ANS_SUMMED)
C     
...
C     BEGIN CODE
C     
      CALL ML5_4_SMATRIX_SPLITORDERS(P,ANS)
      ANS_SUMMED=ANS(0)

      END

      SUBROUTINE ML5_4_SMATRIXHEL(P,HEL,ANS)
      IMPLICIT NONE
C     
C     CONSTANT
C     
...
C     
      INTEGER USERHEL
      COMMON/ML5_4_HELUSERCHOICE/USERHEL
C     ----------
C     BEGIN CODE
C     ----------
      USERHEL=HEL
      CALL ML5_4_SMATRIX(P,ANS)
      USERHEL=-1

      END

      SUBROUTINE ML5_4_SMATRIX_SPLITORDERS(P,ANS)
C     
...
C     
      ...
      COMMON/ML5_4_CHOSEN_BORN_SQSO/CHOSEN_SO_CONFIGS
C     
C     ARGUMENTS 
C     
...
C     
      ...
      COMMON/ML5_4_BORN_HEL_CONFIGS/NHEL
      ...
      COMMON/ML5_4_BORN_BEAM_POL/POLARIZATIONS
C     
C     FUNCTIONS
C     
      LOGICAL ML5_4_IS_BORN_HEL_SELECTED
...
C      only three external particles.
     ...
     $       -1.AND.(.NOT.ML5_4_IS_BORN_HEL_SELECTED(IHEL))) THEN
              CYCLE
            ENDIF
            CALL ML5_4_MATRIX(P ,NHEL(1,IHEL),JC(1), T)
            BUFF=0D0
            DO I=1,NSQAMPSO
              IF(POLARIZATIONS(0,0).EQ.-1.OR.ML5_4_IS_BORN_HEL_SELECTED
     $(IHEL)) THEN
                ANS(I)=ANS(I)+T(I)
              ENDIF
              BUFF=BUFF+T(I)
      ...
      SUBROUTINE ML5_4_SMATRIXHEL_SPLITORDERS(P,HEL,ANS)
      IMPLICIT NONE
C     
C     CONSTANT
C     
...
C     ----------
      ...
      SUBROUTINE ML5_4_MATRIX(P,NHEL,IC,RES)
C     
C     Generated by MadGraph5_aMC@NLO v. 2.5.0.alpha, 2016-08-28
C     By the MadGraph5_aMC@NLO Development Team
C     Visit launchpad.net/madgraph5 and amcatnlo.web.cern.ch
...
C     Amplitude(s) for diagram number 2
      CALL FFV2_0(W(1,7),W(1,2),W(1,6),GC_47,AMP(2))
      CALL FFV1_2(W(1,1),W(1,5),GC_5,ZERO,ZERO,W(1,7))
      CALL FFV2_3(W(1,4),W(1,2),GC_47,MDL_MW,MDL_WW,W(1,6))
...
C     JAMPs contributing to orders QCD=1
      ...
      DO M = 1, NAMPSO
        DO I = 1, NCOLOR
          ...
          ENDDO
          DO N = 1, NAMPSO
            RES(ML5_4_SQSOINDEX(M,N)) = RES(ML5_4_SQSOINDEX(M,N)) +
     $        ZTEMP*DCONJG(JAMP(I,N))/DENOM(I)
      ...
      SUBROUTINE ML5_4_GET_ME(P, ALPHAS, NHEL ,ANS)
      IMPLICIT NONE
C     
C     CONSTANT
C     
...
C     the include file with the values of the parameters and masses 
      ...
      IF (NHEL.NE.0) THEN
        CALL ML5_4_SMATRIXHEL(P, NHEL, ANS)
      ELSE
        CALL ML5_4_SMATRIX(P, ANS)
      ENDIF
      RETURN
      ...
      SUBROUTINE ML5_4_INITIALISE(PATH)
C     ROUTINE FOR F2PY to read the benchmark point.    
      IMPLICIT NONE
      CHARACTER*180 PATH
CF2PY INTENT(IN) :: PATH
      ...
      LOGICAL FUNCTION ML5_4_IS_BORN_HEL_SELECTED(HELID)
      IMPLICIT NONE
C     
C     CONSTANTS
C     
...
C     
      ...
      COMMON/ML5_4_BORN_HEL_CONFIGS/HELC
...
C     ----------
      ...
      ML5_4_IS_BORN_HEL_SELECTED = .TRUE.
      ...
      DO I=1,NEXTERNAL
        ...
        ENDDO
        IF(.NOT.FOUNDIT) THEN
          ML5_4_IS_BORN_HEL_SELECTED = .FALSE.
          RETURN
        ENDIF
      ENDDO
...
C     Set of functions to handle the array indices of the split orders
      ...
      INTEGER FUNCTION ML5_4_SQSOINDEX(ORDERINDEXA, ORDERINDEXB)
C     
...
C     
      ...
      COMMON/ML5_4_AMPSPLITORDERS/AMPSPLITORDERS
C     
C     FUNCTION
C     
      INTEGER ML5_4_SOINDEX_FOR_SQUARED_ORDERS
C     
C     BEGIN CODE
C     
      DO I=1,NSO
     ...
     $NDEXB,I)
      ENDDO
      ML5_4_SQSOINDEX=ML5_4_SOINDEX_FOR_SQUARED_ORDERS(SQORDERS)
      END

      INTEGER FUNCTION ML5_4_SOINDEX_FOR_SQUARED_ORDERS(ORDERS)
C     
...
C     
      ...
      COMMON/ML5_4_SQPLITORDERS/SQPLITORDERS
C     
C     BEGIN CODE
C     
      DO I=1,NSQSO
        ...
        ENDDO
        ML5_4_SOINDEX_FOR_SQUARED_ORDERS = I
        RETURN
 1009   CONTINUE
      ENDDO

      WRITE(*,*) 'ERROR:: Stopping in function'
      WRITE(*,*) 'ML5_4_SOINDEX_FOR_SQUARED_ORDERS'
      WRITE(*,*) 'Could not find squared orders ',(ORDERS(I),I=1,NSO)
      STOP

      END

      SUBROUTINE ML5_4_GET_NSQSO_BORN(NSQSO)
C     
...
C      Not directly useful, but provided nonetheless.
      SUBROUTINE ML5_4_GET_SQUARED_ORDERS_FOR_SOINDEX(SOINDEX,ORDERS)
C     
...
C     
      ...
      WRITE(*,*) 'ERROR:: Stopping function ML5_4_GET_SQUARED_ORDERS_FO'
     $ //'R_SOINDEX'
...
C      Not directly useful, but provided nonetheless.
      SUBROUTINE ML5_4_GET_ORDERS_FOR_AMPSOINDEX(SOINDEX,ORDERS)
C     
...
C     
      ...
      WRITE(*,*) 'ERROR:: Stopping function ML5_4_GET_ORDERS_FOR_AMPSOI'
     $ //'NDEX'
...
C      completeness
      INTEGER FUNCTION ML5_4_SOINDEX_FOR_AMPORDERS(ORDERS)
C     
...
C     
      DO I=1,NAMPSO
        ...
        ENDDO
        ML5_4_SOINDEX_FOR_AMPORDERS = I
        RETURN
 1009   CONTINUE
      ENDDO

      WRITE(*,*) 'ERROR:: Stopping function ML5_4_SOINDEX_FOR_AMPORDERS'
     $ //''
      WRITE(*,*) 'Could not find squared orders ',(ORDERS(I),I=1,NSO)
      STOP
