...
C     --=========================================--
      ...
      SUBROUTINE ML5_8_SLOOPMATRIX(P_USER,ANS)
C     
...
C     
C     Process: d~ g > t u~ b~ [ virt = QCD ] $$ w+ w- @8
C     
C     Modules
C     
      USE ML5_8_POLYNOMIAL_CONSTANTS
C     
      IMPLICIT NONE
C     
...
C      provide QP
      ...
      COMMON/ML5_8_STABILITY_TESTS/MAXSTABILITYLENGTH
...
C      DIFFERENT EVALUATION METHODS IN ORDER TO ASSESS STABILITY.
     ...
     $ /NSQSOXNLG*.FALSE./
      ...
      COMMON/ML5_8_RUNTIME_OPTIONS/AUTOMATIC_CACHE_CLEARING
...
C      stability check to take place
      ...
      COMMON/ML5_8_BYPASS_CHECK/BYPASS_CHECK, ALWAYS_TEST_STABILITY
C     
C     FUNCTIONS
C     
      INTEGER ML5_8_TIRCACHE_INDEX
      INTEGER ML5_8_ML5SOINDEX_FOR_BORN_AMP
      INTEGER ML5_8_ML5SOINDEX_FOR_LOOP_AMP
      INTEGER ML5_8_ML5SQSOINDEX
      INTEGER ML5_8_ISSAME
      LOGICAL ML5_8_ISZERO
      LOGICAL ML5_8_IS_HEL_SELECTED
...
C     
      ...
      COMMON/ML5_8_CHOSEN_LOOP_SQSO/CHOSEN_SO_CONFIGS

      INTEGER N_DP_EVAL, N_QP_EVAL
      DATA N_DP_EVAL/1/
      DATA N_QP_EVAL/1/
      COMMON/ML5_8_N_EVALS/N_DP_EVAL,N_QP_EVAL

      LOGICAL CHECKPHASE
      DATA CHECKPHASE/.TRUE./
      LOGICAL HELDOUBLECHECKED
      DATA HELDOUBLECHECKED/.FALSE./
      COMMON/ML5_8_INIT/CHECKPHASE, HELDOUBLECHECKED
      INTEGER NTRY
      DATA NTRY/0/
      REAL*8 REF
      DATA REF/0.0D0/
...
C      AVAILABLE OR NOT
     ...
     $ ,.FALSE.,.FALSE./
      COMMON/ML5_8_LOOPLIBS_AV/ LOOPLIBS_AVAILABLE
...
C     Typically, the hel_offset is 10000
      ...
      COMMON/ML5_8_FILTERS/GOODAMP,GOODHEL,HELOFFSET

      INTEGER HELPICKED
      DATA HELPICKED/-1/
      COMMON/ML5_8_HELCHOICE/HELPICKED
      INTEGER USERHEL
      DATA USERHEL/-1/
      COMMON/ML5_8_USERCHOICE/USERHEL
...
C      SQSO_TARGET and below.
      INTEGER SQSO_TARGET
      DATA SQSO_TARGET/-1/
      COMMON/ML5_8_SOCHOICE/SQSO_TARGET
...
C      the rest can be skipped.
     ...
     $ ,CTCALL_REQ_SO_DONE,FILTER_SO
      ...
      COMMON/ML5_8_SO_REQS/UVCT_REQ_SO_DONE,MP_UVCT_REQ_SO_DONE
...
C     RETURNCODE=100*RET_CODE_H+10*RET_CODE_T+RET_CODE_U
      ...
      COMMON/ML5_8_ACC/ACCURACY,RET_CODE_H,RET_CODE_T,RET_CODE_U

      LOGICAL MP_DONE_ONCE
      DATA MP_DONE_ONCE/.FALSE./
      COMMON/ML5_8_MP_DONE_ONCE/MP_DONE_ONCE
...
C      reading the parameters
     ...
     $ ,FORCED_CHOICE_OF_COLLIER_IR_POLE_COMPUTATION/.FALSE.,.FALSE./
      COMMON/ML5_8_COLLIERPOLESFORCEDCHOICE/FORCED_CHOICE_OF_COLLIER_UV
...
C      independently by each SubProcess.
      ...
      COMMON/ML5_8_FPE_IN_REDUCTION/FPE_IN_DP_REDUCTION,
     $  FPE_IN_QP_REDUCTION
...
C      requirement.
      ...
      COMMON/ML5_8_BEAM_POL/POLARIZATIONS
...
C       Setup the file paths
        ...
        CALL ML5_8_SET_N_EVALS(N_DP_EVAL,N_QP_EVAL)
...
C         We write a dummy filter for structural reasons here
 ...
 104    CONTINUE
     ...
     $   //' ML5_8_ColorNumFactors.dat. File not found'
 105    CONTINUE
        CLOSE(1)
 ...
 106    CONTINUE
        STOP 'Color factors could not be initialized from file'
     $   //' ML5_8_ColorDenomFactors.dat. File not found'
 107    CONTINUE
        CLOSE(1)
 ...
 108    CONTINUE
     ...
     $   //' from file ML5_8_HelConfigs.dat. File not found'
 109    CONTINUE
        CLOSE(1)
...
C     the split_order summed value I=0 is used in ML5 code.
      DO I=0,NSQSO_BORN
        BORNBUFF(I)=0.0D0
      ENDDO
      CALL ML5_8_SMATRIXHEL_SPLITORDERS(P_USER,USERHEL,BORNBUFF(0))
      DO I=0,NSQSO_BORN
        ANS(0,I)=BORNBUFF(I)
      ENDDO
...
C        energy-momentum conservation)
        CALL ML5_8_IMPROVE_PS_POINT_PRECISION(PS)
      ENDIF
...
C     Check if we directly go to multiple precision
      IF (CTMODE.GE.4) THEN
        IF (.NOT.MP_DONE) THEN
          CALL ML5_8_MP_COMPUTE_LOOP_COEFS(MP_P,BUFFR_BIS)
...
C         Handle the possible requirement of specific polarizations
     ...
     $0,0).EQ.0.AND.(.NOT.ML5_8_IS_HEL_SELECTED(H))) THEN
            CYCLE
          ENDIF
...
C          associated to given loops
          CALL ML5_8_HELAS_CALLS_AMPB_1(P,NHEL,H,IC)
 2000     CONTINUE
          CT_REQ_SO_DONE=.TRUE.
...
C         FeynRules, there are none of these type of counterterms.
          CALL ML5_8_HELAS_CALLS_UVCT_1(P,NHEL,H,IC)
 3000     CONTINUE
          ...
          DO J=1,NBORNAMPS
            ...
            DO I=1,NCTAMPS
              ...
              ITEMP = ML5_8_ML5SQSOINDEX(ML5_8_ML5SOINDEX_FOR_LOOP_AMP(
     $I),ML5_8_ML5SOINDEX_FOR_BORN_AMP(J))
          ...
          CALL ML5_8_COEF_CONSTRUCTION_1(P,NHEL,H,IC)
 4000     CONTINUE
...
C     Free cache when using IREGI
     ...
     $_DONE)) THEN
      ...
      DO I_SO=1,NSQUAREDSO
        ...
        CALL ML5_8_LOOP_CT_CALLS_1(P,NHEL,H,IC)
        GOTO 5001
 5000   CONTINUE
...
C          this helicity
          IF (POLARIZATIONS(0,0).EQ.-1.OR.ML5_8_IS_HEL_SELECTED(HELPICK
     $ED)) THEN
...
C         SET THE HELICITY FILTER
          IF(.NOT.FOUNDHELFILTER) THEN
            ...
            IF(ML5_8_ISZERO(DABS(HELSAVED(1,HELPICKED))
...
C                 Be looser for helicity check, bring a factor 100
                  DUMMY=ML5_8_ISSAME(HELSAVED(1,HELPICKED),HELSAVED(1
     $             ,H),REF,.FALSE.)
                  IF(DUMMY.NE.0) THEN
                    IF(NTRY.EQ.1) THEN
...
C         SET THE LOOP FILTER
          IF(.NOT.FOUNDLOOPFILTER.AND.USELOOPFILTER) THEN
            DO I=1,NLOOPGROUPS
              DO J=1,NSQUAREDSO
                IF(.NOT.ML5_8_ISZERO(ABS(LOOPRES(1,J,I))+ABS(LOOPRES(2
...
C         DOUBLE CHECK THE HELICITY FILTER
          IF (GOODHEL(HELPICKED).EQ.-HELOFFSET) THEN
            IF (.NOT.ML5_8_ISZERO(DABS(HELSAVED(1,HELPICKED))
 ...
 348          CONTINUE
          ...
          IF (GOODHEL(HELPICKED).LT.-HELOFFSET.AND.NTRY.NE.0) THEN
            IF(ML5_8_ISSAME(HELSAVED(1,HELPICKED),HELSAVED(1
...
C            answer from mode 1 and carry on.
     ...
     $.1).OR.((.NOT.DOING_QP_EVALS).AND.NROTATIONS_DP.GE.1)) ) THEN
          EVAL_DONE(3)=.TRUE.
          CALL ML5_8_ROTATE_PS(PS,P,1)
          IF (DOING_QP_EVALS) CALL ML5_8_MP_ROTATE_PS(MP_PS,MP_P,1)
          GOTO 200
        ENDIF
     ...
     $.2).OR.((.NOT.DOING_QP_EVALS).AND.NROTATIONS_DP.GE.2)) ) THEN
          EVAL_DONE(4)=.TRUE.
          CALL ML5_8_ROTATE_PS(PS,P,2)
          IF (DOING_QP_EVALS) CALL ML5_8_MP_ROTATE_PS(MP_PS,MP_P,2)
          GOTO 200
        ENDIF

        CALL ML5_8_ROTATE_PS(PS,P,0)
        IF (DOING_QP_EVALS) CALL ML5_8_MP_ROTATE_PS(MP_PS,MP_P,0)

C       END OF THE DEFINITIONS OF THE DIFFERENT EVALUATION METHODS

        IF(DOING_QP_EVALS.AND.LOOPLIBS_QPAVAILABLE(MLREDUCTIONLIB(I_LIB)
     $   )) THEN
          CALL ML5_8_COMPUTE_ACCURACY(QP_RES,N_QP_EVAL,ACC,ANS)
...
C          accuracies to 1.0
     ...
     $         ,.FALSE.)
              NEPS=NEPS+1
              CALL ML5_8_COMPUTE_ACCURACY(DP_RES,N_DP_EVAL,TEMP1,TEMP)
              CALL ML5_8_COMPUTE_ACCURACY(QP_RES,N_QP_EVAL,ACC,ANS)
              IF(NEPS.LE.10) THEN
...
C              flags.
        ...
        ELSEIF(.NOT.DOING_QP_EVALS)THEN
          CALL ML5_8_COMPUTE_ACCURACY(DP_RES,N_DP_EVAL,ACC,ANS)
C         If a floating point exception was encountered during the
C          reduction,
C         the result cannot be trusted at all and we hardset all
C          accuracies to 1.0
...
C     --=========================================--
      ...
      LOGICAL FUNCTION ML5_8_IS_HEL_SELECTED(HELID)
      IMPLICIT NONE
C     
C     CONSTANTS
C     
...
C     ----------
      ...
      LOGICAL FUNCTION ML5_8_ISZERO(TOTEST, REFERENCE_VALUE, LOOP,
     $  SOINDEX)
      IMPLICIT NONE
C     
C     CONSTANTS
...
C     ----------
     ...
     $   //' zero.'
        STOP 1
      ELSE
        ML5_8_ISZERO=((ABS(TOTEST)/ABS(REFERENCE_VALUE)).LT.ZEROTHRES)
      ENDIF

      IF(LOOP.NE.-1) THEN
        IF((.NOT.ML5_8_ISZERO).AND.(.NOT.S(SOINDEX,LOOP))) THEN
     ...
     $     ,(ABS(TOTEST)/ABS(REFERENCE_VALUE)),' but is unstable.'
      ...
      INTEGER FUNCTION ML5_8_ISSAME(RESA,RESB,REF,USEMAX)
      IMPLICIT NONE
...
C      inconsistency, then we only allow for weight one comparisons.
      ...
      DO I=1,N_WGT_TO_TRY
        DO J=1,3
          IF (ML5_8_ISZERO(ABS(RESB(J)),REF,-1,-1)) THEN
            IF(.NOT.ML5_8_ISZERO(ABS(RESB(J))+ABS(RESA(J)),REF,-1,-1))
     $        THEN
              GOTO 1231
            ENDIF
C           Be looser for helicity comparison, so bring a factor 100
          ELSEIF(.NOT.ML5_8_ISZERO(ABS((RESA(J)/RESB(J))
     $     -DBLE(WGT_TO_TRY(I))),1.0D0,-1,-1)) THEN
            GOTO 1231
          ENDIF
        ENDDO
        ML5_8_ISSAME = WGT_TO_TRY(I)
        RETURN
 1231   CONTINUE
      ENDDO
      END

      SUBROUTINE ML5_8_COMPUTE_ACCURACY(FULLLIST, LENGTH, ACC,
     $  ESTIMATE)
      IMPLICIT NONE
C     
C     PARAMETERS 
...
C        use the average but rather the first evaluation.
     ...
     $EST) THEN
      ...
      SUBROUTINE ML5_8_SET_N_EVALS(N_DP_EVALS,N_QP_EVALS)
...
C      VARIABLES FROM A GIVEN VARIABLE IN DOUBLE PRECISION
      ...
      DO I=1,NEXTERNAL
        ...
        ENDDO
      ENDDO
      CALL ML5_8_MP_IMPROVE_PS_POINT_PRECISION(MP_PS)
      DO I=1,NEXTERNAL
        DO J=0,3
          MP_P(J,I)=MP_PS(J,I)
        ENDDO
...
C     --=========================================--
      ...
      SUBROUTINE ML5_8_GET_NSQSO_LOOP(NSQSO)
C     
...
C     
      ...
      SUBROUTINE ML5_8_GET_ANSWER_DIMENSION(ANS_DIM)
C     
...
C     
      ...
      INTEGER FUNCTION ML5_8_ML5SOINDEX_FOR_SQUARED_ORDERS(ORDERS)
C     
...
C     
      ...
      COMMON/ML5_8_ML5SQPLITORDERS/SQPLITORDERS
C     
C     BEGIN CODE
C     
      DO I=1,NSQSO
        ...
        ENDDO
        ML5_8_ML5SOINDEX_FOR_SQUARED_ORDERS = I
        RETURN
 1009   CONTINUE
      ENDDO

      WRITE(*,*) 'ERROR:: Stopping function ML5_8_ML5SOINDEX_FOR_SQUARE'
     $ //'D_ORDERS'
      ...
      INTEGER FUNCTION ML5_8_ML5SOINDEX_FOR_BORN_AMP(AMPID)
C     
...
C     -----------
     ...
     $   //' maximum ',NBORNAMPS
      ENDIF
      ML5_8_ML5SOINDEX_FOR_BORN_AMP = BORNAMPORDERS(AMPID)

      END

      INTEGER FUNCTION ML5_8_ML5SOINDEX_FOR_LOOP_AMP(AMPID)
C     
...
C     -----------
     ...
     $   //' maximum ',NLOOPAMPS
      ENDIF
      ML5_8_ML5SOINDEX_FOR_LOOP_AMP = LOOPAMPORDERS(AMPID)

      END


      INTEGER FUNCTION ML5_8_ML5SQSOINDEX(ORDERINDEXA, ORDERINDEXB)
C     
...
C     
      ...
      COMMON/ML5_8_ML5AMPSPLITORDERS/AMPSPLITORDERS
C     
C     FUNCTION
C     
      INTEGER ML5_8_ML5SOINDEX_FOR_SQUARED_ORDERS
C     
C     BEGIN CODE
C     
      DO I=1,NSO
     ...
     $NDEXB,I)
      ENDDO
      ML5_8_ML5SQSOINDEX=ML5_8_ML5SOINDEX_FOR_SQUARED_ORDERS(SQORDERS)
      END

C     This is the inverse subroutine of ML5SOINDEX_FOR_SQUARED_ORDERS.
C      Not directly useful, but provided nonetheless.
      SUBROUTINE ML5_8_ML5GET_SQUARED_ORDERS_FOR_SOINDEX(SOINDEX
     $ ,ORDERS)
C     
...
C     
      ...
      WRITE(*,*) 'ERROR:: Stopping function ML5_8_ML5GET_SQUARED_ORDERS'
     $ //'_FOR_SOINDEX'
...
C      Not directly useful, but provided nonetheless.
      SUBROUTINE ML5_8_ML5GET_ORDERS_FOR_AMPSOINDEX(SOINDEX,ORDERS)
C     
...
C     
      ...
      WRITE(*,*) 'ERROR:: Stopping function ML5_8_ML5GET_ORDERS_FOR_AMP'
     $ //'SOINDEX'
...
C      completeness
      INTEGER FUNCTION ML5_8_ML5SOINDEX_FOR_AMPORDERS(ORDERS)
C     
...
C     
      DO I=1,NAMPSO
        ...
        ENDDO
        ML5_8_ML5SOINDEX_FOR_AMPORDERS = I
        RETURN
 1009   CONTINUE
      ENDDO

      WRITE(*,*) 'ERROR:: Stopping function ML5_8_ML5SOINDEX_FOR_AMPORD'
     $ //'ERS'
      WRITE(*,*) 'Could not find squared orders ',(ORDERS(I),I=1,NSO)
      STOP
...
C     --=========================================--
      ...
      SUBROUTINE ML5_8_COLLIER_COMPUTE_UV_POLES(ONOFF)
C     
...
C      overwrite the choice made here
      ...
      SUBROUTINE ML5_8_COLLIER_COMPUTE_IR_POLES(ONOFF)
C     
C     This function can be called by the MadLoop user so as to chose
C      to have COLLIER
...
C      overwrite the choice made here
      ...
      SUBROUTINE ML5_8_FORCE_STABILITY_CHECK(ONOFF)
C     
...
C     
      ...
      SUBROUTINE ML5_8_SET_AUTOMATIC_CACHE_CLEARING(ONOFF)
C     
...
C     
     ...
     $ //' SET_AUTOMATIC_CACHE_CLEARING did nothing.'
      ...
      SUBROUTINE ML5_8_SET_COUPLINGORDERS_TARGET(SOTARGET)
      IMPLICIT NONE
C     
...
C     ----------
      ...
      SUBROUTINE ML5_8_SET_LEG_POLARIZATION(LEG_ID, LEG_POLARIZATION)
      IMPLICIT NONE
C     
C     ARGUMENTS
C     
...
C     ----------
      ...
      ELSE
        ...
        ENDDO
      ENDIF
      CALL ML5_8_SET_LEG_POLARIZATIONS(LEG_ID,LEG_POLARIZATIONS)

      END

      SUBROUTINE ML5_8_SET_LEG_POLARIZATIONS(LEG_ID, LEG_POLARIZATIONS)
      IMPLICIT NONE
C     
C     CONSTANTS
C     
...
C     leg is POLARIZATIONS(legID,1: POLARIZATIONS(legID,0)   ).
      ...
      COMMON/ML5_8_BORN_BEAM_POL/BORN_POLARIZATIONS
...
C     ----------
      ...
      SUBROUTINE ML5_8_SLOOPMATRIXHEL(P,HEL,ANS)
      IMPLICIT NONE
C     
C     CONSTANTS
C     
...
C     ----------
      USERHEL=HEL
      CALL ML5_8_SLOOPMATRIX(P,ANS)
      END

      SUBROUTINE ML5_8_SLOOPMATRIXHEL_THRES(P,HEL,ANS,PREC_ASKED
     $ ,PREC_FOUND,RET_CODE)
      IMPLICIT NONE
C     
C     CONSTANTS
...
C     
      ...
      INTEGER H,T,U
      REAL*8 ACCURACY(0:NSQUAREDSO)
      COMMON/ML5_8_ACC/ACCURACY,H,T,U
...
C     ----------
      ...
      CALL ML5_8_SLOOPMATRIXHEL(P,HEL,ANS)
...
C     Reset it to default value not to affect next runs
      ...
      SUBROUTINE ML5_8_SLOOPMATRIX_THRES(P,ANS,PREC_ASKED,PREC_FOUND
     $ ,RET_CODE)
C     
C     Inputs are:
