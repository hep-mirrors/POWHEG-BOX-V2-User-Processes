...
C      CALLS BUILDING THE LOOP
      ...
      SUBROUTINE ML5_11_LOOPNUM(Q,RES)
C     
C     CONSTANTS 
C     
      INTEGER    NLOOPGROUPS
...
C     
      ...
      COMMON/ML5_11_LOOP/ID,SQSOINDEX,RANK

      COMPLEX*16 LOOPCOEFS(0:LOOPMAXCOEFS-1,NSQUAREDSO,NLOOPGROUPS)
      COMMON/ML5_11_LCOEFS/LOOPCOEFS

      RES=(0.0D0,0.0D0)

      CALL ML5_11_EVAL_POLY(LOOPCOEFS(0,SQSOINDEX,ID),RANK,-Q,RES)
      END

      SUBROUTINE ML5_11_MPLOOPNUM(Q,RES)
C     
C     MODULE
C     
      INCLUDE 'cts_mprec.h'
...
C     
      ...
      COMMON/ML5_11_MP_LCOEFS/LOOPCOEFS
...
C      PURPOSE
      ...
      COMMON/ML5_11_MP_PSPOINT/MP_PS,MP_P
...
C     ----------
      ...
      IF (.NOT.MP_DONE) THEN
        CALL ML5_11_MP_COMPUTE_LOOP_COEFS(MP_P,DUMMY)
        MP_DONE=.TRUE.
      ENDIF

      CALL MP_ML5_11_EVAL_POLY(LOOPCOEFS(0,SQSOINDEX,ID),RANK,-QQ,QRES)
      RES=QRES

      END

      SUBROUTINE ML5_11_MPLOOPNUM_DUMMY(Q,RES)
C     
C     MODULE
C     
      INCLUDE 'cts_mprec.h'
