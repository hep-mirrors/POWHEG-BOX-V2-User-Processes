...
c      write(nlf,'(a)') '<init>'
     ...
     1        pdfgup(1),pdfgup(2),pdfsup(1),pdfsup(2),idwtup,nprup
         ...
         write(buffer,110) -idbmup(2),-idbmup(1),ebmup(2),ebmup(1),
     1        pdfgup(2),pdfgup(1),pdfsup(2),pdfsup(1),idwtup,nprup
      endif
      call  pwhg_io_write(nlf,trim(buffer))
      do 100 ipr=1,nprup
...
c      write(nlf,'(a)')'<event>'
     ...
     &           vtimup(i),spinup(i)
            ...
            ! charge conjugation (idup(i) and icolup(1:2,i)) and parity
            ! flip (pup(1:3,i))
            if(i.eq.1) then
               k=2
            elseif(i.eq.2) then
            ...
            if (abs(idup(k)).le.6) then
               idtmp=-idup(k)
            else
               idtmp=idup(k)
            endif
            write(buffer,220) idtmp,istup(k),mothup(1,k),
     $           mothup(2,k),icolup(2,k),icolup(1,k),(-pup(j,k),j=1,3),
     $           (pup(j,k),j=4,5),vtimup(k),spinup(k)
         endif
         call  pwhg_io_write(nlf,trim(buffer))
 200  continue
