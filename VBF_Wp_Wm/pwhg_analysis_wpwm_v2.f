c  The next subroutine opens some histograms and prepares them 
c      to receive data 
...
c syntax is:      call  bookupeqbins('name',binsize,lower,upper)
c
      call bookupeqbins('sig(no cuts)',1d0,0d0,1d0)

C -----------------------------------------------------

c cross section with pt_jet constraint only:
c
      call bookupeqbins('sig(no lept cuts)',1d0,0d0,1d0)
...
C     -- HISTOGRAMS WITH VBF CUTS 
      ...
      call bookupeqbins('PT jet 1- VBF CUTS',40d0,0d0,880d0)

      call bookupeqbins('PT jet 2- VBF CUTS',20d0,0d0,520d0)

      call bookupeqbins('Y jet 1- VBF CUTS',0.4d0,-4d0,4d0)

      call bookupeqbins('Y jet 2- VBF CUTS',0.4d0,-4d0,4d0)

      call bookupeqbins('Y j1j2(tag)- VBF CUTS',0.4d0,-6d0,6d0)
    
      call bookupeqbins('PT LEPT- VBF CUTS',20d0,0d0,440d0)

      call bookupeqbins('Eta_lept- VBF CUTS',0.4d0,-4d0,4d0)

      call bookupeqbins('M(l1l2)- VBF CUTS',60d0,0d0,960d0)

      call bookupeqbins('Phi(j1j2)- VBF CUTS',0.2d0,0d0,3.2d0)

      call bookupeqbins('Pt J3- VBF CUTS',5d0,0d0,500d0)

      call bookupeqbins('Y J3- VBF CUTS',0.1d0,-5d0,5d0)

      call bookupeqbins('Ptrel J1- VBF CUTS',10d0,0d0,200d0)

      call bookupeqbins('M_j1j2(tag) - VBF CUTS',200d0,0d0,3000d0)

      call bookupeqbins('y*- VBF CUTS',0.4d0,-4d0,4d0)
...
c     from pico to femto      
     ...
     $        ) then
             ...
             write(*,*) '**************************************'
             write(*,*) ' template analysis works only for jj+WpWm  '
...
c  fully inc cross section
      call filld('sig(no cuts)',0.5d0,dsig)
...
ccccccc
c
c syntax for histograms is 
c      call filld('name', x, y)


c cross section with ptj constraint only
      call filld('sig(no lept cuts)',0.5d0,dsig)
...
c     PT of jet 1
      call filld('PT jet 1- VBF CUTS',kt(itag1),dsig)
c     PT of jet 2
      call filld('PT jet 2- VBF CUTS',kt(itag2),dsig)
c     Y of jet 1
      call filld('Y jet 1- VBF CUTS',rap(itag1),dsig)
c     Y of jet 2
      call filld('Y jet 2- VBF CUTS',rap(itag2),dsig)
c     Y jet1 - Y jet 2
      pj12 = pj(:4,itag1)+pj(:4,itag2)
      y12 = rap(itag1)-rap(itag2)
      call filld('Y j1j2(tag)- VBF CUTS',y12,dsig)

C     Pt lept     
      call filld('PT LEPT- VBF CUTS',ptl1,dsig/2d0)
      call filld('PT LEPT- VBF CUTS',ptl2,dsig/2d0)

C     ETA lept
      call filld('Eta_lept- VBF CUTS',etal1,dsig/2d0)
      call filld('Eta_lept- VBF CUTS',etal2,dsig/2d0)

C     M(l1l2) 
      pl12 = phep(:4,ileptons(ltag1))+phep(:4,ileptons(ltag2))
      mll = invmass(pl12)
      call filld('M(l1l2)- VBF CUTS',mll,dsig)

c     Phi(j1j2) 
      phijj=fphi(pj(1:4,itag1),pj(1:4,itag2))
      call filld('Phi(j1j2)- VBF CUTS',phijj,dsig)

c     pt of third jet
      if(itag3.ne.0) call filld('Pt J3- VBF CUTS',kt(itag3),dsig)

c     y of third jet
      if(itag3.ne.0 .and. kt(itag3) > ptj_min ) 
     .     call filld('Y J3- VBF CUTS',rap(itag3),dsig)

c     pt-rel of first jet
      call filld('Ptrel J1- VBF CUTS',ptrel(itag1),dsig)

C     M_j1j2
      mjj = invmass(pj12)
      call filld('M_j1j2(tag) - VBF CUTS',mjj,dsig)

c     y* 
      if(itag3.ne.0 .and. kt(itag3) > ptj_min ) then 
         ystar = rap(itag3)-(rap(itag1)+rap(itag2))/2d0
         call filld('y*- VBF CUTS',ystar,dsig)
      endif
