...
c         ph(0:3) = p87(0:3) + p109(0:3)
      elseif (n_v.eq.2 .or. n_v.eq.1) then
         ph(0:3) = p87(0:3)
cBJ: maybe need to change sign of ph
...
c          if (mod(blha_idsubproc(blha_cursubproc,blha_curproc)/10000,10).eq.0) then
            !NC
c            do i=1,2
c              issymm = mod(blha_idsubproc(blha_cursubproc,blha_curproc),10)
c              if (issymm .eq. 7) then
c                multfact = 1/2d0
c                issymm = 1
c              else if (issymm .eq. 3) then
c                multfact = 1/2d0
c                issymm = 1
c              else if (issymm .eq. 4 .or. issymm .eq. 3-i) then
c                multfact = 1
c                issymm = 0
c              else if (issymm .eq. i) then
c                multfact = 1/2d0
c                issymm = 1
c              else if (issymm .eq. 0) then
c                multfact = 1
c                issymm = 0
c              else
c                call BLHA_error("Wrong quark interference index",__FILE__,__LINE__)
c              endif
c              blha_amp(1)=blha_amp(1)+
c     &          m2snc(mod(blha_idsubproc(blha_cursubproc,blha_curproc)/1000,10),
c     &                mod(blha_idsubproc(blha_cursubproc,blha_curproc)/100, 10),
c     &                mod(blha_idsubproc(blha_cursubproc,blha_curproc)/10,  10),
c     &                i, issymm) *
c     &          multfact
c            enddo
c          else
...
c              multfact = 1
c              issymm = mod(blha_idsubproc(blha_cursubproc,blha_curproc),10)
c              if ( issymm .eq. 5 .and. i .ne. 3 ) cycle
c              if ( issymm .eq. 6 .and. i .ne. 4 ) cycle
c              if ( issymm .eq. 1 .and. i .ne. 1 ) cycle
c              if ( issymm .eq. 2 .and. i .ne. 2 ) cycle
c              if ( issymm .eq. 3 .and. .not. (i .eq. 1 .or. i.eq. 2) ) cycle
c              flav1 = mod(blha_idsubproc(blha_cursubproc,blha_curproc)/1000,10)
c              flav3 = mod(blha_idsubproc(blha_cursubproc,blha_curproc)/100 ,10)
c              flav5 = mod(blha_idsubproc(blha_cursubproc,blha_curproc)/10  ,10)
c              if ( i.eq.1 .and. flav1/4.eq.1 ) multfact = multfact/2d0
c              if ( i.eq.2 .and. flav3/4.eq.1 ) multfact = multfact/2d0
c              if (i.eq.1) then
c                issymm = mod(flav1,4)/2
c              elseif (i.eq.2) then
c                issymm = mod(flav3,4)/2
c              else
c                issymm = 0
c              endif
c              flav1 = mod(flav1,2)
