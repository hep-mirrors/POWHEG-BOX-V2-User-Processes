      SUBROUTINE SQQ_QQGGH_NC_new(pbar,fsign,qbar,gsign,ph,ANS)
...
c make this a subroutine
               call QQ_QQGGH_new(pbar,qbar,ph,nhel(1,ihel),fsign,gsign,T)  
c     print*,'nhel(1,ihel)=',nhel(1,ihel)
c     print*,'nhel(2,ihel)=',nhel(2,ihel)
                do k =1,4
                   do ICOL = 0,6
...
c     now color and spin sum returned
      ...
      subroutine QQ_QQGGH_new(pbar,qbar,ph,NHEL,fsign,gsign,matrix)
C  
C Generated by MadGraph II Version 3.0. Updated 02/19/04                
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
...
c test only:
c      CALL VXXXXX(qbar(0,1),ZERO ,NHEL(5)*gsign(1),gsign(1),W(1,5))
c      CALL VXXXXX(qbar(0,2),ZERO ,NHEL(6)*gsign(2),gsign(2),W(1,6))
...
c      tfac = 1d0
      ...
      DO I = 1, NCOLOR
          ...
          matrix(I) = matrix(I)+ZTEMP*DCONJG(JAMP(I))/DENOM(I)*tfac(I)  
          sum       = sum + (DBLE(JAMP(I))**2 + DIMAG(JAMP(I))**2)*tfac(I)
      ENDDO
      
C      do I=1,NCOLOR
Cc         matrix(I) = matrix(0) * JAMP(I)*DCONJG(JAMP(I)) * tfac(I)/sum
C         matrix(I) = matrix(0) * (DBLE(JAMP(I))**2 + DIMAG(JAMP(I))**2) 
C     $        * tfac(I)!/sum
C         if(abs(sum).gt.0.D0) matrix(I)=matrix(I)/sum
C      enddo
c      print*,'matrix(0)=',matrix(0)
c      print*,'sum mat(i)=',matrix(1) + matrix(2) + matrix(3) + 
c     $     matrix(4) +  matrix(5) + matrix(6) 
c
c     
