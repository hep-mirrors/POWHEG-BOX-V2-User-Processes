      ...
      call bookupeqbins('ynu',dy,-5d0,5d0)
      call bookupeqbins('ylep',dy,-5d0,5d0)

      call bookupeqbins('ptnu',dpt,0d0,500d0)
      call bookupeqbins('ptnuzoom',dptzoom,0d0,100d0)
...
c         print*, ihep,jmohep(1,ihep),idhep(ihep)
         iseminus=idhep(ihep).eq.vdecaytemp
...
c        but (i+1)-jet (and all the others) is too soft
     ...
     $           call filld('Njet-exc'//suffix(icut),i*1d0,dsig)
      ...
      call filld('ynu',  ylp,  dsig)
      call filld('ylep',  ylm,  dsig)

      call filld('ptnu',     ptlp,  dsig)
      call filld('ptnuzoom', ptlp,  dsig)

      call filld('ptlep',     ptlm,   dsig)
      call filld('ptlepzoom', ptlm,   dsig)
