...
!          retval=rad_reg_tot+rad_damp_rem_tot
	  call addupweightsrem(retval) !CH changed: calculate the rad_... values
	                                   !  in seperate routine, sim. to btilde, get sigremnant directly from there
...
c upon the real emission kinematics
         ...
         if(flg_nlotest) then
            call analysis_driver(totreg/suppfact,1)
         endif
! ! !       else
! ! !          rad_reg_tot=0
      endif
...
! ! !          rad_damp_rem_tot=0 !CH: commented out (removed this entry from pwhg_rad.h)
         do alr=1,flst_nalr
...
!!CH: added the following routines:
!!!! takes an array with n entries, if entry is negative: mult. with -1 and save sign in the 2nd array
...
!!! similar to corresponding routine in btilde
	...
	real*8 totrem,etotrem,totabsrem,etotabsrem,totposrem,etotposrem,
     ...
     &             ncall
       ...
       rad_etotreg=errorrem(totreg,etotreg,ncall)
       rad_totabsreg=totabsreg/ncall
       rad_etotabsreg=errorrem(totabsreg,etotabsreg,ncall)
       rad_totposreg=totposreg/ncall
       rad_etotposreg=errorrem(totposreg,etotposreg,ncall)
       rad_totnegreg=totnegreg/ncall
       rad_etotnegreg=errorrem(totnegreg,etotnegreg,ncall)
       rad_totrem=totrem/ncall
       rad_etotrem=errorrem(totrem,etotrem,ncall)
       rad_totabsrem=totabsrem/ncall
       rad_etotabsrem=errorrem(totabsrem,etotabsrem,ncall)
       rad_totposrem=totposrem/ncall
       rad_etotposrem=errorrem(totposrem,etotposrem,ncall)
       rad_totnegrem=totnegrem/ncall
       rad_etotnegrem=errorrem(totnegrem,etotnegrem,ncall)
	end
	
ccccccccccccccccccccccccccccccccccccccccccccccc
cc error-determination (taken from btilde-routines)
	real*8 function errorrem(tot,etot2,n)
	implicit none
	real*8 tot,etot2
	integer n
	errorrem=sqrt((etot2/n-(tot/n)**2)/n)
	end
