ccc colour stripped partial amplitude for M3M6
      real*8 function M3M6()
      implicit none
     ...
     .      +t1)+t2)))/((mgo**2-t1)*(mgo**2-s+s2g-t1)*(s-s1+t2)*(mgo**2+s2g
     .      -t1+t2))
            else
              write(*,*) 'Error in M3M6: deltac1c2=',deltac1c2
            endif
     ...
     .      **2-t1)*(mgo**2-s+s2g-t1)*(s-s1+t2)*(mgo**2+s2g-t1+t2))
            else
              write(*,*) 'Error in M3M6: deltac1c2=',deltac1c2
            endif
      else
        write(*,*) 'Error in amplitude-routine M3M6 for the qg-contributions:' 
        write(*,*) 'gauge = ',gauge
        call exit(-1)
      endif
      end
