ccc colour stripped partial amplitude for M52
     ...
     .      **2+2*s2g+t2))))/(mgo**2-t1)**2
            ...
            write(*,*) "Wrong option for coeffs2g in M52:",coeffs2g
            call exit(-1)
          endif
     ...
     .      *(2*msq1**2+msq2**2+s-s1+t1-t2))-mgo**2*nkq*t2+mgo**2*npg*t2-nk2
     .      *(-(msq1**2*(msq2**2+t1))+t1*(msq2**2+s+t1)+mgo**2*(2*msq1**2
     .      -s1-2*t1+t2))+nk1*(-((msq1**2-t1)*(msq2**2+t1-t2))+mgo**2*(2*msq1
     .      **2+s-s1-2*t1+t2))))/(npg*(mgo**2-t1)**2)
            else
              write(*,*) 'Error in M52: deltac1c2=',deltac1c2
            endif
     ...
     .      0
              ...
              M52 = 
     .      (8*((mgo-msq1)*(mgo+msq1)*npq+(nk1-nk2)*(msq1**2-t1)))/(npg*(mgo
     .      **2-t1)**2)
            else
              write(*,*) 'Error in M52: deltac1c2=',deltac1c2
            endif
     ...
     .      +t1)+(s-s2g+t1)*t2))))/(npg*(mgo**2-t1)**2)
        ...
        write(*,*) 'Error in amplitude-routine M520 for the qg-contributions:' 
        write(*,*) 'gauge = ',gauge
        call exit(-1)
      endif
      end
