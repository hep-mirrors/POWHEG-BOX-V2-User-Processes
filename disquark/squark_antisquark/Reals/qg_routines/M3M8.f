ccc colour stripped partial amplitude for M3M8
      real*8 function M3M8()
      implicit none
     ...
     .      0
              ...
              M3M8 = 
     .      -((3*s*s1*s2g-4*s1**2*s2g-3*s1*s2g**2-5*s**2*t1+9*s*s1*t1-4*s1
     .      **2*t1+9*s*s2g*t1-6*s1*s2g*t1-4*s2g**2*t1-s*t1**2+s1*t1**2+4*s2g
     .      *t1**2+(3*s**2+4*s1*(s2g+t1)+s2g*(3*s2g+t1)-s*(4*s1+6*s2g+t1))
     .      *t2+mgo**4*(4*msq2**2-3*s1-4*t1+3*t2)+4*msq2**4*(-2*s1-s2g+t1
     .      +3*t2)+mgo**2*(-4*msq2**4+3*s*s1-4*s1**2-6*s1*s2g+9*s*t1-6*s1
     .      *t1-8*s2g*t1+4*t1**2+msq2**2*(-5*s+16*s1+8*s2g-11*t2)+(-6*s+4
     .      *s1+6*s2g+t1)*t2)-msq2**2*(3*s**2-4*s2g*(4*s1+s2g)+4*t1**2+s*(
     .      -4*s1+5*s2g+3*t1-3*t2)+(-4*s1+11*s2g+5*t1)*t2+4*t2**2))/(s1*(mgo
     .      **2-t1)*(s-s1+t2)*(mgo**2+s2g-t1+t2)))
            else
              write(*,*) 'Error in M3M8: deltac1c2=',deltac1c2
            endif
     ...
     .      *s2g+t1+2*t2))+nk2*(s*s1*s2g-s1*s2g**2-3*s**2*t1+3*s*s1*t1+3*s
     .      *s2g*t1+2*s1*s2g*t1+s*t1**2-s1*t1**2+(s**2-2*s*s2g+s2g**2-3*s
     .      *t1+4*s1*t1+3*s2g*t1)*t2+4*msq2**4*(s+t2)+mgo**4*(-s1+t2)-msq2
     .      **2*(s**2+s*(3*s2g+5*t1+3*t2)+t2*(5*s2g+3*t1+4*t2))+mgo**2*(-2
     .      *s1*s2g+2*s1*t1+s*(s1+3*t1-2*t2)+2*s2g*t2+3*t1*t2-msq2**2*(3*s
     .      +5*t2)))))/(npg*s1*(mgo**2-t1)*(s-s1+t2)*(mgo**2+s2g-t1+t2))
            else
              write(*,*) 'Error in M3M8: deltac1c2=',deltac1c2
            endif
      else
        write(*,*) 'Error in amplitude-routine M3M8 for the qg-contributions:' 
        write(*,*) 'gauge = ',gauge
        call exit(-1)
      endif
      end
