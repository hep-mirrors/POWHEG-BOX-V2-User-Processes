ccc colour stripped partial amplitude for M2M4
     ...
     .      (-8*mgo**2*(s-s1)*(s1-t2))/(s*(mgo**2-t1)*t2)
              ...
              M2M4 = 
     .      (4*(-(msq1**4*t2)+mgo**4*(-s1+t2)-(msq2**2-s1-t1)*(msq2**2*s+s
     .      *t1-s1*t1-s*t2)+msq1**2*((-s+s1)*t1+(s+s1+t1)*t2+msq2**2*(s-2
     .      *s1+t2))+mgo**2*((msq1**2-s1)*s1+msq2**2*(s+s1-t2)+(s1-t1)*t2
     .      -s*(t1+t2))))/(s*(mgo**2-t1)*t2)
            else
     ...
     .      0
              ...
              M2M4 = 
     .      (4*(msq1**2*s1-s1*(s1+s2g)+msq2**2*(s+s1-t2)+(s1+s2g-t1)*t2+2
     .      *mgo**2*(-s1+t2)-s*(t1+t2)))/(s*(mgo**2-t1)*t2)
            else
              write(*,*) 'Error in M2M4: deltac1c2=',deltac1c2
            endif
     ...
     .      (8*mgo**2*(s-s1)*(s1-t2))/(s*(-mgo**2+t1)*t2)
              ...
              M2M4 = 
     .      (4*(-(msq2**4*s)+msq2**2*s*s1+msq2**2*s*s2g+msq2**2*s1*s2g-s1
     .      **2*s2g-s1*s2g**2+msq2**2*s1*t1+s*s1*t1-s1**2*t1-s*s2g*t1+s*t1
     .      **2-s1*t1**2-msq1**4*t2+(-((s-s2g)*(-msq2**2+s1+s2g))-(s+s2g)
     .      *t1)*t2+mgo**4*(-s1+t2)+mgo**2*(msq1**2*s1-s1*(s1+2*s2g)+msq2
     .      **2*(s+s1-t2)+(s1+2*s2g-t1)*t2-s*(t1+t2))+msq1**2*(-(s*t1)+(s
     .      +t1)*t2+msq2**2*(s-2*s1+t2)+s1*(s2g+t1+t2))))/(s*(mgo**2-t1)*t2)
            else
              write(*,*) 'Error in M2M4: deltac1c2=',deltac1c2
            endif
          else
     ...
     .      *s*t2**2+nk2*(s*(s-s1)*t1+mgo**2*(s-t2)*(s1-t2)+(-(msq1**2*s)
     .      +s**2+(s+s1)*t1)*t2-(msq1**2+s)*t2**2+msq2**2*s*(-s+t2))+nk1*(
     .      -(msq2**2*s*(s+t2))+(s-t2)*(-(s1*t1)+s*(t1-t2)+msq1**2*t2)+mgo
     .      **2*(t2*(-s1+t2)+s*(s1+t2)))+npq*(2*mgo**4*t2+msq2**2*(s**2-s
     .      *t2-2*(mgo**2-2*msq1**2+t1)*t2)+(s+t2)*(s1*t1-msq1**2*t2+s*(-t1
     .      +t2))-mgo**2*(t2*(4*msq1**2-3*s1-2*t1+t2)+s*(s1+3*t2)))))/(npg
     .      *s*(mgo**2-t1)*t2)
            else
              write(*,*) 'Error in M2M4: deltac1c2=',deltac1c2
            endif
     ...
     .      0
              ...
              M2M4 = 
     .      (2*(2*msq1**2*nkq*s-2*msq1**2*npg*s+2*msq2**2*npg*s-4*mgo**2*npg
     .      *s1+2*msq1**2*npg*s1+2*msq2**2*npg*s1-nkq*s*s1+npg*s*s1-2*npg
     .      *s1**2-2*npg*s1*s2g-2*nkq*s*t1+nk2*(s-t2)*(s1-t2)+4*mgo**2*npg
     .      *t2-2*msq1**2*npg*t2-2*msq2**2*npg*t2+nkq*s*t2-3*npg*s*t2-nkq
     .      *s1*t2+3*npg*s1*t2+2*npg*s2g*t2+nkq*t2**2-npg*t2**2+nk1*(t2*(
     .      -s1+t2)+s*(s1+t2))-npq*(t2*(-4*mgo**2+4*msq1**2+2*msq2**2-3*s1
     .      -2*(s2g+t1)+t2)+s*(s1+3*t2))))/(npg*s*(mgo**2-t1)*t2)
            else
              write(*,*) 'Error in M2M4: deltac1c2=',deltac1c2
            endif
          else if(coeffs2g.eq.99) then !return the  complete term, with the resonant propagators left out
     ...
     .      **2+nk2*(s*s1*(mgo**2+s2g)+s*(s-s1)*t1-(msq1**2*s-s**2+mgo**2
     .      *(s+s1)+s*s2g+s1*s2g-(s+s1)*t1)*t2+(mgo**2-msq1**2-s+s2g)*t2*
     .      *2+msq2**2*s*(-s+t2))+nk1*(s*s1*(mgo**2+s2g)+s*(s-s1)*t1+(msq1
     .      **2*s-s**2+mgo**2*(s-s1)+s*s2g-s1*s2g-s*t1+s1*t1)*t2+(mgo**2-msq1
     .      **2+s+s2g)*t2**2-msq2**2*s*(s+t2))+npq*(-(s*(s1*(mgo**2+s2g-t1)
     .      +s*t1))+(2*mgo**4+s**2-3*s*s2g+3*s1*s2g+2*s2g**2-msq1**2*(s+4
     .      *s2g)-s*t1+s1*t1+2*s2g*t1+mgo**2*(-4*msq1**2-3*s+3*s1+4*s2g+2
     .      *t1))*t2-(mgo**2+msq1**2-s+s2g)*t2**2+msq2**2*(s**2-s*t2-2*(mgo
     .      **2-2*msq1**2+s2g+t1)*t2))))/(npg*s*(mgo**2-t1)*t2)
            else
              write(*,*) 'Error in M2M4: deltac1c2=',deltac1c2
            endif
          else
