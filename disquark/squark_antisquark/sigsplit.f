...
c is in fact VERY similar to sigremnant, but with some modifications
	...
	real*8 retval,retval0
      ...
      integer ifirst
      integer j
      real * 8 xjac,totsplit1,totsplit2
	real*8 jaco1,jaco2
	real*8 beta,vec(1:3)
      real*8 powheginput
      logical valid_emitter
      external valid_emitter,powheginput
	sigsplit=1      
...
cc reset the result-arrays here, as we may not explicitly set the corresponding entry
	do j=1,flst_nregularsplit
	  rad_split_arr45(j)=0d0
	  rad_split_arr35(j)=0d0
	enddo

	totsplit1=0d0
	totsplit2=0d0
c first create a momentum-config where the particles 4 and 5 are tan-mapped on a resonant gluino (mind the order of the masses!!!)
...
ccc generate momenta (using the kn_preal already generated in real phase-space)
	if(flg_decay) then
     ...
     .                  kn_cmpreal(0:3,6),kn_cmpreal(0:3,8),jaco2)
      ...
      call sigreal_split(xjac,totsplit1,rad_split_arr45,1)
      call transfersign(rad_split_arr45,rad_split_sign45,flst_nregularsplit)
      if(flg_nlotest) then
        call analysis_driver(totsplit1,1)
      endif

c and now the situation the other way around 
...
ccc generate momenta (using the kn_cmpreal already generated in real phase-space)
	if(flg_decay) then
     ...
     .                  kn_cmpreal(0:3,6),kn_cmpreal(0:3,8),jaco2)
      ...
      xjac=kn_jacborn*ww*hc2
      call sigreal_split(xjac,totsplit2,rad_split_arr35,2)
      call transfersign(rad_split_arr35,rad_split_sign35,flst_nregularsplit)
      if(flg_nlotest) then
        call analysis_driver(totsplit2,1)
      endif

      retval=totsplit1+totsplit2
      end

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!! CH: added the following subroutines
...
c callnr: if we have a process a la sq sqbar qbar: we have a resonance in 35
c        different tan-mapping-> exclude these channesl in first treatment,
c        i.e. for callnr==1; for callnr==2: exclude channels a la sq sqbar q
	subroutine sigreal_split(xjac,sig,r0,callnr)
	implicit none
      include 'nlegborn.h'
      include 'pwhg_flst.h'
      include 'pwhg_kn.h'
	...
	include 'sqantisq_realqg.h'
      ...
      integer lreg,lregpr,iret,callnr
      integer nmomset
      parameter (nmomset=10)
      real * 8 res(nmomset,maxprocreal),preal(0:3,nlegreal,nmomset),
     #   cprop
      integer equivto35(maxprocreal),equivcoef35(maxprocreal)
      integer equivto45(maxprocreal),equivcoef45(maxprocreal)
      integer j
...
c     generate "nmomset" random real-phase space configurations
            ...
            do lreg=1,flst_nregularsplit
		   if(flst_regularsplit(nlegreal,lreg).gt.0) cycle !only need structures with q as 5th particle
               do j=1,nmomset
                 call real_split(flst_regularsplit(1,lreg),preal(0,1,j),res(j,lreg))
               enddo
               call compare_vecs_split(nmomset,lreg,res,lregpr,cprop,iret,callnr) 
               if(iret.eq.0) then
c     they are equal
...
c     generate "nmomset" random real-phase space configurations
            ...
            do lreg=1,flst_nregularsplit
		   if(flst_regularsplit(nlegreal,lreg).lt.0) cycle !simply do nothing in this case
               do j=1,nmomset
                 call real_split(flst_regularsplit(1,lreg),preal(0,1,j),res(j,lreg))
               enddo
               call compare_vecs_split(nmomset,lreg,res,lregpr,cprop,iret,callnr) 
...
c End initialization phase; compute graphs
      ...
      do lreg=1,flst_nregularsplit
c --- test which combination we have
	  if(callnr.eq.1.and.flst_regularsplit(nlegreal,lreg).lt.0) cycle !simply do nothing in this case
	  if(callnr.eq.2.and.flst_regularsplit(nlegreal,lreg).gt.0) cycle !simply do nothing in this case
c ----------------
        if(callnr.eq.1) then
	   if(equivto45(lreg).lt.0) then
          call real_split(flst_regularsplit(1,lreg),kn_cmpreal,r0(lreg))
         else 
...
!!!! pass another flag callnr: if set to 1, we consider resonances in '45', i.e. we 
!!!! use only flst's with the 5th particle being a quark
!!!! same for callnr==2: here the 5th part. needs to be an antiquark
      subroutine compare_vecs_split(nmomset,lreg,res,lregpr,cprop,iret,callnr)
      implicit none
      include 'nlegborn.h'
      include 'pwhg_flst.h'
      real * 8 ep
      parameter (ep=1d-12)
      integer nmomset,lreg,lregpr,iret,j,k,callnr
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!1
      do j=1,lreg-1
!CH: if the res of the amp-routine is 0:
	  if(flst_regularsplit(nlegreal,j).lt.0.and.callnr.eq.1) cycle
	  if(flst_regularsplit(nlegreal,j).gt.0.and.callnr.eq.2) cycle
