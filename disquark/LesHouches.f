...
c   CH: This part is not necessary, as we assign the real color-flow in finalize_lh
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
	   if(.not.flg_ownRCF) then
...
c gluons are numbered 21 in pdg
         if(fl.eq.0) fl=21
	   if(fl.eq.1000000) fl=1000021 !added CH: for gluinos!
         idup(k)=fl
         if(k.gt.2) then
            istup(k)=1
            mothup(1,k)=1
...
c now handle all coloured particles
 ...
 998  continue
	...
	implicit none
      include 'LesHouches.h'
	include 'pwhg_math.h' !added CH, contains pdgfac
...
c CH: change this check to include squarks, antisquarks as well
...
c Btilde event: pass x1 and x2, id1, id2 etc. of the current underlying Born
      ...
      elseif(rad_type.eq.2.or.rad_type.eq.3.or.rad_type.eq.4.or.rad_type.eq.5) then !CH: added rad_type 4
         id1=idup(1)
         id2=idup(2)
         if(id1.eq.21) id1=0
         if(id2.eq.21) id2=0         
