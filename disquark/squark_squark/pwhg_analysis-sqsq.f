...
c separately plot the squarks:
        ...
        call bookupeqbins('m_sq_sq',100d0,2*par_msqmin-mod(2*par_msqmin,100d0),2*par_msqmin-mod(2*par_msqmin,100d0)+100d0*30)
        ...
        call bookupeqbins('eta_sq_sq',0.04d0,-2d0,2d0)
        call bookupeqbins('y_sq_sq',0.04d0,-2d0,2d0)
        call bookupeqbins('pt_sq_sq',10d0,0d0,2000d0)
...
!! number of jets
       call bookupeqbins('n_j',1d0,0.5d0,10.5d0) !in principle only 3 jets possible,...
       endif
       end
      ...
      if(ini) then
	    ...
	    if(flg_decay) then
	      ...
	      ptinvcut=readcuts('ptinv',0)
            ...
            write(*,*) ptjcut(1:3)
            write(*,*) 'requiring nminjets=',nminjets
...
cc add a random azimuthal angel around beam-axis for the histograms
cc similar to add_azimuth for LHE-file
!       azi=random()
!       sazi = dsin(azi)
!       cazi = dcos(azi)
!       do ihep=1, nhep
!          call mrotate(dir,sazi,cazi,phep(1,ihep))
!       enddo
...
ccc sort squarks by pt
	  call sortbypt(nsquarks,psq)
...
cc the real radiation parton is always the last one in the pj-array
...
cccc  get p_t,... of the jet (if one is present)
...
cc the single-jet:         
	   if(ktj(1).ge.ptjmincut.and.abs(etaj(1)).le.etajcut.and.mjets.ge.1) then !keep a minimal pt-value for 1-jets as well!!
	     ...
	     call filld('pt_sq_sq',pt_sqsq,dsigcp) !we need pt_sqsq to compare wiht the pt_j-distribution-> only fill if pt_sqsq>ptj_min!!!
	   endif
         call filld('m_sq_sq',m_sqsq,dsigcp)
         call filld('eta_sq_sq',eta_sqsq,dsigcp)
         call filld('y_sq_sq',y_sqsq,dsigcp)
