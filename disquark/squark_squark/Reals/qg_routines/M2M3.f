ccc colour stripped partial amplitude for M2M3
      ...
      if(gauge.eq.2) then !The Feynman-Gauge
          ...
          M2M3 = 
     .      (4*(-(msq2**2*(2*msq1**2-t1)*(msq1**2-s-t1))+mgo**2*(2*msq1**4
     .      +msq2**2*(2*s+3*t1)-msq1**2*(3*msq2**2+2*s1+3*t1-3*t2)+(s+t1)
     .      *(t1-t2))+mgo**4*(3*msq1**2-2*s1-3*t1+2*t2)))/((mgo**2-t1)*(mgo
     .      **2-s-t1))
            else if(deltac1c2.eq.1) then
              M2M3 = 
     .      (-4*mgo**2*(msq2**2*s-(s+t1)*t2+mgo**2*(s-2*s1+2*t2)+msq1**2*(2
     .      *s-2*s1+3*t2)))/((mgo**2-t1)*(mgo**2-s-t1))
            else
     ...
     .      /((mgo**2-t1)*(mgo**2-s+s2g-t1)*(-mgo**2+s+t1)**2)
            ...
            if(deltac1c2.eq.0) then
          M2M3 = 
     .      (-2*(4*msq1**4*(-msq2**2+s2g)+msq2**2*(s*(s2g-2*t1)+2*(s2g-t1)
     .      *t1)-msq1**2*(2*msq2**2*(-2*s+s2g-3*t1)+s2g*(4*s1-2*s2g+6*t1-5
     .      *t2))+2*mgo**4*(3*msq1**2-2*s1-3*t1+2*t2)-s2g*((s2g-t1)*(s1+2
     .      *t1)-(s2g-2*t1)*t2+s*(-t1+t2))+mgo**2*(4*msq1**4-5*s1*s2g+2*t1
     .      *(s-4*s2g+t1)+msq2**2*(4*s+6*t1)+(-2*s+5*s2g-2*t1)*t2+msq1**2
     .      *(-6*msq2**2-4*s1+8*s2g-6*t1+6*t2))))/((mgo**2-s+s2g-t1)*(-mgo
     .      **2+t1))
            else if(deltac1c2.eq.1) then
              M2M3 = 
     .      (-4*mgo**2*(msq2**2*s+s*s2g-2*s1*s2g-(s-2*s2g+t1)*t2+mgo**2*(s
     .      -2*s1+2*t2)+msq1**2*(2*s-2*s1+3*t2)))/((mgo**2-t1)*(mgo**2-s+s2g
     .      -t1))
            else
              write(*,*) 'Error in M2M3: deltac1c2=',deltac1c2
            endif
     ...
     .      +mgo**2*nkq*t1*t2-nk1*(-3*mgo**2+s+3*t1)*(msq2**2*(-msq1**2+s
     .      +t1)+mgo**2*(msq1**2-s1-t1+t2))+nk2*(mgo**2-s-t1)*(msq2**2*(msq1
     .      **2-t1)-mgo**2*(msq1**2+s-s1-t1+t2))))/(npg*(mgo**2-s-t1)*(-mgo
     .      **2+t1))
            else if(deltac1c2.eq.1) then
              M2M3 = 
     ...
     .      **2-t1)-mgo**2*(msq1**2+s-s1-t1+t2))+nk1*(-(msq2**2*(s**2-2*s
     .      *(s2g-2*t1)+3*t1*(-s2g+t1)))+3*mgo**4*(msq1**2-s1-t1+t2)+msq1
     .      **2*((msq2**2-s2g)*(s-3*s2g+3*t1)+s2g*t2)+s2g*(-((s2g-t1)*(2*s1
     .      +3*t1))+s*(s1+2*t1-2*t2)+(2*s2g-3*t1)*t2)+mgo**2*(s*s1-5*s1*s2g
     .      +s*t1+3*s1*t1-6*s2g*t1+3*t1**2+3*msq2**2*(s+t1)-msq1**2*(3*msq2
     .      **2+s-6*s2g+3*t1)-(s-5*s2g+3*t1)*t2))))/(npg*(mgo**2-s+s2g-t1)
     .      *(-mgo**2+t1))
            else if(deltac1c2.eq.1) then
              M2M3 = 
