ccc colour stripped partial amplitude for M12
     ...
     .      0
            ...
            if(deltac1c2.eq.0) then
          M12 = 
     .      (8*((mgo**2-msq2**2+s2g)*(mgo**2+s2g-t1)+(mgo**2+s2g)*t2))/s
            else if(deltac1c2.eq.1) then
              M12 = 
     .      (-8*mgo**2*t2)/s
            else
            ...
            write(*,*) "Wrong option for coeffs2g in M12:",coeffs2g
            call exit(-1)
          endif
     ...
     .      *(-2*msq1**2+2*s+t1)-mgo**2*(2*msq1**2+msq2**2-2*s1-t1+t2))))
     .      /(npg*s)
            else if(deltac1c2.eq.1) then
              M12 = 
     ...
     .      (8*mgo**2*(-(nkq*s)+npg*(s-s1)+npq*(2*s-2*s1+t2)))/(npg*s)
        ...
        write(*,*) 'Error in amplitude-routine M121 for the qg-contributions:' 
        write(*,*) 'gauge = ',gauge
        call exit(-1)
      endif
      end
