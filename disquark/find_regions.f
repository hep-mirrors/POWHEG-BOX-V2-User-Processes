...
c     debug information
	...
	character* 4 susypartnamesL(min_partnames:max_partnames)!added CH
	character* 4 susypartnamesR(-16:16)!added CH
	character* 4 fspartname !added CH
      data partnames/ 
     &     '   ','W- ','   ','   ','   ','   ','   ','   ','   ',
     $     'vt~','ta+','vm~','mu+','ve~','e+',' ','  ',' ',
     ...
     $     '  ','  ','  ','  ','  ','gam','   ','W+ ','H  '/
!CH: add two array for susy-names:
     ...
     $     'b2 ','t2 ','  ','  ','',' ','er-','','mur-','','ta2-',''/
      ...
      string(next:)=' ==> .'
      next=lastnb(string)
!CH: changed the following: allow for Sparticles in the final state
      do j=3,n
	   if(flav(j).eq.-999) then
	     fspartname='dummy'
	     cycle
	   endif
	   if(flav(j).eq.pdgfac+35) then !fourth neutralino and second chargino: implement as special cases
            fspartname='n4'
...
c cannot come from the same splitting
      ...
      implicit none
	include 'pwhg_math.h' !CH: added
      logical is_charged
      integer fl,flcp
	flcp=mod(fl,pdgfac)! CH: take into account sparticles
      if(fl.eq.0) then
         is_charged=.false.
	...
	include 'pwhg_math.h' !CH: added
      ...
      integer fl
!       if(abs(fl).le.6) then
c CH: add Squarks and Gluinos (set Gluino-PDG to 1000000)
