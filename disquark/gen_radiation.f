      ...
      save mcalls,icalls
      real * 8 pwhg_pt2,pt2max_regular,pt2max_split !CH: added pt2max_split
      external pwhg_pt2,pt2max_regular,pt2max_split
      real * 8 weight
      real*8 ran1 !added CH
      integer i
      real * 8 seconds
...
!CH: change this: regular part now split in several parts
      ran1=random() !added CH

      if(ran1.gt.(rad_totremgen+rad_totreggen+rad_totsplitgen35
     &                +rad_totsplitgen45)/rad_totgen) then
...
! regular or remnant contribution
       elseif(ran1.gt.(rad_totsplitgen35+rad_totsplitgen45)/rad_totgen) then
...
!CH: set the flg_btilde to false for split part
	   flg_btilde=.false.
c generate split n+1 body cross section
         call gen_sigsplit
c pick a configuration according to its cross section
c iret=3: "split" contribution from split-up real with resonance in 35
c iret=4: "split" contribution from split-up real with resonance in 45
         call gen_split(iret)

c if negative weight, flip the sign of xwgtup
	   if(iret.eq.3) then
           if(rad_split_sign35(rad_realsplit).eq.-1) then
            weight=-weight
           endif
         elseif(iret.eq.4) then
           if(rad_split_sign45(rad_realsplit).eq.-1) then
            weight=-weight
           endif
         else
            write(*,*) 'Wrong return-statement of gen_split: iret=',iret
            call exit(-1)         
         endif

         call add_azimuth
	   if(iret.eq.3) then !CH: added
c     set st_muren2 for scalup value for split contributions (alphas is correctly set!!!)
            rad_pt2max=max(rad_ptsqmin,pt2max_split()) !pt2max_split-fct in Real_phsp.f
            call set_rad_scales(rad_pt2max)
            call gen_leshouches_split
c rad_type=4 for split contributions with resonance in 35
            rad_type=4
         else !CH: added
c     set st_muren2 for scalup value for split contributions
            rad_pt2max=max(rad_ptsqmin,pt2max_split())
            call set_rad_scales(rad_pt2max)
            call gen_leshouches_split
c rad_type=5 for split contributions with resonance in 45
            rad_type=5
         endif  
         call increasecnt("split event")       
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!11!!!!!!!!!!!!
      endif
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!11!!!!!!!!!!!!
!CH: add test-function defined in LesHouches.f:
! !          call displeshouches
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!11!!!!!!!!!!!!
