...
ccc CH: added this: if we perform the decays with the expanded formula: have to change btildeborn
ccc NOTE. for NLO in productin only: do not consider this!!!
	if(flg_decay.and.par_NWAapproach.eq.2.and.par_NLOwhich.ne.1) then
	  chir1=abs(flst_born(3,j))/pdgfac !see comments in born: assume here the case sqsq, sq~sq~gives the same results
        chir2=abs(flst_born(4,j))/pdgfac !integer-division
        fla1=abs(flst_born(3,j))-chir1*pdgfac
        fla2=abs(flst_born(4,j))-chir2*pdgfac

	  res(j)=res(j)*(1d0-par_wsqNLO(fla1,chir1)/par_wsq(fla1,chir1)
     .                    -par_wsqNLO(fla2,chir2)/par_wsq(fla2,chir2))
	endif
...
c it prints the set of equivalent Born configurations
     ...
     1           'Beginning sequence of equivalent amplitudes'
      ...
      close(iun)
 100  format(d10.4,5x,i8,5x,100(i8,1x)) !CH: changed this to include sparticles
      end
