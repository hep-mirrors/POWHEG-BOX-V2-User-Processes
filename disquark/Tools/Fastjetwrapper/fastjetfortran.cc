...
void fastjetppgenkt_(const double * p, const int & npart,                   
    ...
    if(first) std::cout<<"###### FASTJET ########"<<std::endl;
    if (palg == 1) {
      jet_def = fj::JetDefinition(fj::kt_algorithm, R);
	if(first) std::cout<<"Using kt_algorithm with R="<<R<<std::endl;
    }  else if (palg == 3) {
      jet_def = fj::JetDefinition(fj::cambridge_algorithm, R);
	if(first) std::cout<<"Using CA_algorithm with R="<<R<<std::endl;
    }  else if (palg == 2) {
      jet_def = fj::JetDefinition(fj::antikt_algorithm, R);
	if(first) std::cout<<"Using antikt_algorithm with R="<<R<<std::endl;
    } else {
      jet_def = fj::JetDefinition(fj::genkt_algorithm, R, palg);
	if(first) std::cout<<"Using generic kt_algorithm with R="<<R<<" and palg="<<palg<<std::endl;
    }
