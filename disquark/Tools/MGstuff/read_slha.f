...
c CH: commented this block out: we don't need the SM-Decays here! (W and Z are not provided by Spheno anyway!)
c     provided by Spheno anyway                    
!       do j1=11,13
!         check_final = check_final * check_sq(j1) 
!       end do
...
c----------------------
      ...
      implicit none
	include 'coupl.inc' !added CH
      integer   nin2,ii,dumi1,dumi2,i,j
      real*8    dumr,m_t(2,2)
      character line1*1
      logical done
...
c----------------------
	...
	include 'PhysPars.h' !added CH
      integer   nin2,ii,jj,dumi1,dumi2
      real*8    dumr,bw(4,4)
      character line1*1
      logical done
...
!CH: copy neutralino mixing matrix!
      do ii=1,4
	  do jj=1,4
	    par_chi0mix(ii,jj)=bw(ii,jj)
	  enddo
	enddo
      return
...
!CH: copy chargino U-mixing matrix!
      do ii=1,2
	  do jj=1,2
	    par_chipmUmix(ii,jj)=uu(ii,jj)
	  enddo
	enddo
      return
...
!CH: copy chargino V-mixing matrix!
      do ii=1,2
	  do jj=1,2
	    par_chipmVmix(ii,jj)=vv(ii,jj)
	  enddo
	enddo
      return
...
c            array defined in Xsugra.f
      if (dumi.eq.1000021) then 
         ...
         twidth = dumr      
         check_sq(11) = 1 
      else if (dumi.eq.23) then !CH: Z
         zwidth = dumr      
         check_sq(12) = 1 
      else if (dumi.eq.24) then !CH: W
         wwidth = dumr      
         check_sq(13) = 1 
      end if 
