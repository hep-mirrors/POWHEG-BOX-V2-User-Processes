...
c
	...
	parameter( Rt2 = dsqrt(2d0)) !CH
!       parameter( Rt2   = 1.414213562d0 ) !CH: get sqrt2 from pwhg_math
      double precision  Fourpi!,Pi
!       parameter( Pi = 3.14159265358979323846d0 ) !commented CH: get pi from pwgh_math.h
...
c changed CH: set G global to 1 and set the correct powers of alphas in the result!
	     G = 1.D0
c comment: this has to be done before the following if-statemnt, as we call setpara for the summing-up of all channels with readlha=.false.
c
...
c
      ...
      double precision  sw, sw2, cw, tw, s2w, c2w, e, gx, gz,
     &                  qe, qu, qd, t3e, t3v, t3u, t3d, root2, pi, !CH changet rt2->root2
     &                  r_l(2,2), r_b(2,2), r_t(2,2)
      common /ewparam/  sw, sw2, cw, tw, s2w, c2w, e, gx, gz,
     &                  qe, qu, qd, t3e, t3v, t3u, t3d, root2, pi,
     &                  r_l, r_b, r_t

c local variables
c
      double precision  zero, two, rt2
      parameter ( zero = 0d0, two = 2d0 )

	parameter (rt2=dsqrt(two)) !need this for the calls for readlha=false
      logical     ldebug
      parameter ( ldebug = .false. )
