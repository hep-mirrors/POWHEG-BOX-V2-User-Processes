c
c adaptation of decay routines from POWHEG-BOX/ttJ
c
      subroutine tth_decay
c
c start from either 
c
c      "parton parton -> h t tbar" or 
c      "parton parton -> h t tbar parton" 
c
c event on the Les Houches interface, 
c add the t-tbar decay products on the Les Houches interface.  
c
c The products of W decays are decided by the subroutine pickwdecays.
c
      implicit none
...
c performs the decay only if a particular channel has been selected
     ...
     $    "POWHEG: top and W on mass shell during decay"
            ...
            write(*,*)
     $    "POWHEG: top and W off-shell effects considered during decay"
         endif
         ini=1
         call setpara ("param_card.dat",.true.) ! set madgraph parameters
      endif

c      print*,'in tth-decay'
c
...
c compute undecayed matrix elements for normalization
      ...
      elseif(nup.eq.5) then   
         ...
         write(*,*)' error: tth_decay, invalid process nup'
         stop
      endif
c check that they are sensible
      if(.not.pwhg_isfinite(res_undec)) then
         write(*,*) "Error in tth_decay : undecayed matrix elements NaN" 
         call exit(1) 
      endif
...
c ttbar 3-momentum in partonic CM
      kpair=0
      do mu=1,3
         kpair=kpair+(pp(mu,4)+pp(mu,5))**2
      enddo
      kpair=sqrt(kpair)
c total final state energy in partonic CM for on-shell tops&Higgs
      cmen0=tten0+pp(4,3)
      if (nup.eq.6) cmen0=cmen0+pp(4,6)
c boost heavy quarks in their rest frame 
      beta(1:3)=-(pp(1:3,4)+pp(1:3,5))/tten0
      call boostm(beta,2,pp(1,4),pp(1,4))
c krtop: relative top momentum (i.e. in the top-antitop CM)
      krtop2=pp(4,4)**2-ph_tmass**2
...
c rescale top energies according to new invariant masses
      pp(4,4)=sqrt(tpvirt2+krtop2)
      pp(4,5)=sqrt(tmvirt2+krtop2)
c boost heavy quarks back to partonic CM
      if(nup.eq.5) then
         beta(1:3)=-pp(1:3,3)/sqrt(kpair**2+(pp(4,4)
     $        +pp(4,5))**2)
      else
         beta(1:3)=-(pp(1:3,3)+pp(1:3,6))/sqrt(kpair**2+(pp(4,4)
     $        +pp(4,5))**2)
      endif
      call boostm(beta,2,pp(1,4),pp(1,4))
c
...
c to store decay products momenta
      ...
      pup(5,4)=sqrt(tpvirt2)
      pup(5,5)=sqrt(tmvirt2)

      if(debug) call momcons(2,pp,nup-2,pp(1,3))
c fill the first two entries of the full phase space array
c ppss has the order (parton ,parton -> h b,l+,vl,bbar,l-,avl,j)
      ppss(1:4,1:3)=pp(1:4,1:3)
...
c add t and tbar decay products to ppss
      ...
      call decaytop(pup(1,4),wpvirt2,ebm,ppss(1,4))
      call decaytop(pup(1,5),wmvirt2,ebp,ppss(1,7))
c check momentum conservation
      if(debug) call momcons(1,pup(1,4),3,ppss(1,4))
      if(debug) call momcons(1,pup(1,5),3,ppss(1,7))
...
c compute decayed matrix elements 
      if(nup.eq.5) then
c         print*,'call born_dec:'
         call born_decayed(ppstd,res_dec)
c         print*,'born_dec=',res_dec
      else
c         print*,'call real_dec: res_undec=',res_undec
         call real_decayed(ppstd,res_dec)  
c         print*,'real_dec=',res_dec   
      endif
c check that they are sensible
...
c evaluate upper bound
      xboundb=1
      xboundb=xboundb/(((tpvirt2-ph_tmass**2)**2+ph_tmass**2
     $     *ph_twidth**2)* ((wpvirt2-ph_wmass**2)**2+ph_wmass**2
     $     *ph_wwidth**2)* ((tmvirt2-ph_tmass**2)**2+ph_tmass**2
     $     *ph_twidth**2)* ((wmvirt2-ph_wmass**2)**2+ph_wmass**2
     $     *ph_wwidth**2))
...
c
     ...
     $           sqrt(tmvirt2),sqrt(wmvirt2)
      ...
      if(rat.gt.boundnorm) then
         call  increasecnt ('ttH_decay upper bound violations')
         if(debug) then
         write(*,*) 'subprocess',(idup(j),j=1,nup)   
...
cccc      ubound=max(rat,ubound)
      if(boundnorm*random().gt.rat) then
         call increasecnt('tth_decay vetoed configurations')
         goto 1
      endif
...
c 2nd W- decay product
      ...
      call set_ebe_couplings_width
      call sborn_proc(p,flav,res,dummy1,dummy2)
...
c do not change: parton_a = 1, parton_b = 2, Higgs = 3
c then come top decay products:
      flav(4)=5   !b
      flav(5)=-11 !e+
      flav(6)=12  !ve
...
c then come top decay products:
      ...
      call set_ebe_couplings_width
c      print*,'call sreal-decay' ! for flav=',flav
      call sreal_decay(p,flav,res) 
...
c(default 10 widths around the pole mass defined in virt2)
      tpvirt2=virt2(ph_tmass,ph_twidth,ran(1))
      tpvirt=sqrt(tpvirt2)
      if((abs(tpvirt-ph_tmass)/ph_twidth).gt.10) stop "error tpvirt"
      tmvirt2=virt2(ph_tmass,ph_twidth,ran(2))
      tmvirt=sqrt(tmvirt2)
      if((abs(tmvirt-ph_tmass)/ph_twidth).gt.10) stop "error tmvirt"
      tmvirt2=virt2(ph_tmass,ph_twidth,ran(2))
...
c equals the sum of n2 momenta in p2
      ...
      do j=1,4
         if(abs(res(j)).gt.1d-7) then
            write(*,*) 'mom. check failed for in-minus-out momenta:'
            ...
            stop " Halting program in ttH decay"
            return
         endif
      enddo
      end
