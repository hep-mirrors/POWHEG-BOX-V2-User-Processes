...
c EW Q contribution
     ...
     1           kn_cmpborn,pborn)
         ...
         if(.not.flg_minlo) then
            ...
            if(flg_withsoftvirt_em)
     1           llew = log(par_csicut**2*s/em_muren2)
            rescfac = 1
         endif
         do jb=1,flst_nborn
c This way, if virtuals are imposed to vanish, also soft terms
c are neglected
            if(virt_arr(jb) .eq. 0d0) then
               resvirt(jb)=0d0
               cycle
            endif
c Strong Q contribution
...
c EW Q contribution
            ...
            if(flg_minlo) then
               call setlocalscales(jb,2,rescfac,-1)
               ll = log(par_csicut**2*s/st_muren2)
               if(flg_withsoftvirt_em)
     1              llew = log(par_csicut**2*s/em_muren2)
c get pdfs
...
c EW photonic contribution
               ...
               if (flg_withsoftvirt_em.and.ch1.ne.0) then
                  Qem=Qem-log(st_mufact2/em_muren2)*( 
     1               (1.5d0+2*log(par_csicut))*ch1**2)
               endif
               if (flg_withsoftvirt_em.and.ch2.ne.0) then
                  Qem=Qem-log(st_mufact2/em_muren2)*( 
...
c WEW
               if (flg_withsoftvirt_em.and.chargeofparticle(fl).ne.0)
     1              then
                  if(kn_masses(leg).eq.0) then
...
c     both particles are charged
                  if (flg_withsoftvirt_em.and.ch_i.ne.0.and.ch_j.ne.0)
     1                 then
c     massless-massless case
                     if (kn_masses(legi).eq.0.and.kn_masses(legj).eq.0)
     1                    then
