...
c.....mauro:randomize leptons/e
      ...
      if (ini) then
         ...
         write (*,*)
c.....mauro:randomize leptons/b
c     idvecbos is local and NOT used
c     vdecaytemp here is used to select the relevant leptons,
c     replaced by funtion selectedlep 
c     id of the charged decay product of the W
c$$$         vdecaytemp=lprup(1)-10000
c$$$         if(vdecaytemp.lt.0) then 
c$$$            vdecay2temp=-vdecaytemp+1 
c$$$            idvecbos=24
c$$$         elseif(vdecaytemp.gt.0) then
c$$$            vdecay2temp=-(vdecaytemp+1)
c$$$            idvecbos=-24
c$$$         else
c$$$            write(*,*) 'Error in decay mode in pwhg_analysis'
c$$$            call exit(1)
c$$$         endif
c$$$         if(abs(vdecaytemp).eq.11.or.abs(vdecaytemp).eq.13
c$$$     $        .or.abs(vdecaytemp).eq.15) then
c$$$            continue
c$$$         else
c$$$            write(*,*) '**************************************'
c$$$            write(*,*) ' Analysis works only for e, mu or tau decays'
c$$$            write(*,*) '                 STOP     '
c$$$            write(*,*) '**************************************'
c$$$            call exit(1)
c$$$         endif
...
c.....mauro:randomize leptons/b
c     if(idhep(ihep).eq.vdecay2temp) then
               if(selectedlep(idhep(ihep),2) ) then                  
c.....mauro:randomize leptons/e
                  p_neutrino(0)=phep(4,ihep)
                  do mu=1,3
                     p_neutrino(mu)=phep(mu,ihep)
                  enddo
c               elseif(idhep(ihep).eq.vdecaytemp) then
c.....mauro:randomize leptons/b
               elseif(selectedlep(idhep(ihep),1) ) then                                    
c.....mauro:randomize leptons/e
...
c     build jets
c     just for fastjet debug purposes use the following lines
c      write(*,*)' remove me! remove me! remove me! remove me! remove me'
c      call buildjets(njets,pjets)
...
c.....mauro:randomize leptons/b
c     use with mod 1 for charced lep, 2 for nu
      function  selectedlep(id,mode)
      implicit none
      logical selectedlep
      integer id,mode
      include 'LesHouches.h'
      ...
      if(ini.eq.0) then
         ini=1
c     gen_emutau is defind in lhefread/write so we need to check here
c     for stage 2
         local_gen_emutau=powheginput("#generate_e_mu_tau")
         if(local_gen_emutau.le.1d0) local_gen_emutau=1d0
         if(local_gen_emutau.ne.1d0.and.local_gen_emutau.ne.2d0
     +        .and.local_gen_emutau.ne.3d0) then
            write(*,*)"wring value for flag generate_e_mu_tau"
            STOP
         endif
         ll=(/100,100,100/)
         nn=(/100,100,100/)
c     lprup is the one of the NLO calculation (the same is written
c     in the event file thought not corresponding to the actual lepton flav)
         sl=1
         if((lprup(1)-10000).lt.0) sl=-1
         sn=-sl

         if(local_gen_emutau.eq.1d0) then
            ll(1)=sl*abs(lprup(1)-10000)
            nn(1)=sn*(abs(lprup(1)-10000) + 1)
c            write(*,*)ll
c            write(*,*)nn
            nl=1
c            stop
         elseif(local_gen_emutau.eq.2d0) then
            ll(1)=sl*11
            ll(2)=sl*13
            nn(1)=sn*12
            nn(2)=sn*14
            nl=2
         elseif(local_gen_emutau.eq.3d0) then
            ll(1)=sl*11
            ll(2)=sl*13
            ll(3)=sl*15
            nn(1)=sn*12
      ...
      elseif(mode.eq.2) then
         ...
         write(*,*)"mode not allowed in selectedlep"
         stop
      endif
      end function selectedlep
c.....mauro:randomize leptons/e
