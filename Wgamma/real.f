*
** function to calculate real of u d~ --> e+ nu_e ga
** masses = 0
** agc xf,yf
*
      ...
      real * 8 pp(0:3,nlegreal),ps(0:3,nlegreal),ptmp(0:3)
      integer rflav(nlegreal),rflavp(nlegreal),rflavs(nlegreal)
      integer i,j,k,tmp,mu
      real * 8 amp2,const
      real * 8 powheginput
...
c     powheg-nc/c-lo
      ...
      integer cmode,calr
      common/crealmode/cmode,calr

c cmode = 0: btilde, 1: remnant, 2: regular, 3 radiation, 4 smartsig
      if(powheg_c_lo) then
...
ccc qcd real corrections to Wj cccccccccccccccccccccc
      if((rflav(5).ne.22).and.(rflav(6).ne.22)) then
         pp(:,3) = p(:,4)
         pp(:,4) = p(:,3)
         rflavp(3) = rflav(4)
         rflavp(4) = rflav(3)
c if present, the gluon is assumed to be the last particle
         call real_ampsq_g_last(pp,rflavp,amp2)
...
c q g -> n e qp p         
         ...
         call compreal(pp,rflavp,amp2)
c 3/8 is for different colour's mean between quark
c and gluon induced processes
         if((rflav(1).eq.0).or.(rflav(2).eq.0)) then
            amp2=amp2*(3d0/8d0) 
         endif
...
*
* real W gamma j
      subroutine compreal(p,rflav,reale)
      implicit none
      include 'nlegborn.h'
      include 'pwhg_math.h'
      include 'mathx.h'
      include 'pwhg_st.h'
      include 'PhysPars.h'
      real * 8 p(0:3,nlegreal),ptmp(4),pmcfm(nlegreal,4)
      integer rflav(nlegreal),i,j
      real * 8 reale
*
      integer idvecbos,vdecaymode
      common/cvecbos/idvecbos,vdecaymode
...
c define momenta for MCFM routines
      ...
      call qqb_wgam_g(pmcfm,rflav,reale)
...
c     q aqp -> W+ g g
            call q_aqp_to_al_vl_g_g(p,ferm_type,amp2)
            amp2=amp2*CKM_sq(i,j)
         elseif ((i.eq.0).and.(l.eq.0)) then
c     g q -> W+ qp g
            call g_q_to_al_vl_qp_g(p,ferm_type,amp2)
            amp2=amp2*CKM_sq(j,k)
         elseif ((j.eq.0).and.(l.eq.0)) then
c     q g -> W+ qp g
            call q_g_to_al_vl_qp_g(p,ferm_type,amp2)
            amp2=amp2*CKM_sq(i,k)
         elseif ((i.eq.0).and.(j.eq.0)) then
c     g g -> W+ q aqp
            call g_g_to_al_vl_q_aqp(p,ferm_type,amp2)
            amp2=amp2*CKM_sq(k,l)
         elseif(i*j*k*l.ne.0) then
c     4 quarks
            call find_diff_flavours(abs(i),abs(j),abs(k),abs(l),idx1
     $           ,idx2)
            if(idx1.ne.0) then
               if(idx2.ne.0) then
c     2 equal flavours, 2 different
                  call cross_q_qp_to_al_vl_q_qpp(p,realflav,idx1,idx2
     $                 ,amp2)
               else
c     3 equal flavours, 1 different
                  call cross_q_qp_to_al_vl_q_q(p,realflav,idx1,amp2)
               endif
            else
               print*, 'error'
               stop
      ...
      elseif(idvecbos.eq.-24) then
         if ((k.eq.0).and.(l.eq.0)) then
c     q aqp -> W- g g
            call q_aqp_to_l_avl_g_g(p,ferm_type,amp2)
            amp2=amp2*CKM_sq(i,j)
         elseif ((i.eq.0).and.(l.eq.0)) then
c     g q -> W- qp g
            call g_q_to_l_avl_qp_g(p,ferm_type,amp2)
            amp2=amp2*CKM_sq(j,k)
         elseif ((j.eq.0).and.(l.eq.0)) then
c     q g -> W- qp g
            call q_g_to_l_avl_qp_g(p,ferm_type,amp2)
            amp2=amp2*CKM_sq(i,k)
         elseif ((i.eq.0).and.(j.eq.0)) then
c     g g -> W- q aqp
            call g_g_to_l_avl_q_aqp(p,ferm_type,amp2)
            amp2=amp2*CKM_sq(k,l)
         elseif(i*j*k*l.ne.0) then
c     4 quarks
            call find_diff_flavours(abs(i),abs(j),abs(k),abs(l),idx1
     $           ,idx2)
            if(idx1.ne.0) then
               if(idx2.ne.0) then
c     2 equal flavours, 2 different
                  call cross_q_qp_to_l_avl_q_qpp(p,realflav,idx1,idx2
     $                 ,amp2)
               else
c     3 equal flavours, 1 different
                  call cross_q_qp_to_l_avl_q_q(p,realflav,idx1,amp2)
               endif
            else
               print*, 'error'
               stop
      ...
      real *8 CKM_sq
      if(mod(abs(i),2).eq.0) then
         CKM_sq=ph_CKM(abs(i)/2,(abs(j)+1)/2)**2
      elseif(mod(abs(i),2).eq.1) then   
         CKM_sq=ph_CKM(abs(j)/2,(abs(i)+1)/2)**2
      endif
      end
...
c     different flavours    
      subroutine find_diff_flavours(i,j,k,l,idx1,idx2)
      implicit none
      integer i,j,k,l
      integer idx1,idx2
      idx1=0
...
c     ferm_type = -1 antifermion
      ...
      subroutine q_aqp_to_al_vl_g_g(pphy,fermion_type,amp2)      
      implicit none
         ...
         do  pol6=1,2         
            if (p(0,5).lt.0d0) then
               do mu=0,3
                  pp5(mu) = -p(mu,5)
               enddo         
               call POLVEC(pp5(0),pol5,epsg5(0,pol5))
            else   
            ...
            if (p(0,6).lt.0d0) then
               do mu=0,3
                  pp6(mu) = -p(mu,6)
               enddo         
               call POLVEC(pp6(0),pol6,epsg6(0,pol6))
            else
               call polvec(p(0,6),pol6,epsg6(0,pol6))
            endif
         
c     first colour order: 5 before 6             
            call bra_slash_ket_g1_g2(psi2,psi1,-1,p2,p1,
...
c     third configuration (triple gluon vertex)             
            call bra_gamma_ket_3gv(psi2,psi1,-1,p2,p1,p(0,5),
     #           epsg5(0,pol5),p(0,6),epsg6(0,pol6),jqua)
            
            amp_3gv=ccdotp(jlep,jqua)
...
c     q_aqp_to_al_vl_g_g
      subroutine q_g_to_al_vl_qp_g(pphy,fermion_type,amp2)      
      implicit none
      integer nleg
      parameter (nleg=6)
      integer fermion_type(nleg)
...
c     the correct call of
      ...
      call q_aqp_to_al_vl_g_g(pp,ferm_type,amp2)
...
c     amplitude. This amplitude is obtained by crossing the formula for 
c     q_aqp_to_al_vl_g_g
      subroutine g_q_to_al_vl_qp_g(pphy,fermion_type,amp2
     $     )      
      implicit none
      integer nleg
      parameter (nleg=6)
...
c     amplitude. This amplitude is obtained by crossing the formula for 
c     q_aqp_to_al_vl_g_g
      subroutine g_g_to_al_vl_q_aqp(pphy,fermion_type,amp2)      
      implicit none
      integer nleg
      parameter (nleg=6)
      integer fermion_type(nleg)
...
c     ferm_type = 1 fermion
c     ferm_type = -1 antifermion
      subroutine q_qp_to_al_vl_q_q(pphy,fermion_type,amp2)   
      implicit none
c     the 5 4-momentum vectors
c     p(i,1) is the i-th component of vector p1...   
      integer nleg
...
c     vl
      call bra(px1(0,4),ferm_type(4),psi4)
c     al
      call ket(px1(0,3),ferm_type(3),psi3)
c     q
...
c     currents
      ...
      do hel_qua=-1,1,2       
         call bra_gamma_ket_curr(psi6,psi2,-1,p6,p2,pcurr15,
     #        jqua15(0,hel_qua),jtemp)
         
         amp_ljj(1,hel_qua) = ccdotp(jlep,jtemp)
...
c     vl
      call bra(px2(0,4),ferm_type(4),psi4)
c     al
      call ket(px2(0,3),ferm_type(3),psi3)
c     q
...
c     correnti
      ...
      do hel_qua=-1,1,2      
         call bra_gamma_ket_curr(psi5,psi2,-1,p5,p2,pcurr16,
     #        jqua16(0,hel_qua),jtemp)
         
         amp_ljj(2,hel_qua) = ccdotp(jlep,jtemp)
      enddo
...
c     ferm_type = 1 fermion
c     ferm_type = -1 antifermion
      subroutine q_qp_to_al_vl_q_qpp(pphy,fermion_type,amp2)   
      implicit none
c     the 5 4-momentum vectors
c     p(i,1) is the i-th component of vector p1...   
      integer nleg
...
c     currents
     ...
     #        jqua15(0,hel_qua),jtemp)
         ...
         amp_ljj(hel_qua) = ccdotp(jlep,jtemp)
      enddo
...
c     idx is the position of qp in the flavour string
      subroutine cross_q_qp_to_al_vl_q_q(pphy,rflav,idx,amp2)
      implicit none
      integer nleg
      parameter (nleg=6)
      integer rflav(nleg)
...
c         call exchange_mom(pp,1,2,6,pp)
c         call swap(ferm_type,1,2,6,ferm_type)
...
C Cross again to obtain either only quarks,
...
c           cross 1-6
            call cross(pp,ferm_type,1,6,6,pp,ferm_type)
         else
...
c     call to fundamental routine
      call q_qp_to_al_vl_q_q(pp,ferm_type,amp2)

c     add CKM
      i=1
      if (idx.eq.1) i=2
      amp2=amp2*CKM_sq(flav(idx),flav(i))
...
c     flavour string
      subroutine cross_q_qp_to_al_vl_q_qpp(pphy,rflav,idx1,idx2,amp2)
      implicit none
      integer nleg
      parameter (nleg=6)
      integer rflav(nleg)
...
c     swap momenta and ferm_type in order to have
...
c     crossing not needed, just relabelling
         call exchange_mom(pp,5,6,6,pp)
         call swap(ferm_type,5,6,6,ferm_type)
      elseif((idx1.eq.1).and.(idx2.eq.6)) then
c     crossing not needed, just relabelling
         call exchange_mom(pp,1,2,6,pp)
         call swap(ferm_type,1,2,6,ferm_type)
...
c     'real crossing': cross 1<->6, changing signs.
...
c     call to fundamental routine
      call q_qp_to_al_vl_q_qpp(pp,ferm_type,amp2)

c     add CKM
      amp2=amp2*CKM_sq(flav(idx1),flav(idx2))
      
      end

      subroutine cross(p,array,i,j,dim,pnew,arraynew)
      implicit none
      ...
      subroutine swap(array,i,j,dim,arraynew)
      implicit none
      integer array(*),arraynew(*)
...
ccccccccccccccccccccccccccccccccccccccccccc
      subroutine q_aqp_to_l_avl_g_g(pphy,fermion_type,amp2)
...
c p(i,1) is the i-th component of vector p1...   
      ...
      call q_aqp_to_al_vl_g_g(pphy,ferm_type,amp2)

      end

ccccccccccccccccccccccccccccccccccccccccccccc

      subroutine q_g_to_l_avl_qp_g(pphy,fermion_type,amp2)
...
c p(i,1) is the i-th component of vector p1...   
      ...
      call q_g_to_al_vl_qp_g(pphy,ferm_type,amp2)

      end

cccccccccccccccccccccccccccccccccccccc

      subroutine g_q_to_l_avl_qp_g(pphy,fermion_type,amp2)
...
c p(i,1) is the i-th component of vector p1...   
      ...
      call g_q_to_al_vl_qp_g(pphy,ferm_type,amp2)

      end
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccc


      subroutine g_g_to_l_avl_q_aqp(pphy,fermion_type,amp2)
...
c p(i,1) is the i-th component of vector p1...   
      ...
      call g_g_to_al_vl_q_aqp(pphy,ferm_type, amp2)

      end
cccccccccccccccccccccccccccccccccccccccccccccccccc


      subroutine cross_q_qp_to_l_avl_q_qpp(pphy,rflav,idx1,idx2,amp2)
...
c p(i,1) is the i-th component of vector p1...   
      ...
      call cross_q_qp_to_al_vl_q_qpp(pp,flav,idx1,idx2,amp2)
