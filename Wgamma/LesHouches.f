...
c and conjugate its colour in output, to make it incoming.
            call setcolour_rad(coluborn,conj_flavour(flem),
     1           flrad,icolup(1,em),icolup(1,rad))
...
c conjugate their colours in the output, to make them outgoing.
            call setcolour_rad(coluborn,conj_flavour(flem),
     1           conj_flavour(flrad),icolup(1,em),icolup(1,rad))
...
c      write(*,*)'fin_lh',idup(1:nup)
      ...
      implicit none
      integer conj_flavour,ifl
      if( (abs(ifl).ge.1.and.abs(ifl).le.6) .or.
     1    (abs(ifl).ge.11.and.abs(ifl).le.16) .or.
     2     abs(ifl).eq.24) then
         conj_flavour = -ifl
      elseif(ifl.eq.0.or.ifl.eq.21.or.ifl.eq.22.or.ifl.eq.23) then
         conj_flavour = ifl
      else         
         write(*,*)' conj_flavour: cannot find conjugate of ',ifl
         call pwhg_exit(-1)
      endif
      end         
