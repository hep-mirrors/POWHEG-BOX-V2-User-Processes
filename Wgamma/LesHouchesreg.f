...
c     Don't forget to set scale for scalup equal to the pt of th c     radiation (whatever it is now!)
      scalup=sqrt(rad_pt2max)
c     set color connections for all particles
      call regularcolor_lh
c id of the event
      idprup=lprup(1)
      xwgtup=+1
...
c
      ...
      call colourjoin(icolup(1,iq),icolup(1,iqb),
     1           icolup(1,iqp),icolup(1,iqbpp))
      end


      subroutine colourjoin(icol1,icol2,icol3,icol4)
