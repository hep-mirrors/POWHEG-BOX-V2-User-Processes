      subroutine setvirtual(p,vflav,amp2) 
c Virtual needs to be provided by the user and put here
      ...
      real * 8 pp(0:3,nlegborn),ptmp(0:3)
      integer vflav(nlegborn)
      integer vflavp(nlegborn)
      integer i,j,k,tmp
      real * 8 amp2,amp2p,tree
      real * 8 pmcfm(nlegreal,4)
c     powheg-nc/c-lo
...
c define momenta for MCFM routines
      ...
      if(vflav(5).eq.22)  then
c     virtual Wgam
         call qqb_wgam_v(pmcfm,vflav,amp2) ! MCFM routines (virtual)
         call qqb_wgam(pmcfm,vflav,tree)   ! MCFM routines (Born)
c     dimensional reduction MCFM
         amp2 = amp2 -  4d0/3d0 *  tree
      else
c     virtual Wj
         if(.not.powheg_c_lo.and..not.powheg_nc) then
            call compvirt_Wj(p,vflav,amp2)
         else
            amp2=0d0
         endif
      endif
...
**********************************************************
*******
* The following routines compute the QCD virtual 
* corrections to Wj using MCFM routines
******
**********************************************************
...
c     Use MCFM subroutines
      subroutine compvirt_Wj(p,vflav,virtual)
      implicit none
      include 'nlegborn.h'
      include 'pwhg_flst.h'
      include 'pwhg_st.h'
...
c      if (j.eq.0) j=abs(k)
      ...
      virtual=virt(vflav(1),vflav(2))

c      if(mod(abs(i),2).eq.0) then
c         virtual=virt(vflav(1),vflav(2))*ph_CKM(abs(i)/2,(abs(j)+1)/2)
c     $        **2
c      elseif(mod(abs(i),2).eq.1) then   
c         virtual=virt(vflav(1),vflav(2))*ph_CKM(abs(j)/2,(abs(i)+1)/2)
c     $        **2
c      endif
...
c     THIRD  momentum must be the lepton in MCFM for W-
         pmcfm(3,4)=cmpborn(0,4)
         do mu=1,3
            pmcfm(3,mu)=cmpborn(mu,4)
         enddo
c     FOURTH momentum must be the antineutrino in MCFM for W-
...
c      write(*,*) 'A51 e A52',A51(j2,j5,j1,j4,j3,zb,za),
c     #      A52(j2,j1,j5,j4,j3,zb,za)
...
c      subroutine spinoru(N,p,za,zb)
...
c      include 'MCFM_include/zprods_decl.f'
c      include 'MCFM_include/sprods_com.f'
c      double precision p(mxpart,4),rt(mxpart)
c      double complex c23(mxpart),f(mxpart)
c      integer i,j,N
...
c         za(i,j)=f(i)*f(j)
c     &   *(c23(i)*dcmplx(rt(j)/rt(i))-c23(j)*dcmplx(rt(i)/rt(j)))

c         if (abs(s(i,j)).lt.1d-9) then
c         zb(i,j)=-(f(i)*f(j))**2*dconjg(za(i,j))
c         else
c         zb(i,j)=-dcmplx(s(i,j))/za(i,j)
c         endif
c         za(j,i)=-za(i,j)
c         zb(j,i)=-zb(i,j)
c         s(j,i)=s(i,j)
...
c      implicit none
c      include 'MCFM_include/constants.f'
c      double complex Lnrat
c      double precision x,y,denom
c      denom=one-x/y
c      if (abs(denom) .lt. 1d-7) then
c      L0=-cone-dcmplx(denom*(half+denom/3d0))
c      else
c      L0=Lnrat(x,y)/dcmplx(denom)
c      endif
c      return
c      end
...
c      denom=one-x/y
c      if (abs(denom) .lt. 1d-7) then
c      L1=-half*cone-dcmplx(denom/3d0*(one+0.75d0*denom))
c      else
c      L1=(L0(x,y)+cone)/dcmplx(denom)
c      endif
c      return
c      end
...
c      implicit none
c      include 'MCFM_include/constants.f'
c      double precision x1,x2,y1,y2,r1,r2,omr1,omr2,ddilog
c      double complex dilog1,dilog2,Lnrat
c      r1=x1/y1
c      r2=x2/y2
c      omr1=one-r1
c      omr2=one-r2
c      if (omr1 .gt. one) then 
c         dilog1=dcmplx(pisqo6-ddilog(r1))-Lnrat(x1,y1)*dcmplx(log(omr1))
c      else
c         dilog1=dcmplx(ddilog(omr1))
c      endif
c      if (omr2 .gt. one) then 
c         dilog2=dcmplx(pisqo6-ddilog(r2))-Lnrat(x2,y2)*dcmplx(log(omr2))
c      else
c          dilog2=dcmplx(ddilog(omr2))
c      endif
c      lsm1=dilog1+dilog2+Lnrat(x1,y1)*Lnrat(x2,y2)-dcmplx(pisqo6)
c      return
c      end
...
C--- define Heaviside theta function (=1 for x>0) and (0 for x < 0)
c      htheta(x)=half+half*sign(one,x)
