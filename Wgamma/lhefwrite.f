...
c      write(nlf,'(a)') '<init>'
     ...
     1     pdfgup(1),pdfgup(2),pdfsup(1),pdfsup(2),idwtup,nprup
      ...
      do 100 ipr=1,nprup
c.....mauro:randomize leptons/b
c$$$         write(buffer,120) xsecup(ipr),xerrup(ipr),xmaxup(ipr),
c$$$     &        lprup(ipr)
c..... adjust xsect and error if 3 lepton flavours are selected         
         write(buffer,120) gen_emutau*xsecup(ipr),
     &        gen_emutau*xerrup(ipr),xmaxup(ipr),
     &        lprup(ipr)
         if(gen_emutau.ne.1d0) then
            if(  (lprup(ipr).ne.(10000+13)).and.
     &           (lprup(ipr).ne.(10000-13))  )then
               write(*,*)"in order to use generate_e_mu_tau"
               write(*,*)"only vdecaymode +/-13 can be used"
               STOP
            endif
         endif
c.....mauro:randomize leptons/e         
...
c     write(nlf,'(a)')'<event>'
c.....mauro:randomize leptons/b      
c      write(buffer,210) nup,idprup,xwgtup,scalup,aqedup,aqcdup
      write(buffer,210)nup,idprup,gen_emutau*xwgtup,scalup,aqedup,aqcdup
c.....mauro:randomize leptons/e      
      call  pwhg_io_write(nlf,trim(buffer))
      do 200 i=1,nup
