     ...
     # rrrp,rrrps,rrrm,rrrms,remnant,out0,out1
      logical nonzreal(maxalr)
      common/cnonzreal/nonzreal
      integer j,iuborn
...
c     in final state radiation csimax is independent of y
     ...
     #                 *jac_over_csi/(1-kn_y)/kn_csitilde
                  if(flg_withsubtr.and.nonzreal(j)) then
...
c If one is forcing some of the real to be zero, also the counterterms
...
c            if(equivto(alr).lt.0.or..not.computed(equivto(alr))) then
            ...
            if(condition) then
               ...
               flst_cur_alr = alr
               call realgr(3,alr,flst_alr(1,alr),kn_cmpreal,r0(alr))
               sumdijinv=0
...
c First mark as being computed
               ...
               flst_cur_alr = alr
               call realgr(imode,alr,flst_alr(1,alr),kn_preal,r0(alr))
               nonzreal(alr) = r0(alr) .ne. 0d0
...
c supply Born zero damping factor, if required
     ...
     1                 dampfac)
               ...
               r0(alr)=r0(equivto(alr))*equivcoef(alr)
               nonzreal(alr)=nonzreal(equivto(alr))
               markused(alr)=1
            endif
         endif
      enddo
      ...
      do alr=1,flst_nalr
         if(flg_minlo) then
            flg_minlo_real=.true.
            call setlocalscales(flst_alr2born(alr),2,rescfac,alr)
...
c are not considered.
 ...
 10      continue
      ...
      subroutine realgr(mode,alr,rflav,p,res)
      implicit none
      include 'nlegborn.h'
      include 'pwhg_flst.h'
