...
c (if the flag argument 2 is given)
            if(flg_minlo) call setlocalscales(jb,2,rescfac,-1)
...
c if Wj bornonly
         if(powheg_nc.or.powheg_c_lo) then
            if(flst_born(5,jb).ne.22) then
               rescoll(jb)=0d0
               cycle
            endif
         endif
...
c qg remnant
     ...
     #                             - ppqg(z1))/z1 * pdfs1sng*xjac1 
         elseif(flg_withcollremn_em.and.fl1.eq.22) then
c qgamma remnant
...
c qgamma DIS subtraction 
            if (flg_withcollremn_em.and.pdf_dis_photon) then
                resgam1 = resgam1  - 3d0*
...
c qg remnant
     ...
     #                             - ppqg(z2))/z2 * pdfs2sng*xjac2 
         elseif(flg_withcollremn_em.and.fl2.eq.22) then
c qgamma remnant
