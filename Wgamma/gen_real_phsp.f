...
c     variables for smooth isolation cut in case of nlo test
      logical smoothiso
      real*8 e_h,R_0,delta_R,delta_eta,delta_phi,eta_gamma,eta_part
      real*8 p_part(0:3),p_gamma(0:3),pt_part,pt_gamma
c
      rad_kinreg=1
...
c to remove the collinear photon FSR from quark
      if(nlo_test) then 
         ...
         pt_part = sqrt(p_part(1)**2 + p_part(2)**2)
         call get_pseudorap(p_gamma,eta_gamma)
         call get_pseudorap(p_part,eta_part)
         delta_eta = eta_gamma - eta_part
         delta_phi = dabs(atan2(p_gamma(2),p_gamma(1)) - 
     $        atan2(p_part(2),p_part(1)))
         delta_phi = min(delta_phi,2d0*pi-delta_phi)
         delta_R = sqrt(delta_eta**2+delta_phi**2)
         if(delta_R.lt.R_0) then
            smoothiso = pt_part.lt.(e_h*pt_gamma*(1d0-cos(delta_R))
     $           /(1d0-cos(R_0)))
         else  
            smoothiso = .true.
         endif
         if (.not.smoothiso) then
            jac_over_csi = 0d0
            jac_over_csi_p = 0d0
            jac_over_csi_m = 0d0
            jac_over_csi_s = 0d0
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
         if(kn_emitter.gt.2.and.kn_emitter.lt.6) then
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
            ires = flst_bornres(kn_emitter,1)
            if(ires.gt.0) then
