...
c     ZZ -> H
     ...
     #           (ferm_charge(2)-ferm_charge(4).eq.0))) then
      ...
      call q1_q2_to_q3_q4_H(pin,ferm_type,ferm_charge,amp2)     
      born=amp2
      do mu=0,3
         do nu=0,3
            bmunu(mu,nu)=0d0
...
c
      subroutine q1_q2_to_q3_q4_H_virt(pphy,ferm_type,
     #     fermion_charge,ren_scale, interf)
      ...
      call q1_q2_to_q3_q4_H(pphy,ferm_type,fermion_charge,
     #     ampborn2)

c     squared momentum of the weak boson connected with the upper line
      q2_up = -2*dotp(pphy(0,1),pphy(0,3))
c     squared momentum of the weak boson connected with the lower line
      q2_down = -2*dotp(pphy(0,2),pphy(0,4))
      
c upper-line correction 
      r = ren_scale**2/(-q2_up)
      if (r.lt.0d0) then
         write(*,*) 
     #        'Error in q1_q2_to_q3_q4_H_virt: q2_up should be < 0!!'
      endif      
      lr = log(r)
...
c lower-line correction 
      ...
      if (r.lt.0d0) then
         write(*,*) 
     #        'Error in q1_q2_to_q3_q4_H_virt: q2_down should be < 0!!'
      endif      
      lr = log(r)
