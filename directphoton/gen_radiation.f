...
c Use highest bid procedure (see appendix B of FNO2006)
      tmax=0
      kinreg=0
c if Sudakov for QED radiation should be reweighted
      if (flg_sudakov_rwgt.AND.flst_qcdqedrad(rad_ubornidx,2)) then
         flg_sudarw_tmp = .TRUE.
      else
         flg_sudarw_tmp = .FALSE.
      endif
      sudawgt = 1.d0
...
c     deviding log(r) by sudarwgtfac has same effect as multiplying 
c     log(Delta(ptold)/Delta(pt)) with sudarwgtfac, i.e. enhancing
c     "splitting kernel"
      if (flg_sudarw_tmp) then
         xlr=xlr+log(random())/sudarwgtfac
      else
         xlr=xlr+log(random())
      endif
...
c that some pdf vanish (typically heavy flavour pdf's)
     ...
     # 'upper bound failures in generation of radiation')
      ...
      if(rvalue.gt.value) then
         if (flg_sudarw_tmp) then ! emission vetoed -> update weight
            sudawgt = sudawgt * (1-value/ubound*1/sudarwgtfac)
     &           /(1-value/ubound)
         endif
         call increasecnt('vetoed radiation')
         goto 1
      endif
      if (flg_sudarw_tmp) then ! emission accepted -> update weight
c     Lonnblad's algorithm
c$$$         if (random().gt.0.5d0) then ! additional veto to correct Sudakov
c$$$            sudawgt = sudawgt * (2.d0 - 2.d0/sudarwgtfac)
c$$$            goto 1
c$$$         endif
