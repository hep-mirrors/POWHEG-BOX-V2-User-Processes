...
void fastjetd0runiicone_(const double * p, const int & npart,                   
    ...
    fj::Selector select_rapidity = fj::SelectorAbsRapMax(ymax);
    fj::Selector select_pt       = fj::SelectorPtMin(ptmin);
    fj::Selector select_both     = select_pt && select_rapidity;

//    cout << "Number of jets before selection: " << jets.size() << endl;

    jets = select_both(jets);

//    cout << "Number of jets after selection: " << jets.size() << endl;
