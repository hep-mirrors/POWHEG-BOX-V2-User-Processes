      SUBROUTINE SB_SF_173_014(P1,ANS)
C  
...
C  
C FOR PROCESS : b~ c~ -> c~ b~ g g  
C  
C BORN AMPLITUDE IS b~ c~ -> c~ b~ g  
      IMPLICIT NONE
C  
C CONSTANTS
C  
...
C  
      ...
      common/to_amps_173/  amp2,       jamp2

      integer j,jj
      integer max_bhel
      parameter ( max_bhel =          32 )
      double complex saveamp(ngraphs,max_bhel)
      common/to_saveamp_173/saveamp
      double precision savemom(nexternal-1,2)
      common/to_savemom_173/savemom
...
C          write (*,*) "momenta not the same in Born"
      ...
      ANS(IPROC) = 0D0
          DO IHEL=1,NCOMB
             IF (GOODHEL(IHEL,IPROC) .OR. NTRY .LT. 2) THEN
                 T=B_SF_173_014(P1,NHEL(1,IHEL),IHEL,JC(1))              
      ...
      REAL*8 FUNCTION B_SF_173_014(P,NHEL,HELL,IC)
C  
C Generated by MadGraph II                                              
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
...
C  
      DATA Denom(1  )/           18/                                       
      DATA (CF(i,1  ),i=1  ,4  ) /     7,    6,    6,    7/                
C               T[ 2, 4]T[ 1, 3, 5]                                        
      DATA Denom(2  )/           18/                                       
      DATA (CF(i,2  ),i=1  ,4  ) /     6,  -56,    7,  -57/                
C               T[ 1, 3]T[ 2, 4, 5]                                        
      DATA Denom(3  )/           18/                                       
      DATA (CF(i,3  ),i=1  ,4  ) /     6,    7,    7,    6/                
C               T[ 2, 4]T[ 1, 3, 5]                                        
      DATA Denom(4  )/           18/                                       
      DATA (CF(i,4  ),i=1  ,4  ) /     7,  -57,    6,  -56/                
C               T[ 2, 4, 5]T[ 1, 3]                                        
C ----------
C BEGIN CODE
C ----------
      ...
      DO I = 1, NCOLOR
          ...
          ENDDO
          B_SF_173_014 =B_SF_173_014+ZTEMP*DCONJG(JAMP(I))/DENOM(I)   
      ENDDO
      Do I = 1, NGRAPHS
