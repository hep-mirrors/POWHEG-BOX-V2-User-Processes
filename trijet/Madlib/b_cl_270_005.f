      SUBROUTINE SB_CL_270_005(P1,ANS)
C  
...
C  
      ...
      common/to_amps_270/  amp2,       jamp2

      integer j,jj
      integer max_bhel
      parameter ( max_bhel =          32 )
      double complex saveamp(ngraphs,max_bhel)
      common/to_saveamp_270/saveamp
      double precision savemom(nexternal-1,2)
      common/to_savemom_270/savemom
...
C          write (*,*) "momenta not the same in Born"
     ...
     &       nhel(5,ihel).eq.-1) THEN
                 T=B_CL_270_005(P1,NHEL(1,IHEL),IHEL,JC(1),T1)            
     ...
     &                 .NOT. GOODHEL(IHEL,IPROC)) THEN
      ...
      COMPLEX*16 FUNCTION B_CL_270_005(P,NHEL,HELL,IC,BORNTILDE)
C  
C Generated by MadGraph II                                              
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
...
C  
      DATA Denom(1  )/            9/                                       
      DATA (CF(i,1  ),i=1  ,6  ) /    64,    1,   -8,   10,   -8,    1/    
C               T[ 3, 2, 4, 1, 5]                                          
      DATA Denom(2  )/            9/                                       
      DATA (CF(i,2  ),i=1  ,6  ) /     1,   64,   10,   -8,   -8,    1/    
C               T[ 3, 2, 1, 5, 4]                                          
      DATA Denom(3  )/            9/                                       
      DATA (CF(i,3  ),i=1  ,6  ) /    -8,   10,   64,    1,    1,   -8/    
C               T[ 3, 2, 4, 5, 1]                                          
      DATA Denom(4  )/            9/                                       
      DATA (CF(i,4  ),i=1  ,6  ) /    10,   -8,    1,   64,    1,   -8/    
C               T[ 3, 2, 5, 1, 4]                                          
      DATA Denom(5  )/            9/                                       
      DATA (CF(i,5  ),i=1  ,6  ) /    -8,   -8,    1,    1,   64,   10/    
C               T[ 3, 2, 1, 4, 5]                                          
      DATA Denom(6  )/            9/                                       
      DATA (CF(i,6  ),i=1  ,6  ) /     1,    1,   -8,   -8,   10,   64/    
C               T[ 3, 2, 5, 4, 1]                                          
C ----------
C BEGIN CODE
C ----------
      B_CL_270_005 = (0D0,0D0)  
      BORNTILDE = (0d0,0d0)
      do ihel=-1,1,2
      if (.not. calculatedBorn) then
      ...
      CALL FVOXXX(W(1,3   ),W(1,5   ),GG ,BMASS   ,ZERO    ,W(1,10  ))     
      ...
      CALL IOVXXX(W(1,12  ),W(1,3   ),W(1,13  ),GG ,AMP(9   ))             
      CALL FVOXXX(W(1,3   ),W(1,1   ),GG ,BMASS   ,ZERO    ,W(1,14  ))     
      CALL FVOXXX(W(1,14  ),W(1,4   ),GG ,BMASS   ,ZERO    ,W(1,15  ))     
      CALL IOVXXX(W(1,2   ),W(1,15  ),W(1,5   ),GG ,AMP(10  ))             
      CALL IOVXXX(W(1,12  ),W(1,10  ),W(1,1   ),GG ,AMP(11  ))             
      CALL JIOXXX(W(1,2   ),W(1,14  ),GG ,ZERO    ,ZERO    ,W(1,16  ))     
      CALL VVVXXX(W(1,5   ),W(1,4   ),W(1,16  ),G ,AMP(12  ))              
      CALL IOVXXX(W(1,12  ),W(1,14  ),W(1,5   ),GG ,AMP(13  ))             
      CALL FVOXXX(W(1,3   ),W(1,4   ),GG ,BMASS   ,ZERO    ,W(1,17  ))     
      CALL IOVXXX(W(1,2   ),W(1,17  ),W(1,13  ),GG ,AMP(14  ))             
      CALL IOVXXX(W(1,9   ),W(1,17  ),W(1,1   ),GG ,AMP(15  ))             
      CALL FVIXXX(W(1,2   ),W(1,1   ),GG ,BMASS   ,ZERO    ,W(1,18  ))     
      CALL FVIXXX(W(1,18  ),W(1,4   ),GG ,BMASS   ,ZERO    ,W(1,19  ))     
      ...
      JAMP(   1) = +AMP(   1)+AMP(   2)+AMP(   5)-AMP(   6)+AMP(   8)
     &             -AMP(  14)+AMP(  15)
      JAMP(   2) = -AMP(   1)+AMP(   4)+AMP(   6)-AMP(   7)-AMP(   9)
     &             +AMP(  12)+AMP(  13)
      JAMP(   3) = -AMP(   1)+AMP(   4)+AMP(   6)-AMP(   7)+AMP(  14)
     &             -AMP(  17)+AMP(  18)
      JAMP(   4) = +AMP(   1)-AMP(   3)+AMP(   5)-AMP(   6)+AMP(   8)
     &             +AMP(   9)+AMP(  11)
     ...
     &             +AMP(  16)+AMP(  17)
      DO I = 1, NCOLOR
          ...
          if (ihel.eq.-1) then
             B_CL_270_005=B_CL_270_005 +RTEMP
          elseif (ihel.eq.1) then
             B_CL_270_005=B_CL_270_005 +(0d0,1d0)*RTEMP
          endif
      ENDDO
      Do I = 1, NGRAPHS
