      SUBROUTINE SB_CL_103_002(P1,ANS)
C  
...
C  
C FOR PROCESS : c~ g -> c~ s s~ g  
C  
C BORN AMPLITUDE IS c~ g -> c~ s s~  
      IMPLICIT NONE
C  
C CONSTANTS
C  
...
C  
      ...
      common/to_amps_103/  amp2,       jamp2

      integer j,jj
      integer max_bhel
      parameter ( max_bhel =          32 )
      double complex saveamp(ngraphs,max_bhel)
      common/to_saveamp_103/saveamp
      double precision savemom(nexternal-1,2)
      common/to_savemom_103/savemom
...
C          write (*,*) "momenta not the same in Born"
     ...
     &       nhel(2,ihel).eq.-1) THEN
                 T=B_CL_103_002(P1,NHEL(1,IHEL),IHEL,JC(1),T1)            
     ...
     &                 .NOT. GOODHEL(IHEL,IPROC)) THEN
      ...
      COMPLEX*16 FUNCTION B_CL_103_002(P,NHEL,HELL,IC,BORNTILDE)
C  
C Generated by MadGraph II                                              
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
...
C ----------
      ...
      DO I = 1, NCOLOR
          ...
          if (ihel.eq.-1) then
             B_CL_103_002=B_CL_103_002 +RTEMP
          elseif (ihel.eq.1) then
             B_CL_103_002=B_CL_103_002 +(0d0,1d0)*RTEMP
          endif
      ENDDO
      Do I = 1, NGRAPHS
