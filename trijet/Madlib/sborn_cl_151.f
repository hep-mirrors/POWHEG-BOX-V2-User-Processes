      subroutine sborn_cl_151(p_born,wgtmunu,wgt)
      implicit none
     ...
     &    wgtmunu(0:3,0:3,nexternal-1)
      ...
      if ( leg.eq.           1  ) then
         call sb_cl_151_001(p_born,wgt1)
         wgt(0)=dble(wgt1(1))+dimag(wgt1(1))
      elseif ( leg.eq.           2  ) then
         call sb_cl_151_002(p_born,wgt1)
         wgt(0)=dble(wgt1(1))+dimag(wgt1(1))
      elseif ( leg.eq.           3  ) then
         call sb_cl_151_003(p_born,wgt1)
         wgt(0)=dble(wgt1(1))+dimag(wgt1(1))
      elseif ( leg.eq.           4  ) then
         call sb_cl_151_004(p_born,wgt1)
         wgt(0)=dble(wgt1(1))+dimag(wgt1(1))
      elseif ( leg.eq.           5  ) then
         ...
         call sb_cl_151_005(p_born,wgt1)
         wgt(0)=dble(wgt1(1))+dimag(wgt1(1))
      endif
     ...
     &                      dconjg(wgt1(2))*wm(i+1)*wps(j+1)
      ...
      !           write (*,*) "ERROR in sborn_cl_151: Bmunu is complex"
      !           stop
      !         endif
      !      endif
