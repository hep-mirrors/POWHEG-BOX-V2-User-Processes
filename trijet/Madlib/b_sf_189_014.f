      SUBROUTINE SB_SF_189_014(P1,ANS)
C  
...
C  
C FOR PROCESS : b~ g -> b~ g g g  
C  
C BORN AMPLITUDE IS b~ g -> b~ g g  
      IMPLICIT NONE
C  
C CONSTANTS
C  
...
C  
      ...
      common/to_amps_189/  amp2,       jamp2

      integer j,jj
      integer max_bhel
      parameter ( max_bhel =          32 )
      double complex saveamp(ngraphs,max_bhel)
      common/to_saveamp_189/saveamp
      double precision savemom(nexternal-1,2)
      common/to_savemom_189/savemom
...
C          write (*,*) "momenta not the same in Born"
      ...
      ANS(IPROC) = 0D0
          DO IHEL=1,NCOMB
             IF (GOODHEL(IHEL,IPROC) .OR. NTRY .LT. 2) THEN
                 T=B_SF_189_014(P1,NHEL(1,IHEL),IHEL,JC(1))              
      ...
      REAL*8 FUNCTION B_SF_189_014(P,NHEL,HELL,IC)
C  
C Generated by MadGraph II                                              
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
...
C  
      DATA Denom(1  )/           54/                                       
      DATA (CF(i,1  ),i=1  ,6  ) /     1,  -62,   10,   10,  -62,   28/    
C               T[ 1, 3, 5, 4, 2]                                          
      DATA Denom(2  )/           54/                                       
      DATA (CF(i,2  ),i=1  ,6  ) /   -62,    1,   10,   28,  -62,   10/    
C               T[ 1, 3, 4, 2, 5]                                          
      DATA Denom(3  )/           54/                                       
      DATA (CF(i,3  ),i=1  ,6  ) /    10,   10,    1,  -62,   28,  -62/    
C               T[ 1, 3, 4, 5, 2]                                          
      DATA Denom(4  )/           54/                                       
      DATA (CF(i,4  ),i=1  ,6  ) /    10,   28,  -62,    1,   10,  -62/    
C               T[ 1, 3, 5, 2, 4]                                          
      DATA Denom(5  )/           54/                                       
      DATA (CF(i,5  ),i=1  ,6  ) /   -62,  -62,   28,   10,    1,   10/    
C               T[ 1, 3, 2, 5, 4]                                          
      DATA Denom(6  )/           54/                                       
      DATA (CF(i,6  ),i=1  ,6  ) /    28,   10,  -62,  -62,   10,    1/    
C               T[ 1, 3, 2, 4, 5]                                          
C ----------
C BEGIN CODE
C ----------
      ...
      CALL FVIXXX(W(1,3   ),W(1,2   ),GG ,BMASS   ,ZERO    ,W(1,6   ))     
      ...
      CALL IOVXXX(W(1,3   ),W(1,8   ),W(1,9   ),GG ,AMP(2   ))             
      CALL FVIXXX(W(1,3   ),W(1,5   ),GG ,BMASS   ,ZERO    ,W(1,10  ))     
      CALL IOVXXX(W(1,10  ),W(1,8   ),W(1,2   ),GG ,AMP(3   ))             
      ...
      CALL IOVXXX(W(1,17  ),W(1,13  ),W(1,2   ),GG ,AMP(14  ))             
      CALL IOVXXX(W(1,17  ),W(1,1   ),W(1,9   ),GG ,AMP(15  ))             
      CALL FVOXXX(W(1,1   ),W(1,2   ),GG ,BMASS   ,ZERO    ,W(1,18  ))     
     ...
     &             +AMP(  11)-AMP(  13)
      JAMP(   3) = -AMP(   2)+AMP(   4)-AMP(   5)+AMP(   7)-AMP(   9)
     &             -AMP(  11)+AMP(  12)
     ...
     &             -AMP(  16)+AMP(  17)
      ...
      DO I = 1, NCOLOR
          ...
          ENDDO
          B_SF_189_014 =B_SF_189_014+ZTEMP*DCONJG(JAMP(I))/DENOM(I)   
      ENDDO
      Do I = 1, NGRAPHS
