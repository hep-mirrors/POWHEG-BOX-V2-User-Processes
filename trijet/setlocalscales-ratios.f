 ...
 33   continue
     ...
     >                  sudakovs
      ...
      if(ini) then
         ...
         ktmerging = .false.
         if (powheginput("#ktmerging").gt.0d0) ktmerging = .true.
         ini = .false.
      endif
...
c the merging will be done from jmerge,kmerge into jmerge (kmerge will
c beinvalidated. At the beginning, the scales of all partons is zero.
c At subsequent steps, this scale will be set at the relative pt of the pair.
...
c factors:
               alphafactAll = basicfac
               alphafact1st = basicfac
               sudakovs = 1d0
c
               nlofac=basicfac
...
c invalidate the kmerge entry
            lflav(kmerge)=onem
c set the j entry to the merged flavour
...
c factors:
               sudakovs = sudakovs*
     >                    sudakov(q2merge0,q2merge,scales(j),lflav(j))
c
               bornfac=bornfac+
...
c factors:
         alphafactAll = alphafactAll
     >                * (alphas/st_alpha)**(st_bornorder-inlofac)
         alphafact2nd3rd = (alphas/st_alpha)**(st_bornorder-inlofac)
c
...
C                                                                     
      subroutine sudakov_exponent_HWJ(q20,q2h,theExponent,
     $     isquark,theAccuracy)
      implicit none 
      include 'pwhg_st.h'
      include 'pwhg_math.h'
