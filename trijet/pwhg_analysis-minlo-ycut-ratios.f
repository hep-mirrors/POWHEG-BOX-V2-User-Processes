...
c  pwhgfill  :  fills the histograms with data
     ...
     >                      //'-ymax-'//cymax(j),dy,-5d0,5d0)
          if ((k.eq.3).and.(i.eq.1).and.(j.eq.3)) then
            call bookupeqbins('j'//cnum(k)//'-pt-ptmin'//cptmin(i)
     >                        //'-ymax-'//cymax(j),dpt,0d0,800d0)
          else
            call bookupeqbins('j'//cnum(k)//'-pt-ptmin'//cptmin(i)
...
c     1           '-m'//cptmin(i),dpt,0d0,400d0)  
      ...
      enddo
      call bookupeqbins('rescfact',dpt,0d0,800d0)
      call bookupeqbins('alphafactAll',dpt,0d0,800d0)
      call bookupeqbins('alphafact1st',dpt,0d0,800d0)
      call bookupeqbins('alphafact2nd3rd',dpt,0d0,800d0)
      call bookupeqbins('sudakovs',dpt,0d0,800d0)
      call bookupeqbins('pdfrwgt',dpt,0d0,800d0)
      call bookupeqbins('rationorm',dpt,0d0,800d0)
      call bookupeqbins('mufs',dpt,0d0,800d0)
...
c
      real*8 alphafactAll,alphafact1st,alphafact2nd3rd,sudakovs
      common/alphafacts/alphafactAll,alphafact1st,alphafact2nd3rd,
     >                  sudakovs
      real*8 rescfact
      integer iborn
      real * 8 dsigone(maxmulti)
      real*8 totborn,totbornpdf
      real * 8 pdf1(-pdf_nparton:pdf_nparton),
     1         pdf2(-pdf_nparton:pdf_nparton)
      real * 8 pdf1new(-pdf_nparton:pdf_nparton),
     1         pdf2new(-pdf_nparton:pdf_nparton)
c
      real*8 muf_old,muf_new
c
...
c We plot the ratios as well:
          if ((mjets.ge.3).and.(i.eq.1).and.(j.eq.3)) then
c We calculate the original pdf's:
            call pdfcall(1,kn_xb1,pdf1)
            call pdfcall(2,kn_xb2,pdf2)
            totborn = 0d0
            totbornpdf = 0d0
            do iborn=1,flst_nborn
              totborn = totborn + br_born(iborn)
              totbornpdf = totbornpdf + br_born(iborn)*
     >                     pdf1(flst_born(1,iborn))*
     >                     pdf2(flst_born(2,iborn))
            end do
c We store the original muf:
            muf_old = sqrt(st_mufact2)
c For each born we call the setlocalscales routine:
            do iborn=1,flst_nborn
              dsigone = dsig*br_born(iborn)/totborn
              call setlocalscales(iborn,1,rescfact)
c New muf:
              muf_new = sqrt(st_mufact2)
c For each underlying Born flavor configuration we calculate the new pdf's:
              call pdfcall(1,kn_xb1,pdf1new)
              call pdfcall(2,kn_xb2,pdf2new)
c Plot the ratios:
              call filld('rescfact',ktj(3),rescfact*dsigone)
              call filld('alphafactAll',ktj(3),alphafactAll*dsigone)
              call filld('alphafact1st',ktj(3),alphafact1st*dsigone)
              call filld('alphafact2nd3rd',ktj(3),
     >                   alphafact2nd3rd*dsigone)
              call filld('sudakovs',ktj(3),sudakovs*dsigone)
              call filld('pdfrwgt',ktj(3),pdf1new(flst_born(1,iborn))*
     >                   pdf2new(flst_born(2,iborn))*dsigone*
     >                   totborn/totbornpdf)
              call filld('mufs',ktj(3),muf_new/muf_old*dsigone)
            end do
            call filld('rationorm',ktj(3),dsig)
          end if
