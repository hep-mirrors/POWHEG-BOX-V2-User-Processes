...
#define LL(x,y,z,w) (x-y)*(z-w)
  ...
  ampq2g3::Vg(int p1, int p2, int p3, int p4, int p5) const
  {
    double s12 = S(1,2), s23 = S(2,3), s34 = S(3,4), s45 = S(4,5),
	   s51 = S(5,1);
    _ComplexD l12 = Log(-s12), l23 = Log(-s23), l34 = Log(-s34),
	      l45 = Log(-s45), l51 = Log(-s51);
    
    // commentAK last two lines added to incorporate the mu dependent terms 
    // and the log^2's:
    
    return LL(l12,l23,l34,l45) + LL(l23,l34,l45,l51) + LL(l34,l45,l51,l12)
      + LL(l45,l51,l12,l23) + LL(l51,l12,l23,l34) + 8.224670334241132
      - 0.5*((lmu2 - l12)*(lmu2 - l12) + (lmu2 - l23)*(lmu2 - l23) 
      + (lmu2 - l34)*(lmu2 - l34) + (lmu2 - l45)*(lmu2 - l45) + (lmu2 - l51)*(lmu2 - l51)); 
  }  
...
//    std::cout << p1 << " " << p2 << " " << p3 << " " << p4 << " " << p5 << std::endl;
    ...
    VL    += -1.5*lmu2;
    // from Vgqqb:
    VL    += 0.5*((lmu2 - l12)*(lmu2 - l12) + (lmu2 - l23)*(lmu2 - l23));
...
//    std::cout << p1 << " " << p2 << " " << p3 << " " << p4 << " " << p5 << std::endl;
  ...
  { 
    ...
    _ComplexD Lsum(0.0), Csum(0.0), Asum(0.0), Csumsf(0.0), Asumsf(0.0);
    
    for(unsigned i = 0; i < 6; i++) {
      res[i].A0 = A[i].Atree;
      res[i].A1 = (1.0+onc2)*A[i].AL - onc2*A[i].Asusy - nf1*A[i].Asf;
    // To obtain the result in the HV scheme the following quantity should be added:
      res[i].A1 += -0.5*(1.0 - onc2)*A[i].Atree;
      
      Lsum += A[i].AL + B[i].AL;
      if(i < 3) Csum += A[i].Asusy + B[i].Asusy;
      else Asum += A[i].Asusy + B[i].Asusy;
      if(i < 3) Csumsf += A[i].Asf + B[i].Asf;
      else Asumsf += A[i].Asf + B[i].Asf;
    }
    
    for(unsigned i = 0; i < 6; i++) {
//      res[i].A3 =  Lsum - B[i].Asusy - B[(i+3)%6].Asusy;
//      res[i].A4 = -Lsum + (i < 3 ? Csum : Asum) + nf2*(A[i].Asf + B[i].Asf); 
      res[i].A4 = -Lsum + (i < 3 ? Csum : Asum) + nf2*(i < 3 ? Csumsf : Asumsf); 
    }
//
    res[0].A3 =  Lsum - B[2].Asusy - B[4].Asusy;
    res[1].A3 =  Lsum - B[0].Asusy - B[3].Asusy;
    res[2].A3 =  Lsum - B[1].Asusy - B[5].Asusy;
    res[3].A3 =  Lsum - B[5].Asusy - B[1].Asusy;
    res[4].A3 =  Lsum - B[3].Asusy - B[0].Asusy;
    res[5].A3 =  Lsum - B[4].Asusy - B[2].Asusy;
  } 
