...
c     Look for number to read
 ...
 999  write(*,*) 'next_key_value_pair: malformed input buffer'
      ...
      integer j
c     EB: added ISR/FSR QED matching scale
...
c        EB: added ISR/FSR QED macthing scales
         events(count)%mc_isr_scale  = mc_isr_scale
         events(count)%mc_fsr_scale  = mc_fsr_scale
...
c        EB: added ISR/FSR QED matching scale
         mc_isr_scale = events(count)%mc_isr_scale
         mc_fsr_scale = events(count)%mc_fsr_scale
