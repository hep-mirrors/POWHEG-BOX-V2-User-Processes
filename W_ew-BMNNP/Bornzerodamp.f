...
c the real contribution to implement Born zero suppression
     ...
     1     powheginput,dotp,amp,ratio,hnew_damp,omcth
      logical ini
      data ini/.true./
      logical new_damp,theta_damp
      integer numopt,em
      save ini,h,new_damp,hnew_damp,theta_damp
...
c In case of photon emission, we need to damp the region where the
            if(em.lt.3) then
               pt2 = kn_preal(1,5)**2+kn_preal(2,5)**2
            else
               pt2 = 2*dotp(kn_preal(0,em),kn_preal(0,5))
     1              +kn_masses(em)**2
            endif
