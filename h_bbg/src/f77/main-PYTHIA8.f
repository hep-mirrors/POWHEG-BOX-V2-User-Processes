...
!     !>> read scalup of production stage
        scalup= scalup * scalupfac
!     !>> calculate scalup of decay stage
        call scalup_hbbg_minlo(scalup_dec)
        scalup_dec= scalup_dec * scalupfac

!     !>> this is passed to Pythia (in case "canveto=1")
        vetoscaleH= -1d0
        if( canveto.eq.1 ) vetoscaleH= scalup_dec
        
        iatt=1  !> reset counter of attempts
        call copylh

        do kloop=1,maxloop
!     !>> insist to shower this event
          ...
          if(iret.ne.1) then
!     !>> showering attempt failed
            write(*,*) '>> return code ',iret,' [ retry ]'
            if(iatt.eq.1) then
...
!     !>> showering attempt successful (still check if scalup_dec respected)
            continue
            
!     !>> translate Pythia to HepEvt
            call pythia_to_hepevt(nmxhep,nhep,isthep,idhep,jmohep,jdahep,phep,vhep)

!     !>> check Higgs decay shower history to find Hdec_hardness
            call get_Hdec_hardness(Hdec_hardness)

            if(nevhep.lt.10)then
              write(*,*) '## veto scales:'
     $             ,'scalup_dec=',scalup_dec
     $             ,'Hdec_hardness=',Hdec_hardness
            endif

!     !>> check if parton shower history is acceptable for scalup_dec
            if( Hdec_hardness.gt.scalup_dec )then
!     !>> attempt showering again
              continue
              Hdec_hardness_min=min(Hdec_hardness_min,Hdec_hardness) !save minimal hardness for statistics
            else
!     !>> shower history compatible with scalup: exit
              success= .true.
              exit
            endif
          endif
...
!     !>> print event if not successfull
        if( .not.success )then
          write(*,*)'WARNING: problem with showering this event'
          write(*,*)'>> attempts=',kloop
          write(*,*)'>> scalup_dec=',scalup_dec
          write(*,*)'>> Hdec_hardness_min=',Hdec_hardness_min
          write(*,*)''
          nfail= nfail + 1
        endif

!     !>> if shower successful and shower history accepted print and analyse
        if(iret.eq.1) then
!     !>> print a few examples
          if(nevhep.lt.10) then
            do ihep=1,nhep
              write(*,100) 
     $             ihep,isthep(ihep),idhep(ihep),jmohep(1,ihep),
     $             jmohep(2,ihep),jdahep(1,ihep),jdahep(2,ihep), (phep(k,ihep),k=1,5)
            enddo
            write(*,*)'#############################################'
            write(*,*)'#############################################'
          endif
...
!     !>> call analysis
          if( success ) call pyanal
          
!     !>> print out histograms and statistics
