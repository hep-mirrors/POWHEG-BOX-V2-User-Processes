      subroutine ww_smcouplings_test
      implicit none
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
c PDG 7-7-2011
      ww_ph_Zmass  = 91.1876d0
c PDG 7-7-2011
      ww_ph_Wmass  = 80.399d0     
c PDG 7-7-2011, SM Review (Langaker)
      ww_ph_gmu    = 1.166364d-5
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      ww_ph_alphaem = sqrt(2d0)*ww_ph_gmu*ww_ph_Wmass2/pi*ww_ph_sthw2

c alphas(Mz) from PDG review 7-7-2011      
      asmzopi=pwhg_alphas(ww_ph_Zmass2,st_lambda5MSB,5)/pi
      asmwopi=pwhg_alphas(ww_ph_Wmass2,st_lambda5MSB,5)/pi
...
c 2.0995 with this formula, 0.7% difference, 0.34 standard deviations
      ww_ph_Wwidth = ww_ph_gmu*ww_ph_Wmass**3/(6*sqrt(2d0)*pi)
     1 *(3+3*2*(1+deltas))

      deltas=asmzopi
c     ph_Zwidth = 2.4952 +- 0.0023, PDG 7-7-2011
c 2.5106 with this formula, 0.6% differemnce, 6.7 standard deviations
      ww_ph_Zwidth = ww_ph_gmu*ww_ph_Zmass**3/(6*sqrt(2d0)*pi)*(
c gv=t3L-2q sin^2 thetaW, ga=t3L
c neutrinos
     1     3*((0.5d0)**2+(0.5d0)**2)  +
c leptons
     2     3*((-0.5d0+2*ww_ph_sthw2)**2+(-0.5d0)**2) +
     3    (
c up quarks
     4     3*2*((0.5d0-2*2d0/3d0*ww_ph_sthw2)**2+(0.5d0)**2) +
c down quarks
     5     3*3*((-0.5d0+2*1d0/3d0*ww_ph_sthw2)**2+(-0.5d0)**2) 
     6    )*(1+deltas) )
c
      write(*,*) ' 1/alpha',1/ww_ph_alphaem
      write(*,*) ' W width',ww_ph_Wwidth
      write(*,*) ' Z width',ww_ph_Zwidth
      write(*,*) ' Wbranch lep ', 3/(3+3*2*(1+ww_ph_deltas))
      write(*,*) ' Wbranch had ', 1-3/(3+3*2*(1+ww_ph_deltas))
