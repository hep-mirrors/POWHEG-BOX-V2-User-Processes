...
c$$$      CHARACTER*(*) LTIT,BTIT,SCALE
c$$$      include 'histo.f'
c$$$      
c$$$      IF(BOOK(N).NE.'YES') RETURN
c$$$      WRITE(99,101) TITLE(N),TITLE(N),TITLE(N),SCALE,HMIN(N),HMAX(N)
c$$$  100 FORMAT( /1x,                               
...
c$$$     &' SET WINDOW X 2.5 TO 10.'/,1X,
c$$$     &' SET SYMBOL 5O SIZE 1.8'/,1X,
c$$$     &' TITLE TOP ','"',A,' distribution"',/1X,
c$$$     &' TITLE BOTTOM ','"',A,'"',/1X,
c$$$     &' TITLE LEFT ','"dS/d',A,' [fb]"',/1X,
c$$$     &' CASE       ','" G"',/1X,
c$$$     &' SET SCALE Y ',A5,/1X,
c$$$     &' (SET TICKS TOP OFF)   '/1x,     
c$$$     &' SET LIMITS X ',F10.5,' ',F10.5,/1X,
...
c$$$      WRITE(99,'(3X,G13.6,2(2X,G13.6))')  
c$$$     &                            XHIS(N,J),HIST(N,J),HIST(M,J)
c$$$    1 CONTINUE
c$$$      WRITE(99,200)
c$$$  200 FORMAT('   PLOT')
c$$$      WRITE(99,300) HINT(N),HAVG(N),HSIG(N),IENT(N),IUSCORE(N)
c$$$     &   ,IOSCORE(N)
c$$$  300 FORMAT( /1x,                               
c$$$     &' BOX 7. 0.75 SIZE 9. 1.5'/,1X,
c$$$     &' SET WINDOW Y 0. TO 2.'/,1X,
c$$$     &' SET TITLE SIZE -1.5'/1X,
c$$$     &' TITLE 2.8 1.2 "INTGRL =',E12.5,'   AVGE =',E12.5,
c$$$     &             '   RMS =',E12.5,'"',/1X,
c$$$     &' TITLE 2.8 0.8 "Entries =',I9,2x,'U`flow =',I9,2X
c$$$     &                                 ,'O`flow =',I9,'"',/1X,
c$$$     &' SET TITLE SIZE -2')
c$$$      WRITE(99,400)
c$$$  400 FORMAT('   NEW PLOT')
c$$$      END
      ...
      IF(BOOK(N).NE.'YES') RETURN
      l = lenocc(TITLE(N))
      write(99,'(a,i3)')'# '//TITLE(N)(1:l)//' index ',N-1
c$$$      write(99,'(a,a,e14.8,a,e14.8,a,e14.8,a,i9,a,i9,a,i9)')
c$$$     $ '#','  Int = ',   HINT(N),   ', Avg = ',    HAVG(N),
c$$$     $     ', RMS = ',   HSIG(N),   ', Entries = ',IENT(N),
c$$$     $     ', U-flow = ',IUSCORE(N),', O-flow = ', IOSCORE(N)
      halfbinwidth=(HMAX(N)-HMIN(N))/NBIN(N)/2d0
      DO 1 J=1,NBIN(N)
      write(99,'(4(1x,d16.8))') 
     &      XHIS(N,J)-halfbinwidth,
     &      XHIS(N,J)+halfbinwidth,HIST(N,J),HIST(M,J)
    1 CONTINUE
      WRITE(99,'(a)') ''
      WRITE(99,'(a)') ''
      END
