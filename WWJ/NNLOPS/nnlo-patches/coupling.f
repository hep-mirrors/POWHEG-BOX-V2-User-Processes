...
c--- Now set up the other derived parameters
      gwsq=fourpi*aemmz/xw       !      gwsq=fourpi/xw/137.035999679d0     ! MI WS
      esq=gwsq*xw
      gw=dsqrt(gwsq)
      call couplz(xw)
...
c--- initialize alpha_s for H-> b bbar decay
      asdec=alphas(abs(scaledec),amz,nlooprun)
      ason2pidec=asdec/twopi
      gsqdec=fourpi*asdec

***************************************

      write(*,*) '============================='
      write(*,*) 'ew scheme ',ewscheme
      write(*,*) 'for scheme 1, inputs are wmass, zmass, gfermi'
      write(*,*) 'aeminv,sthw2,gweak,gfermi ',1./aemmz,xw,gw,gf
c      write(*,*) 'mh,gammah ',hmass,hwidth
      write(*,*) 'mb (kinematic) ',mb
      write(*,*) 'mbsq (yukawa) ',mbsq
      write(*,*) 'gwsq ',gwsq
      write(*,*) 'as ',ason2pi*2*pi
      write(*,*) '============================='


      write(6,99)gsq/fourpi,gsqdec/fourpi,amz,nlooprun

 99   format(' CCCCCCCCCCC  Strong coupling, alpha_s   CCCCCCCCCCCC'/,
     .      ' C                                                  C'/,
     .      ' C   alpha_s (mur)=',f8.5,'                         C'/,                 
     .      ' C   alpha_s (mur_dec)=',f8.5,'                     C'/,                 
     .      ' C   alpha_s (mz) =',f8.5,'                         C'/,     
     .      ' C   (using ',i1,
