     ...
     $     ,pt_lplm,deta_lplm,dy_lplm,dphi_ll_nunu,absctheta

      real *8 dabsy_ww,dabsy_lplm
      
      real *8 yv,deta_ww,dy_ww,dphi_ww,dr_ww,httot
...
c      integer jetvec(maxjet)
     ...
     $     ,dr_l_j_max
      ...
      real *8 dphi,dphi_lmmiss, dphi_lpmiss  

!      integer, parameter :: ngamma_max = 20 
!      integer :: ngamma 
!      real *8 pgamma(4,ngamma_max) 
!      common/cgamma/ngamma,pgamma 
!      logical add_ele(ngamma_max), add_pos(ngamma_max) 
...
c=== BOOKING ===========================================================
      if(book) then
         ...
         if (process == 100) write(*,*) 'Doing WWJ production' 
         write(*,*) '====================================='
...
C     build histograms for inclusive (in the number of jets) observables
            ...
            call bookupeqbins('total_posdyWW'//prefix,1d0,0d0,1d0)
            call bookupeqbins('total_negdyWW'//prefix,1d0,0d0,1d0)
...
c     Collins-Soper variables:
            ...
            call bookupeqbins('phi_CS.Wm_0.2_dy23'//prefix,  0.2d0,0d0,6.4d0)
            call bookupeqbins('phi_CS.Wp_0.2_dy23'//prefix,  0.2d0,0d0,6.4d0)
            call bookupeqbins('phi_CS.Wm_0.2_dym23'//prefix,  0.2d0,0d0,6.4d0)
            call bookupeqbins('phi_CS.Wp_0.2_dym23'//prefix,  0.2d0,0d0,6.4d0)

            call bookupeqbins('phi_CS.Wm_0.2_dy35'//prefix,  0.2d0,0d0,6.4d0)
            call bookupeqbins('phi_CS.Wp_0.2_dy35'//prefix,  0.2d0,0d0,6.4d0)
            call bookupeqbins('phi_CS.Wm_0.2_dym35'//prefix,  0.2d0,0d0,6.4d0)
            call bookupeqbins('phi_CS.Wp_0.2_dym35'//prefix,  0.2d0,0d0,6.4d0)


C            if (icut == 0) then 
C               call bookupeqbins('phi_CS.Wm_r1',0.2d0,0d0,6.4d0) 
C               call bookupeqbins('phi_CS.Wp_r1',0.2d0,0d0,6.4d0) 
C               call bookupeqbins('phi_CS.Wm_r2',0.2d0,0d0,6.4d0) 
C               call bookupeqbins('phi_CS.Wp_r2',0.2d0,0d0,6.4d0) 
C               call bookupeqbins('phi_CS.Wm_r3',0.2d0,0d0,6.4d0) 
C               call bookupeqbins('phi_CS.Wp_r3',0.2d0,0d0,6.4d0) 
C               call bookupeqbins('phi_CS.Wm_r4',0.2d0,0d0,6.4d0) 
C               call bookupeqbins('phi_CS.Wp_r4',0.2d0,0d0,6.4d0) 
C            endif
...
c     ------this is the system made by the two charged leptons
            ...
            call bookupeqbins('dabsy.WpWm_FBasymmetry_100'//prefix,
     $           100d0,-100d0,100d0)
            call bookupeqbins('dabsy.WpWm_FBasymmetry_0.1'//prefix,
     $           0.1d0,-10d0,10d0)
...
c     ------yWW, with reweighting bins
            call bookupeqbins('y.WW'//prefix,1d0,0d0,dble(nbin_yWW))
...
c     lepton and lepton pairs
            ...
            call bookupeqbins('eta.lp_0.1'//prefix,  0.1d0,-5d0,5d0)
            call bookupeqbins('eta.lm_0.1'//prefix,  0.1d0,-5d0,5d0)

            call bookupeqbins('dabsy.lplm_FBasymmetry_100'//prefix,
     $           100d0,-100d0,100d0)
            call bookupeqbins('dabseta.lplm_FBasymmetry_0.1'//prefix,
     $           0.1d0,-10d0,10d0)

            
c     abscosthetastar = | tanh(Delta_eta(l_1,l_2)/2) |
            call bookupeqbins('abscosthetastar_0.01'//prefix,  0.01d0,0d0,1d0)
...
c     CS momenta
     ...
     c              1d0,0d0,dble(nbin_yWW))
            ...
            if (icut == 1) call bookupeqbins('total_jetveto2',1d0,0d0,1d0)            
...
CC dress leptons with photons before doing anything else          
C         add_ele = .false. 
C         do i=1,ngamma 
C            call getdydetadphidr(p_ele,pgamma(1:4,i),dummy,dummy,dummy,dR)
C            if (dR < 0.1d0) then 
C               add_ele(i) = .true.
C            endif 
C         enddo
C         do i=1,ngamma 
C!            write(*,*) 'pgamma', i, pgamma(i,:4) 
C            if (add_ele(i)) p_ele = p_ele+pgamma(1:4,i) 
C         enddo
C
C         add_pos = .false. 
C         do i=1,ngamma 
C            call getdydetadphidr(p_pos,pgamma(1:4,i),dummy,dummy,dummy,dR)
C            if (dR < 0.1d0) then 
C               add_pos(i) = .true.
C            endif 
C         enddo
C         do i=1,ngamma 
C            if (add_pos(i) .and. .not.add_ele(i)) p_pos = p_pos+pgamma(1:4,i) 
C         enddo
C!         write(*,*) 'add?', add_pos(:ngamma),add_ele(:ngamma)
C!         write(*,*) 'after: p_ele', p_ele
C!         write(*,*) 'after: p_pos', p_pos
...
c     leptons and Ws
         ...
         call getyetaptmass(plp,y_lp,eta_lp,pt_lp,dummy)
         call getyetaptmass(plm,y_lm,eta_lm,pt_lm,dummy)
...
c         print*, eta_lm/y_lm
         ...
         dabsy_lplm  = abs(eta_lp)-abs(eta_lm) 
        
         plplm    = p_pos+p_ele
         call getyetaptmass(plplm,y_lplm,dummy,pt_lplm,m_lplm)

         call getdydetadphidr(plp,plm,dy_lplm,deta_lplm,dphi_lplm,dummy)

         absctheta = dabs(tanh(deta_lplm/2d0))
     ...
     $        ,dummy)
         ...
         call getdydetadphidr(plp,pmiss,dummy,dummy,dphi_lpmiss,dummy)
         call getdydetadphidr(plm,pmiss,dummy,dummy,dphi_lmmiss,dummy)
         dphi = min(dphi_lpmiss,dphi_lmmiss)

         if(dphi .lt. pi/2d0) then
            ptmiss_rel = ptmiss * sin(dabs(dphi))
         else
            ptmiss_rel = ptmiss
         endif
...
C            call getdydetadphidr(pj(:,i),plp,dummy,dummy,dummy,dR)
C     GZ change to account for the fact that MW has mu+ e- and we generated events with e+ and mu-
C     (the cut should act on leptons) 
            call getdydetadphidr(pj(:,i),plm,dummy,dummy,dummy,dR)
...
c     inclusive
            prefix=trim('_incl')
            condition=.true.
C     GZ add pt_wp and pt_wm cuts 
           condition = condition .and. (pt_wp .gt. 200d0) 
     C           .and. (pt_wm. gt. 200d0) 
         elseif(icut.eq.1) then
c     fiducial (NO JET VETO)
            prefix=trim('_fid')
            condition=.true.
...
c     fill histograms
            ...
            if (dy_WW > 0d0) then 
               call filld('total_posdyWW'//prefix,      0.5d0,    wt)
            else
               call filld('total_negdyWW'//prefix,      0.5d0,    wt)
            endif
...
CC      add cuts one by one to understadn where the dip in phi comes from 
C            if (icut == 0) then 
C               if ( m_lplm.gt.mll_min) then 
C               call filld('phi_CS.Wm_r2',  phi_CSWm, wt) 
C               call filld('phi_CS.Wp_r2',  phi_CSWp, wt) 
C               if (abs(eta_lp).lt.etal_max.and. abs(eta_lm).lt.etal_max) then 
C               call filld('phi_CS.Wm_r3',  phi_CSWm, wt) 
C               call filld('phi_CS.Wp_r3',  phi_CSWp, wt) 
C               if (  ptmiss.gt.ptmiss_min.and. ptmiss_rel.gt.ptmiss_rel_min) then 
C               call filld('phi_CS.Wm_r4',  phi_CSWm, wt) 
C               call filld('phi_CS.Wp_r4',  phi_CSWp, wt) 
C               if (pt_lp.gt.ptl_min.and. pt_lm.gt.ptl_min) then
C               call filld('phi_CS.Wm_r1',  phi_CSWm, wt) 
C               call filld('phi_CS.Wp_r1',  phi_CSWp, wt) 
C               endif
C               endif
C               endif
C               endif
...
C
            ...
            if (2d0 .lt. dy_ww .and. dy_ww .lt. 3d0) then 
               call filld('phi_CS.Wm_0.2_dy23'//prefix, phi_CSWm, wt)  
               call filld('phi_CS.Wp_0.2_dy23'//prefix,  phi_CSWp, wt)  
            elseif (-3d0 .lt. dy_ww .and. dy_ww .lt. -2d0) then 
               call filld('phi_CS.Wm_0.2_dym23'//prefix, phi_CSWm, wt) 
               call filld('phi_CS.Wp_0.2_dym23'//prefix, phi_CSWp, wt) 
            endif
            if (3d0 .lt. dy_ww .and. dy_ww .lt. 5d0) then 
               call filld('phi_CS.Wm_0.2_dy35'//prefix, phi_CSWm, wt) 
               call filld('phi_CS.Wp_0.2_dy35'//prefix,  phi_CSWp, wt) 
            elseif (-5d0 .lt. dy_ww .and. dy_ww .lt. -3d0) then 
               call filld('phi_CS.Wm_0.2_dym35'//prefix,phi_CSWm, wt) 
               call filld('phi_CS.Wp_0.2_dym35'//prefix, phi_CSWp, wt) 
            endif
...
c     ------ptwm, with reweighting bins
            ...
            call filld('dabsy.WpWm_FBasymmetry_100'//prefix, dabsy_ww, wt)
...
!            write(*,*) 'rap,eta,,wt',icut,rapj_1,etaj_1,wt(1)
            ...
            call filld('eta.lp_0.1'//prefix,  eta_lp, wt)
            call filld('eta.lm_0.1'//prefix,  eta_lm, wt)

            call filld('dabsy.lplm_FBasymmetry_100'//prefix, dabsy_lplm, wt)
            call filld('dabseta.lplm_FBasymmetry_0.1'//prefix, dabsy_lplm, wt)
            call filld('abscosthetastar_0.01'//prefix,  absctheta, wt)
...
C            write(*,*) 'x_dyWW, dy_ww',x_dyWW, dy_ww
     ...
     c              x_yww , mult_Wp(i) * wt)
            ...
            if (icut == 1 .and. .not. jetveto_muon) 
     C           call filld('total_jetveto2',0.50,wt)            
