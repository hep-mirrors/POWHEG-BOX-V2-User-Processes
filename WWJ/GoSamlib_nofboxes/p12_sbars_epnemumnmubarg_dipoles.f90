module     p12_sbars_epnemumnmubarg_dipoles
   use p12_sbars_epnemumnmubarg_config, only: ki
   use p12_sbars_epnemumnmubarg_color, only: gammaF, gammaA, &
       & CF, CA, numcs, KF, KA, &
       & T1T2, &
       & T1T7, &
       & T2T7
   use p12_sbars_epnemumnmubarg_kinematics, only: num_legs, dotproduct, &
       lambda
   implicit none
...
contains
   ...
   function I_ff_qed(mu_sq, vec, I, J)
      ...
      if (flag) then
              ...
              term = V_SING_QED(s_jk, real(0, ki), real(0, ki))
              term(1) = (term(1) + log_s * term(2) ) * ncs*(-3.0_ki)*(3.0_ki)/9.0_ki
              term(2) = term(2)* ncs*(-3.0_ki)*(3.0_ki)/9.0_ki
              I_ff_qed(:,:,1) = I_ff_qed(:,:,1) + term(1) * matrix(:,:)
              I_ff_qed(:,:,2) = I_ff_qed(:,:,2) + term(2) * matrix(:,:)
      end if
      ...
      if (flag) then
              ...
              term = V_SING_QED(s_jk, real(0, ki), real(0, ki))
              term(1) = (term(1) + log_s * term(2) ) * ncs*(-0.0_ki)*(3.0_ki)/9.0_ki
              term(2) = term(2)* ncs*(-0.0_ki)*(3.0_ki)/9.0_ki
              I_ff_qed(:,:,1) = I_ff_qed(:,:,1) + term(1) * matrix(:,:)
              I_ff_qed(:,:,2) = I_ff_qed(:,:,2) + term(2) * matrix(:,:)
      end if
   ...
   function I_ii_qed(mu_sq, vec, I, J)
      ...
      if (flag) then
              ...
              term(1) = ncs*(1.0_ki)*(-1.0_ki)/9.0_ki
              term(2) = ncs*(1.0_ki)*(-1.0_ki)/9.0_ki
...
end module p12_sbars_epnemumnmubarg_dipoles
