module p16_gdbar_epnemumnmubardbar_matrix
...
contains
   ...
   pure function prefactor()
      ...
      use p0_dbard_epnemumnmubarg_model, only: NC
      use p0_dbard_epnemumnmubarg_kinematics, only: &
      & in_helicities, symmetry_factor
      use p0_dbard_epnemumnmubarg_color, only: incolors
      implicit none
   ...
   subroutine     initgolem(is_first)
      use p0_dbard_epnemumnmubarg_matrix, only: orig_initgolem => initgolem
      implicit none
   ...
   subroutine     exitgolem(is_last)
      use p0_dbard_epnemumnmubarg_matrix, only: orig_exitgolem => exitgolem
      implicit none
   ...
   subroutine     samplitude(vecs, scale2, amp, prec, ok, h)
      use p0_dbard_epnemumnmubarg_matrix, only: orig_sub => samplitude
      implicit none
   ...
   function     samplitudel0(vecs, h) result(amp)
      use p0_dbard_epnemumnmubarg_matrix, only: orig_func => samplitudel0
      implicit none
      real(ki), dimension(7, 4), intent(in) :: vecs
   ...
   function     samplitudel1(vecs,scale2,ok,rat2,h) result(amp)
      use p0_dbard_epnemumnmubarg_matrix, only: orig_func => samplitudel1
      implicit none
      real(ki), dimension(7, 4), intent(in) :: vecs
   ...
   subroutine     ir_subtraction(vecs,scale2,amp)
      use p0_dbard_epnemumnmubarg_matrix, &
   ...
   subroutine OLP_color_correlated(vecs,ampcc)
      use p0_dbard_epnemumnmubarg_matrix, only: orig_func => OLP_color_correlated
      implicit none
      real(ki), dimension(7, 4), intent(in) :: vecs
   ...
   subroutine OLP_spin_correlated_lo2(vecs, ampsc)
      use p0_dbard_epnemumnmubarg_matrix, only: orig_func => OLP_spin_correlated_lo2
      implicit none
      real(ki), dimension(7, 4), intent(in) :: vecs
...
end module p16_gdbar_epnemumnmubardbar_matrix
