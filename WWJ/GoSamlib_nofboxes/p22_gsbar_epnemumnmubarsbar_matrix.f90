module p22_gsbar_epnemumnmubarsbar_matrix
...
contains
   ...
   pure function prefactor()
      ...
      use p12_sbars_epnemumnmubarg_model, only: NC
      use p12_sbars_epnemumnmubarg_kinematics, only: &
      & in_helicities, symmetry_factor
      use p12_sbars_epnemumnmubarg_color, only: incolors
      implicit none
      ...
      if (include_color_avg_factor) then
         prefactor = prefactor * real(incolors, ki)/NA/NC
      end if
   ...
   subroutine     initgolem(is_first)
      use p12_sbars_epnemumnmubarg_matrix, only: orig_initgolem => initgolem
      implicit none
   ...
   subroutine     exitgolem(is_last)
      use p12_sbars_epnemumnmubarg_matrix, only: orig_exitgolem => exitgolem
      implicit none
   ...
   subroutine     samplitude(vecs, scale2, amp, prec, ok, h)
      use p12_sbars_epnemumnmubarg_matrix, only: orig_sub => samplitude
      implicit none
   ...
   function     samplitudel0(vecs, h) result(amp)
      use p12_sbars_epnemumnmubarg_matrix, only: orig_func => samplitudel0
      implicit none
      real(ki), dimension(7, 4), intent(in) :: vecs
   ...
   function     samplitudel1(vecs,scale2,ok,rat2,h) result(amp)
      use p12_sbars_epnemumnmubarg_matrix, only: orig_func => samplitudel1
      implicit none
      real(ki), dimension(7, 4), intent(in) :: vecs
   ...
   subroutine     ir_subtraction(vecs,scale2,amp)
      use p12_sbars_epnemumnmubarg_matrix, &
   ...
   subroutine OLP_color_correlated(vecs,ampcc)
      use p12_sbars_epnemumnmubarg_matrix, only: orig_func => OLP_color_correlated
      implicit none
      real(ki), dimension(7, 4), intent(in) :: vecs
   ...
   subroutine OLP_spin_correlated_lo2(vecs, ampsc)
      use p12_sbars_epnemumnmubarg_matrix, only: orig_func => OLP_spin_correlated_lo2
      implicit none
      real(ki), dimension(7, 4), intent(in) :: vecs
...
end module p22_gsbar_epnemumnmubarsbar_matrix
