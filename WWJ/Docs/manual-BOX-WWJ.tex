...
directory. In the \tmtexttt{testrun-nnlops} directory, instead, we
provide a template input card to perform a runs in parallel with the
\MINLO{} option and the \NNLOPS{} activated and to obtain events with
multiple (scale variation) weights.
...
\noindent Several decay modes can be selected by an appropriate flag
