      double complex function ww_I3m(s1,s2,s3)
...
!      include 'constants.f'
      ...
      double precision ww_i3m1a,flag
      double complex ww_i3m1b
...
c---case all negative
             flag=0d0
             ww_i3m=ww_i3m1b(smax,smid,smin,rtdel3,flag)
         elseif (smin .gt. 0) then
c---case all positive
             flag=0d0
             ww_i3m=-ww_i3m1b(-smin,-smid,-smax,rtdel3,flag)
...
c---case two positive and one negative
      ...
      rtdel3=sqrt(-del3)
         if (smax .lt. 0) then
c---case all negative
             ww_i3m=+dcmplx(ww_i3m1a(+s1,+s2,+s3,rtdel3))
         elseif (smin .gt. 0) then
c---case all positive
             ww_i3m=-dcmplx(ww_i3m1a(-s1,-s2,-s3,rtdel3))  
          endif
      ...
      double precision function ww_I3m1a(s1,s2,s3,rtmdel)
      implicit none
...
!      include 'constants.f'
      ...
      double precision two,pisq,one
      parameter (two=2d0,pisq=pi**2,one=1d0)
      double precision s1,s2,s3,d1,d2,d3,rtmdel,arg1,arg2,arg3,ww_dclaus
      ...
      ww_i3m1a=two/rtmdel*(ww_Dclaus(arg1)+ww_Dclaus(arg2)
     $                                    +ww_Dclaus(arg3))

      end

      
      double complex function ww_I3m1b(s1,s2,s3,rtdel,flag)
      implicit none
...
!      include 'constants.f'
      ...
      if(argdlx.gt.1.and.argdlx-1d0.lt.1d-6) argdlx = 1d0
      if(argdly.gt.1.and.argdly-1d0.lt.1d-6) argdly = 1d0

      if ((argdlx .gt. 1d0) .or. (argdly .gt. 1d0)) then
      write(6,*) 'problems with call of ww_I3m1b'
     ...
     & +two*(ddilog(argdlx)+ddilog(argdly))
      ww_I3m1b=Dcmplx(temp-abs(flag)*pisq)+impi*Dcmplx(flag*(xlog+ylog))
      ww_I3m1b=-ww_I3m1b/Dcmplx(rtdel)
      end
