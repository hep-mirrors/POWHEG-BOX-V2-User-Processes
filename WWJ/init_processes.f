...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
c      st_nlight=5
      st_nlight=4 !PM use nf=4 pdfs
      call init_couplings


      aswopi=pwhg_alphas(ph_Wmass2,st_lambda5msb,st_nlight)/pi
...
C     set e+mu- as default process (can be overwriten with in the powheg.input) 
      rad_branching=1d0 
      decay=12
      using_default = .true. 
...
c     do only tau+tau-
      if(powheginput("#tau+tau-").eq.1) then
         ...
         decay=33
         write(*,*) '---------------------------'
         write(*,*) 'Doing decay to tau+ tau-'
         write(*,*) '---------------------------'
         using_default = .false. 
      endif
c     do fully leptonic (without taus)
      if(powheginput("#leptonic_notau").eq.1) then
         rad_branching=4d0
         decay=44
         write(*,*) '---------------------------'
         write(*,*) 'Doing leptonic decay (no taus)'
         write(*,*) '---------------------------'
         using_default = .false. 
      endif
c     do fully leptonic (including taus)
      if(powheginput("#leptonic").eq.1) then
         rad_branching=9d0
         decay=55
         write(*,*) '---------------------------'
         write(*,*) 'Doing leptonic decay'
         write(*,*) '---------------------------'
         using_default = .false. 
      endif
c     do fully hadronic
...
C     9 = nc^2 
C     4 = 2^2. 2 for each W, e.g. for W+ we have (udb,usb), (cdb, csb)
C     factor 2 refers to u or c, while down quark is generated in 
C     finalize_lhe with probability sth^2 or csth^2 
C     factors (1+as(pw^2)/pi) included later 
         rad_branching=9d0*4d0*(1d0+aswopi)**2
         decay=66
         write(*,*) '---------------------------'
         write(*,*) 'Doing hadronic decay'
         write(*,*) '---------------------------'
         using_default = .false. 
      endif
c     do fully semi-leptonic
      if(powheginput("#semileptonic_notau").eq.1) then
C     Nc * 2 (up/charm) * 2 (e,mu) * 2 (since we randomize on who decays leptonically or hadronically) 
         rad_branching=3d0*2d0*2d0*2d0*(1d0+aswopi) 
         decay=46
         write(*,*) '---------------------------'
         write(*,*) 'Doing semileptonic decay (no taus)'
         write(*,*) '---------------------------'
         using_default = .false. 
      endif
      if(powheginput("#semileptonic").eq.1) then
C     Nc * 2 (up/charm) * 3 (e,mu,tau) * 2 (since we randomize on who decays leptonically or hadronically) 
         rad_branching=3d0*2d0*3d0*2d0*(1d0+aswopi)  
         decay=56
         write(*,*) '---------------------------'
         write(*,*) 'Doing semiletonic decay'
         write(*,*) '---------------------------'
         using_default = .false. 
      endif
c     do only mu- e+
      if(powheginput("#e+mu-").eq.1) then
         ...
         decay=12
         write(*,*) '---------------------------'
         write(*,*) 'Doing decay to mu- e+'
         write(*,*) '---------------------------'
         using_default = .false. 
      endif 
c     do only mu+ e-
      if(powheginput("#mu+e-").eq.1) then
         rad_branching=1d0 
         decay=21
         write(*,*) '---------------------------'
         write(*,*) 'Doing decay to mu+ e-'
