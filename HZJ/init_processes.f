...
C$$$      enddo
      ...
      include "nlegborn.h"
      include "pwhg_flst.h"
      integer i,temp(nlegreal,maxprocreal)
      real * 8 vdecaymode,powheginput
      external powheginput
...
c     1002: up-type quark
      if(vdecaymode.eq.0) then
         do i=1,flst_nreal
            temp(:,2*i-1) = flst_real(:,i)
            temp(:,  2*i) = flst_real(:,i)
            temp(4, 2*i-1) = -1001
            temp(5, 2*i-1) =  1001
            temp(4,   2*i) = -1002
            temp(5,   2*i) =  1002       
         enddo
         flst_nreal = 2*flst_nreal
         flst_real = temp
      endif
...
c     12  : neutrino
      if(vdecaymode.eq.10) then
         do i=1,flst_nreal
            temp(:,4*i-3) = flst_real(:,i)
            temp(:,4*i-2) = flst_real(:,i)
            temp(:,4*i-1) = flst_real(:,i)
            temp(:,  4*i) = flst_real(:,i)
            temp(4,4*i-3) = -1001
            temp(5,4*i-3) =  1001
            temp(4,4*i-2) = -1002
            temp(5,4*i-2) =  1002
            ...
            temp(5,  4*i) =  12            
         enddo
         flst_nreal = 4*flst_nreal
         flst_real = temp
      endif

C$$$      do i=1,flst_nreal
C$$$         write(*,*) i, flst_real(:,i)
C$$$      enddo
