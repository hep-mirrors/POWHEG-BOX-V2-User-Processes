...
module ol_external_ppttj_ttxddxgg_1
  implicit none
  integer :: dummy_counter
  ! Permutation and inverse permutation of external particles
  integer, save :: external_perm_ppttj_ttxddxgg_1(6) = &
                     [ (dummy_counter, dummy_counter = 1, 6) ]
  integer, save :: external_perm_inv_ppttj_ttxddxgg_1(6) = &
                     [ (dummy_counter, dummy_counter = 1, 6) ]
  integer, save :: extcomb_perm_ppttj_ttxddxgg_1(0:22) = &
                     [ (dummy_counter, dummy_counter = 0, 22) ]
  ! Particle types (mapping of fields to integers is not fixed!)
  integer, save :: particle_types_ppttj_ttxddxgg_1(6) = &
                     [ 1, 2, 3, 4, 5, 5 ]
  ! Colour and helicity average factors per particle
  integer, save :: average_factors_ppttj_ttxddxgg_1(6) = &
                     [ 6, 6, 6, 6, 16, 16 ]
  ! Average factor; initialised to the identity permutation
  integer, save :: average_factor_ppttj_ttxddxgg_1 = &
                     72
  integer, save :: channel_number_ppttj_ttxddxgg_1 = -1
  ...
  integer, save :: photonid_ppttj_ttxddxgg_1(6) = 0

  contains

  subroutine n_external(n) &
      & bind(c,name="ol_f_n_external_ppttj_ttxddxgg_1")
  ...
  subroutine n_external_c(n) &
      & bind(c,name="ol_n_external_ppttj_ttxddxgg_1")
  ...
  subroutine set_permutation(perm) &
      & bind(c,name="ol_f_set_permutation_ppttj_ttxddxgg_1")
    ...
    integer :: i, j, ii, jj
    integer :: particle_types_perm_ppttj_ttxddxgg_1(6)
    external_perm_ppttj_ttxddxgg_1 = perm
    do i = 1, 6
      external_perm_inv_ppttj_ttxddxgg_1( &
        external_perm_ppttj_ttxddxgg_1(i)) = i
      particle_types_perm_ppttj_ttxddxgg_1(i) = &
        particle_types_ppttj_ttxddxgg_1( &
        external_perm_ppttj_ttxddxgg_1(i))
    end do
    do i = 1, 6
      do j = 1, i
        if (external_perm_ppttj_ttxddxgg_1(i) >= &
          external_perm_ppttj_ttxddxgg_1(j)) then
          ii = external_perm_ppttj_ttxddxgg_1(i)
          jj = external_perm_ppttj_ttxddxgg_1(j)
        else
          ii = external_perm_ppttj_ttxddxgg_1(j)
          jj = external_perm_ppttj_ttxddxgg_1(i)
        end if
        extcomb_perm_ppttj_ttxddxgg_1((i*(i-1))/2 + j) = (ii*(ii-1))/2 + jj
      end do
    end do
    ! Colour and helicity average factor
    average_factor_ppttj_ttxddxgg_1 = 1
    do i = 1, n_scatt
      average_factor_ppttj_ttxddxgg_1 = &
        average_factor_ppttj_ttxddxgg_1 &
        * average_factors_ppttj_ttxddxgg_1( &
        external_perm_ppttj_ttxddxgg_1(i))
    end do
    ...
    if (out_symmetry_on /= 0) then
      do i = 1, 6
        average_factor_ppttj_ttxddxgg_1 = &
          average_factor_ppttj_ttxddxgg_1 &
          * factorial(count(particle_types_perm_ppttj_ttxddxgg_1(n_scatt+1:6) == i))
      end do
    end if
  ...
  subroutine set_permutation_c(perm) &
      & bind(c,name="ol_set_permutation_ppttj_ttxddxgg_1")
    use, intrinsic :: iso_c_binding, only: c_int
    implicit none
  ...
  subroutine get_masses(m_ex) &
      & bind(c,name="ol_f_get_masses_ppttj_ttxddxgg_1")
    ...
    do i = 1, 6
      m_ex(i) = m_ex_orig(external_perm_ppttj_ttxddxgg_1(i))
    end do
  end subroutine get_masses


  subroutine get_masses_c(m_ex) &
      & bind(c,name="ol_get_masses_ppttj_ttxddxgg_1")
    use KIND_TYPES, only: DREALKIND
  ...
  subroutine rambo(sqrt_s, p_rambo) &
      & bind(c,name="ol_f_rambo_ppttj_ttxddxgg_1")
    use KIND_TYPES, only: DREALKIND
  ...
  subroutine rambo_c(sqrt_s, p_rambo) &
      & bind(c,name="ol_rambo_ppttj_ttxddxgg_1")
    use KIND_TYPES, only: DREALKIND
    use, intrinsic :: iso_c_binding, only: c_double
    implicit none
  ...
  subroutine pol_init(pol) &
      & bind(c,name="ol_f_pol_init_ppttj_ttxddxgg_1")
    implicit none
  ...
  subroutine set_photons(photon_id) &
      & bind(c,name="ol_f_set_photons_ppttj_ttxddxgg_1")
    implicit none
    integer, intent(in) :: photon_id(6)
    photonid_ppttj_ttxddxgg_1 = photon_id
  end subroutine set_photons


end module ol_external_ppttj_ttxddxgg_1


module ol_settings_ppttj_ttxddxgg_1
  implicit none
  ...
  subroutine set_hel_mem_opt(set) &
      & bind(c,name="ol_hel_mem_opt_ppttj_ttxddxgg_1")
    implicit none
  ...
  subroutine set_loopcc(set,was) &
      & bind(c,name="ol_loopcc_ppttj_ttxddxgg_1")
    implicit none
    logical, intent(in) :: set
    logical :: was
    was = loopcc
...
end module ol_settings_ppttj_ttxddxgg_1

module colour_basis_ppttj_ttxddxgg_1
  implicit none
  ! tree colour basis
  ...
  pure subroutine tree_colbasis_dim(extcols, ncolb, ncoupl, maxpows, nhel) &
    & bind(c, name="ol_tree_colbasis_dim_ppttj_ttxddxgg_1")
    implicit none
  ...
  subroutine tree_colbasis(basis, powers) &
    & bind(c, name="ol_tree_colbasis_ppttj_ttxddxgg_1")
    implicit none
...
end module colour_basis_ppttj_ttxddxgg_1

! **********************************************************************
module ol_heltables_ppttj_ttxddxgg_1
! **********************************************************************
...
end module ol_heltables_ppttj_ttxddxgg_1
