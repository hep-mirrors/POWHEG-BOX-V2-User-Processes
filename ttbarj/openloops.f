...
! Register & Loopup processes
     ...
     &                         tree_colbasis_dim, tree_colourflow
          ...
          if ( register < 0 ) then
            print*, "[POWHEG-BOX+OpenLoops2] Process not found!"
...
! Physical parameters
        ...
        call set_parameter('alpha_qed_mz',ph_alphaem)
        call set_parameter("z_mass",  ph_Zmass)
!        if (ph_Hmass    /= 0) call set_parameter("z_width", ph_Zwidth)
        call set_parameter("w_mass",  ph_Wmass)
        call set_parameter("w_width", 0d0)
!        if (ph_Hmass    /= 0) call set_parameter("h_mass",  ph_Hmass)
!        if (ph_Hwidth   /= 0) call set_parameter("h_width", ph_Hwidth)
        call set_parameter("t_mass",  ph_topmass)
        call set_parameter("t_width", 0d0)
...
!     call set_parameter("use_cms",0)
      ...
      if (powheginput('#ol_hp_mode').ge.0) then
...
! Fortran77 Interfaces
      subroutine openloops_born(p,bflav,born,bornjk,bmunu)
        ...
        implicit none
        include 'nlegborn.h'
        double precision,intent(in) :: p(0:3,nlegborn)
        integer,intent(in) :: bflav(nlegborn)
        double precision,intent(out) :: born
        double precision,intent(out),optional :: bornjk(nlegborn,nlegborn)
        double precision,intent(out),optional :: bmunu(0:3,0:3,nlegborn)
        if (present(bornjk) .and. present(bmunu)) then
           call ol_born(p,bflav,born,bornjk,bmunu)
        else
           call ol_born(p,bflav,born)
        endif
      end subroutine openloops_born
      ...
      subroutine openloops_real(p,rflav,amp2real)
        ...
        implicit none
        include 'nlegborn.h'
        double precision, intent(in) ::p(0:3,nlegborn+1)
        integer, intent(in) :: rflav(nlegborn+1)
        double precision, intent(out) :: amp2real
      ...
      subroutine openloops_virtual(p,vflav,virtual)
        ...
        include 'nlegborn.h'
        double precision, intent(in) ::  p(0:3,nlegborn)
        integer, intent(in) :: vflav(nlegborn)
        double precision, intent(out) ::  virtual
      ...
      subroutine openloops_borncolour_ifl(p,bflav,color,ifl)
        ...
        double precision, intent(in) ::  p(0:3,nlegborn)
        integer, intent(in) :: bflav(nlegborn)
        integer, intent(out) ::  color(2,nlegborn), ifl
      ...
      subroutine openloops_realcolour_ifl(p,rflav,color,ifl)
        ...
        double precision, intent(in) ::  p(0:3,nlegborn+1)
        integer, intent(in) :: rflav(nlegborn+1)
        integer, intent(out) ::  color(2,nlegborn+1), ifl
      ...
      subroutine openloops_borncolour(p,bflav,color)
        ...
        double precision, intent(in) ::  p(0:3,nlegborn)
        integer, intent(in) :: bflav(nlegborn)
        integer, intent(out) ::  color(2,nlegborn)
        integer ifl
        call ol_colour(p,bflav,color,ifl)
      end subroutine openloops_borncolour
      ...
      subroutine openloops_realcolour(p,rflav,color)
        ...
        double precision, intent(in) ::  p(0:3,nlegborn+1)
        integer, intent(in) :: rflav(nlegborn+1)
        integer, intent(out) ::  color(2,nlegborn+1)
        integer ifl
        call ol_colour(p,rflav,color,ifl)
      end subroutine openloops_realcolour
