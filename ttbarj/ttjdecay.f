...
c     evaluation of OpenLoops decayed matrix element is required and that
...
c     the 'undecayed part' of the OpenLoops full matrix element, being b
...
c     the top and W widths to a non zero value in OpenLoops routines for
c     decayed matrix elements, otherwise the top and W propagators
c     present there may cause divergences (OpenLoops propagators do
...
c     OpenLoops routine for decayed processes all the widths in the
...
c     requiring explicitly OpenLoops to generate routines which do
c     not include these contributions. This is done by requiring
c     the approximation "ttj", which also restricts the diagrams to
c     only include double-resonant top-quark production.
...
c spin summmed and averaged amplitude squared for top decay
     ...
     &     -ph_bottommass**2*wvirt2-tvirt2**2+2*ph_bottommass**2*tvirt2
     &     -ph_bottommass**4)/wvirt2 *4*pi*alpha/(2*sin2w) /2
...
c set top and w width to zero to evaluate undecayed matrix elements
      call set_parameter('width(6)',0d0)
      call set_parameter('width(24)',0d0)
...
c add t and tbar decay products to ppss
      ebm=(tpvirt2+ph_bottommass**2-wpvirt2)/(2*sqrt(tpvirt2))
      ebp=(tmvirt2+ph_bottommass**2-wmvirt2)/(2*sqrt(tmvirt2))
...
c elements
      call set_parameter('width(6)',ph_topwidth)
      call set_parameter('width(24)',ph_Wwidth)
      call set_parameter('approx','ttj')
...
c
      ...
      implicit none
      include 'LesHouches.h'
      double precision p(0:3,nup),res
      integer flav(nup)
      call flavstdform(idup,nup,flav)
      call openloops_born(p,flav,res)
      end
      ...
      integer flav(nup)
      call flavstdform(idup,nup,flav)
      call openloops_real(p,flav,res)
      end
      ...
      implicit none
      include 'LesHouches.h'
      double precision p(0:3,nup+4),res
      integer i,flav(nup+4),flav_part(nup-2),flavour_mapping
      call flavstdform(idup,nup,flav(1))
      flav_part(1) = flav(1)
      flav_part(2) = flav(2)
      flav_part(3) = flav(5)
      do i=1,3
         if (flav_part(i)/=0) flav_part(i) = sign(1,flav_part(i))
      enddo
      flav(1)=flav_part(1)
      flav(2)=flav_part(2)
      flav(3)=5
      ...
      flav(8)=-12
      flav(9)=flav_part(3)
      call get_parameter("flavour_mapping",flavour_mapping)
      call set_parameter("flavour_mapping",0)
      call openloops_born(p,flav,res)
      call set_parameter("flavour_mapping",flavour_mapping)
      end
      ...
      include 'LesHouches.h'
      double precision p(0:3,nup+4),res
      integer i,flav(nup+4),flav_part(nup-2),flavour_mapping,flav_found
      call flavstdform(idup,nup,flav(1))
      flav_part(1) = flav(1)
      flav_part(2) = flav(2)
      flav_part(3) = flav(5)
      ...
      flav_found = 0
      do i=1,4
         if (flav_part(i)/=0) then
            if (flav_found==0) then
               flav_found = abs(flav_part(i))
            endif
            if (abs(flav_part(i))/=flav_found) then
               flav_part(i) = sign(2,flav_part(i))
            else
               flav_part(i) = sign(1,flav_part(i))
            endif
         endif
      enddo
      flav(1)=flav_part(1)
...
c
     ...
     $     ,ttmass,kb0,wpkl0,wmkl0,tten,cmen,cmen0,ran(5)
      ...
      save cfacbound
c
      bmass2=ph_bottommass**2
      tmass2=ph_topmass**2
      wmass2=ph_Wmass**2
c b energy in t rest frame in zero width limit
      eb0=(tmass2+bmass2-wmass2)/(2*ph_topmass)
c momentum of decay products in t rest frame
      kb0=decmom(ph_topmass,ph_Wmass,ph_bottommass)
c momentum of decay products in W
      wpkl0=decmom(ph_Wmass,0d0,0d0)
      wmkl0=decmom(ph_Wmass,0d0,0d0)
...
c this is from phase space in t->W+b
      cfac=cfac*(decmom(tpvirt,wpvirt,ph_bottommass)/tpvirt)
     &     /(kb0/ph_topmass)
c this is from phase space in tbar->W- + b
      cfac=cfac*(decmom(tmvirt,wmvirt,ph_bottommass)/tmvirt)
     &     /(kb0/ph_topmass)
c this is from phase space in W+ decay
      cfac=cfac*(decmom(wpvirt,0d0,0d0)/wpvirt)/(wpkl0/ph_Wmass)
c this is from phase space in W- decay
      cfac=cfac*(decmom(wmvirt,0d0,0d0)/wmvirt)/(wmkl0/ph_Wmass)
...
c vin and vout may be the same
      ...
      double precision betav,gamma,tmp(4,m)
     ...
     $        *beta(3)
         do idim=1,3
            tmp(idim,ipart)=vin(idim,ipart)+beta(idim)/betav*((gamma
     $           -1)*vdotb/betav +gamma*betav*vin(4,ipart))
         enddo
         tmp(4,ipart)=gamma*(vin(4,ipart)+vdotb)
      enddo
      vout=tmp
      end
