...
c$$$      if (icut.eq.0) then
c$$$         cut = ' nocut'
c$$$      elseif (icut.eq.1) then
c$$$         numplots=diag
c$$$         cut = ' cut_1'
c$$$      elseif (icut.eq.2) then
c$$$         goto 222
c$$$      else
c$$$         write(*,*) 'Error in init_hist, icut ',icut
c$$$         call exit(1)
c$$$      endif
...
c     top
      diag=diag+1
      binsize(diag) = 4d0
      call bookupeqbins('t_pt'//cut,binsize(diag),0d0,200d0)

      diag=diag+1
      binsize(diag) = 1.2d0
      call bookupeqbins('t_pt_zoom'//cut,binsize(diag),
     $     0d0,60d0)

      diag=diag+1
      binsize(diag) = 20d0
      call bookupeqbins('t_pt_tail'//cut,binsize(diag),
     $     0d0,1000d0)

      diag=diag+1
      binsize(diag) = 0.24d0
      call bookupeqbins('t_y'//cut,binsize(diag),-6d0,6d0)

      diag=diag+1
      binsize(diag) = 0.24d0
      call bookupeqbins('t_eta'//cut,binsize(diag),-6d0,6d0)

      diag=diag+1
      binsize(diag) = 0.8d0
      call bookupeqbins('t_minv'//cut,binsize(diag),
...
c     W
      diag=diag+1
      binsize(diag) = 4d0
      call bookupeqbins('W_pt'//cut,binsize(diag),0d0,200d0)

      diag=diag+1
      binsize(diag) = 1.2d0
      call bookupeqbins('W_pt_zoom'//cut,binsize(diag),
     $     0d0,60d0)

      diag=diag+1
      binsize(diag) = 20d0
      call bookupeqbins('W_pt_tail'//cut,binsize(diag),
     $     0d0,1000d0)

      diag=diag+1
      binsize(diag) = 0.24d0
      call bookupeqbins('W_y'//cut,binsize(diag),-6d0,6d0)

      diag=diag+1
      binsize(diag) = 0.24d0
      call bookupeqbins('W_eta'//cut,binsize(diag),-6d0,6d0)

      diag=diag+1
      binsize(diag) = 0.8d0
      call bookupeqbins('W_minv'//cut,binsize(diag),
     $     80d0-20d0,80d0+20d0)

c     hardest b
      diag=diag+1
      binsize(diag) = 4d0
      call bookupeqbins('b_pt'//cut,binsize(diag),0d0,200d0)

      diag=diag+1
      binsize(diag) = 1.2d0
      call bookupeqbins('b_pt_zoom'//cut,binsize(diag),
     $     0d0,60d0)

      diag=diag+1
      binsize(diag) = 20d0
      call bookupeqbins('b_pt_tail'//cut,binsize(diag),
     $     0d0,1000d0)

      diag=diag+1
      binsize(diag) = 0.24d0
      call bookupeqbins('b_y'//cut,binsize(diag),-6d0,6d0)

c     hardest bbar
      diag=diag+1
      binsize(diag) = 4d0
      call bookupeqbins('bbar_pt'//cut,binsize(diag),0d0,200d0)

      diag=diag+1
      binsize(diag) = 1.2d0
      call bookupeqbins('bbar_pt_zoom'//cut,binsize(diag),
     $     0d0,60d0)

      diag=diag+1
      binsize(diag) = 20d0
      call bookupeqbins('bbar_pt_tail'//cut,binsize(diag),
     $     0d0,1000d0)

      diag=diag+1
      binsize(diag) = 0.24d0
      call bookupeqbins('bbar_y'//cut,binsize(diag),-6d0,6d0)

c     J1
      diag=diag+1
      binsize(diag) = 4d0
      call bookupeqbins('j1_pt'//cut,binsize(diag),0d0,200d0)

      diag=diag+1
      binsize(diag) = 0.24d0
      call bookupeqbins('j1_eta'//cut,binsize(diag),-6d0,6d0)

c     J2
      diag=diag+1
      binsize(diag) = 4d0
      call bookupeqbins('j2_pt'//cut,binsize(diag),0d0,200d0)

      diag=diag+1
      binsize(diag) = 0.24d0
      call bookupeqbins('j2_eta'//cut,binsize(diag),-6d0,6d0)

c     pt(t,j1)
      diag=diag+1
      binsize(diag) = 4d0
      call bookupeqbins('tj1_pt'//cut,binsize(diag),0d0,200d0)

c     dphi t-j1
      diag=diag+1
      binsize(diag) = pi/50d0
      call bookupeqbins('phitj1'//cut,binsize(diag),0d0,pi)

c     hardest charged lepton
      diag=diag+1
      binsize(diag) = 4d0
      call bookupeqbins('lep_pt'//cut,binsize(diag),0d0,200d0)

      diag=diag+1
      binsize(diag) = 0.24d0
      call bookupeqbins('lep_eta'//cut,binsize(diag),-6d0,6d0)

c     hardest neutrino
      diag=diag+1
      binsize(diag) = 4d0
      call bookupeqbins('v_pt'//cut,binsize(diag),0d0,200d0)

      diag=diag+1
      binsize(diag) = 0.24d0
      call bookupeqbins('v_eta'//cut,binsize(diag),-6d0,6d0)

c     s-channel spin correlation
c     cth between charged lepton and right beam axis (-z direction)
      diag=diag+1
      binsize(diag) = 0.04d0
      call bookupeqbins('cth_l-rb'//cut,binsize(diag),-1d0,1d0)
...
c     blank all that needs be blanked
         nt=0
         nw=0
         nl=0
         nv=0
...
c            write(*,*) istup(ihep),idup(ihep),pup(1:4,ihep)
            ist=istup(ihep)
            id=idup(ihep)
            condition_b=(ist.eq.1).and.id.eq.5
            condition_bbar=(ist.eq.1).and.id.eq.-5
...
c     tracks entering jet algorithm
c     exclude leptons, gauge and Higgs bosons, but include gluons
            if (ist.eq.1.and.abs(id).le.5) then
...
c-----spin corr
         diag=diag+1
         if(nt.gt.0)
     $        call filld('cth_l-rb'//cut,cth,dsig)
