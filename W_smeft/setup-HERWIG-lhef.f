...
C---CALL HWUSTA TO MAKE ANY PARTICLE STABLE
      CALL HWUSTA('PI0     ')
      if (lprup(1).eq.10015)  CALL HWUSTA('TAU-    ')
      if (lprup(1).eq.9985)  CALL HWUSTA('TAU+    ')
      end
...
c      open(unit=99,file=pwgprefix(1:lprefix)//'POWHEG+HERWIG-output.top'
c     #     ,status='unknown')
...
c      if(abs(idwtup).eq.3) xwgtup=xwgtup*xsecup(3)
c      call analysis(lhrwgt_weights(3))
      call pwhgaccumup 
      end
