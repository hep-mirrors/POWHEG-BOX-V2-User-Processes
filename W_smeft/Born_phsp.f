     ...
     # z,zhigh,zlow
      real * 8 ne
      parameter (ne=2.71828182845904523536028747135d0)
      integer mu,k
      logical ini
      data ini/.true./
      save ini
...
c     set initial- and final-state masses for Born and real
         ...
         if(ph_Wmass2low.ge.ph_Wmass2) genflat=.true.
         if(ph_Wmass2high.le.ph_Wmass2) genflat=.true.
         if(genflat) then
            write(*,*) '*************************************'
            write(*,*) 
     $ "WARNING: BW importance sampling around W peak switched off"
            write(*,*) '*************************************'
         endif
         ini=.false.
      endif
...
c     d m^2 jacobian
         ...
         m2=ph_Wmass2low + (ph_Wmass2high-ph_Wmass2low)*(exp(xborn(1))
     $  -1d0) / (ne-1d0)
c     d m^2 jacobian
         xjac=xjac*(ph_Wmass2high-ph_Wmass2low)/(ne-1d0)*exp(xborn(1))
      endif
...
c minimal final state mass 
      ...
      if(pt.le.0) then
         ...
         m2=(kn_pborn(0,3)+kn_pborn(0,4))**2-(kn_pborn(1,3)
     $        +kn_pborn(1,4))**2-(kn_pborn(2,3)+kn_pborn(2,4))**2
