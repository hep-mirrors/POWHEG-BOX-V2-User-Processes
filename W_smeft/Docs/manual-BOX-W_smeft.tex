...
\title{The POWHEG BOX user manual:\\
  $\boldsymbol{W^\pm}$ production in the SM-EFT} \vfill
...
\abstract{This note documents the use of the package
  \tmtexttt{POWHEG BOX} for $W^\pm$ production processes, in the presence of SM-EFT operators.
 }
...
This program is an implementation of the charged-current Drell-Yan NLO cross sections
$pp\to W^\pm \to \ell^\pm \nu_\ell$, induced by SM-EFT operators. A detailed description of the
...
\tmtexttt{POWHEG-BOX/W\_smeft} subdirectory package is based on the original work of Ref.\ \cite{Alioli:2008gx}, to which we refer for more details.
...
parameters specific to $W^\pm$ implementation.
