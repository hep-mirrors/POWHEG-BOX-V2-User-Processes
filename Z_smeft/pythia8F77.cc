...
extern "C" {
  ...
  void pythia_init_() {
    ...
    if(cpy8pars_.hepmc > 0) {
      ptrHepMC = new HepMC::IO_GenEvent(std::string(cpwgprefix_.pwgprefix,cpwgprefix_.lprefix)+"POWHEG+PYTHIA8-"+std::string(pwhg_rnd_.rnd_cwhichseed)+".hepmc");
  ...
  void pythia_to_hepevt_(const int &nmxhep, int & nhep, int * isthep,
    ...
    if(nhep>nmxhep) {cout << "too many particles! " <<nhep <<" > "<<nmxhep<<endl;  exit(-1); }
