...
\title{The POWHEG BOX user manual:\\
  $\boldsymbol{Z/\gamma^*}$ production in the SM-EFT} \vfill
...
\abstract{This note documents the use of the package
  \tmtexttt{POWHEG BOX} for $Z/\gamma^*$ production processes, in the presence of SM-EFT operators.
 }
...
$pp\to Z/\gamma^* \to \ell^+ \ell^-$ induced by SM-EFT operators. A detailed description of the
...
\tmtexttt{POWHEG-BOX/Z\_smeft} subdirectory package 
is based on the original work of Ref.\ \cite{Alioli:2008gx}, to which we refer for more details.
...
\tmtexttt{\$ cd POWHEG-BOX-V2/Z$\_$smeft \\
\$ make pwhg\_main  \\
\$ make main-PYTHIA8-lhef  \\
}\\
Then do (for example) \\
\tmtexttt{\$
cd testrun-lhc-smeft-13TeV\\
\$ ../pwhg\_main}\\
At
the end of the run, the file \tmtexttt{pwgevents.lhe} will contain
100000 events for the process $Z/\gamma^* \to e^+ e^-$  in the Les Houches format. In
order to shower them with \PYTHIA{} do\\
\tmtexttt{\$ cd
testrun-lhc-smeft-13TeV \\
\$ ../main-PYTHIA8-lhef}
