c Invariant mass bins 
      ...
      parameter (nmtbins = 21)
      parameter (nmtbins2 = 9)
      real*8 mtbins(nmtbins + 1)
      real*8 mtbins2(nmtbins2 + 1)
      data mtbins/125d0,    150d0,  175d0, 200d0, 250d0, 300d0, 350d0,
     1            425d0, 525d0, 625d0, 750d0, 900d0, 1100d0, 1300d0, 
     2	          1600d0, 1900d0, 2300d0, 2700d0, 3200d0, 3800d0, 
     3		  4500d0, 5500d0/
      data mtbins2/125d0,    250d0,  400d0, 500d0, 700d0, 900d0, 
     1 		 1200d0, 1800d0, 3000d0, 6000d0/ 
      
      
      call inihists

      call bookup('mll',nmtbins,mtbins)
      call bookup('mll_nocut',nmtbins,mtbins)
      call bookup('mll2',nmtbins2,mtbins2)  
      call bookup('mll2_nocut',nmtbins2,mtbins2)  
      call bookupeqbins("a0",100d0,1000d0,2500d0)
      call bookupeqbins("a4",100d0,1000d0,2500d0)
      call bookupeqbins('V_pt',1d0,0d0,200d0)
...
c     electron cuts
	...
	pass_cuts = (ptl.gt.30d0).and.(ptnu.gt.30d0).and.
     .(abs(etal).lt.2.47d0) 
c     muon cuts      
	...
	pass_cuts = (ptl.gt.30d0).and.(ptnu.gt.30d0).and.
     .(abs(etal).lt.2.5d0) 
      ...
      if(pass_cuts) then 
	call filld('mll',m,dsig)
	call filld('mll2',m,dsig)
      endif
      
      call filld('mll_nocut',m,dsig)
      call filld('mll2_nocut',m,dsig)
      call filld('a0',m,aa(0)*dsig)
      call filld('a4',m,4*cs*dsig)
