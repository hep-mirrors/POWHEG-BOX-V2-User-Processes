module p22_gsbar_epnebbbarubar_matrix
...
contains
   ...
   pure function prefactor()
      ...
      use p5_usbar_epnebbbarg_model, only: NC
      use p5_usbar_epnebbbarg_kinematics, only: &
      & in_helicities, symmetry_factor
      use p5_usbar_epnebbbarg_color, only: incolors
      implicit none
   ...
   subroutine     initgolem(is_first)
      use p5_usbar_epnebbbarg_matrix, only: orig_initgolem => initgolem
      implicit none
   ...
   subroutine     exitgolem(is_last)
      use p5_usbar_epnebbbarg_matrix, only: orig_exitgolem => exitgolem
      implicit none
   ...
   subroutine     samplitude(vecs, scale2, amp, ok, h)
      use p5_usbar_epnebbbarg_matrix, only: orig_sub => samplitude
      implicit none
   ...
   function     samplitudel0(vecs, h) result(amp)
      use p5_usbar_epnebbbarg_matrix, only: orig_func => samplitudel0
      implicit none
      real(ki), dimension(7, 4), intent(in) :: vecs
   ...
   function     samplitudel1(vecs,scale2,ok,rat2,h) result(amp)
      use p5_usbar_epnebbbarg_matrix, only: orig_func => samplitudel1
      implicit none
      real(ki), dimension(7, 4), intent(in) :: vecs
   ...
   subroutine     ir_subtraction(vecs,scale2,amp)
      use p5_usbar_epnebbbarg_matrix, &
...
end module p22_gsbar_epnebbbarubar_matrix
