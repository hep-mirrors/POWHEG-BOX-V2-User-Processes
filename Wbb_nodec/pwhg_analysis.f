...
c$$$      if(nvl.ne.1.or.nlep.ne.1) then
c$$$         write(*,*) 'Problems with leptons from W decay'
c$$$c     write(*,*) 'PROGRAM ABORT'
c$$$         write(*,*) 'nvl= ',nvl, 'nlep= ',nlep
c$$$         call pwhg_exit(-1)
c$$$      endif
c$$$      do mu=1,4
c$$$         plep(mu,nlep)=phep(mu,ilep)
c$$$         pvl(mu,nvl)=phep(mu,ivl)
c$$$      enddo
c$$$      if (ilep*ivl.eq.0) then
c$$$         write(*,*) 
c$$$     $        'ERROR... have NOT found the electron/neutrino/Higgs'
c$$$         write(*,*) 'idhep'
c$$$         write(*,*) (idhep(ihep),ihep=1,nhep)
c$$$         call pwhg_exit(-1)
c$$$      endif
c$$$       if (nb*nbbar.eq.0) then
c$$$         if (((nb.eq.0).and.(nbbar.ne.0)).or.
c$$$     $        ((nb.ne.0).and.(nbbar.eq.0))) then
c$$$            write(*,*) 'SEVERE WARNING: ***** One b is missing ******' 
c$$$c     write(*,*) 'ihepb,ihepbbar ',ihepb,ihepbbar            
c$$$            return
c$$$         endif
c$$$      endif
c$$$c     change status of l vu 
