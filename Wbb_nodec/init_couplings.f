...
c     used only in LH event file
      rad_branching=powheginput('#Wbranching')
...
c     OLD WAY!!!
c      masswindow = 30
c      ph_Wmass2low=max(0d0,ph_Wmass-masswindow*ph_Wwidth)
c      ph_Wmass2low=ph_Wmass2low**2
c      ph_Wmass2high=(ph_Wmass+masswindow*ph_Wwidth)**2
c     NEW WAY
      ph_Wmass2low=powheginput("min_w_mass")**2
      ph_Wmass2high=powheginput("max_w_mass")**2

      if (ph_Wmass2low.lt.ph_lmass**2) then
         write(*,*) 'min_w_mass less than the minimun invariant mass of'
         write(*,*) 'the final-state leptonic system ',ph_lmass
         write(*,*) 'POWHEG aborts'
         call pwhg_exit(-1)
      endif        
