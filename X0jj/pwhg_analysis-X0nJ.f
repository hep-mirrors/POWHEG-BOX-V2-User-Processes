...
c      character * 1, parameter :: cnum(9) = ['1','2','3','4','5','6','7','8','9']
      integer maxjet
      parameter (maxjet=4)
      integer nptmin
c$$$      parameter (nptmin=3)
c$$$      character * 4 cptmin(nptmin)
c$$$      real * 8 ptminarr(nptmin)
c$$$      data cptmin/  '-020',  '-050',  '-100'/
c$$$      data ptminarr/   20d0,    50d0,   100d0/
      ...
      call bookupeqbins('sigmatot', 1d0,0d0,2d0)
...
c      if(rad_kinreg.ne.1) return
      ...
      if(mjets >= 2) then
         if( ktj(1) > 30d0 .and. ktj(2) > 30d0
     $        .and. abs(etaj(1)) < 4.5d0 .and. abs(etaj(2)) < 4.5d0)
     $        then
            call filld('sigmatot',1.5d0,dsig0)
         endif
      endif
