      ...
      save ini,fullphsp
      real * 8 powheginput
      real * 8 ptjet_born_min,djetmass_min
      real * 8 pt2jet_born_min, djetmass_square_min
      save pt2jet_born_min, djetmass_square_min
            
      real * 8 pt2jet, djetmass_square
      integer j
      real * 8 dotp
      external dotp
      common/cjac/jac
      ...
      if(ini) then
c     minimal set of CUTS applied at the Born level
c     We need to apply cuts otherwise the virtual are evaluated at
c     phase-space points that give very unstable results, and the integration 
c     fails
         if (flg_minlo) then
            ptjet_born_min = 0.5d0
            djetmass_min = 0.5d0            
         else
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
c      RIPRISTINARE i valori commentati
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC                        
            ptjet_born_min = 0.5d0
            djetmass_min = 0.5d0            
c            ptjet_born_min = 10d0
c            djetmass_min =10d0
         endif

         pt2jet_born_min = ptjet_born_min**2
         djetmass_square_min = djetmass_min**2
...
c     cuts
      do j=4,5
         pt2jet = kn_cmpborn(1,j)**2+kn_cmpborn(2,j)**2
         if (pt2jet.lt.pt2jet_born_min) jac = 0d0
      enddo
      djetmass_square = 2*dotp(kn_cmpborn(0,4),kn_cmpborn(0,5))
      if (djetmass_square < djetmass_square_min) jac = 0d0
c     end cut     
...
c with minlo active, this should be switched off
c      if(ptmin.gt.0 .and. .not. flg_minlo) then
      if(ptmin.gt.0) then
     ...
     2        /(kn_cmpborn(0,4)**2+kn_cmpborn(0,5)**2)
      ...
      external powheginput
      real * 8 hmass,hwidth
      save ini,runningscales,hmass,hwidth
      ...
      if (ini) then
         ...
         if (runningscales.and.flg_minlo) then
            write(*,*) 
