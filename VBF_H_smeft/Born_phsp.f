...
c     squared momentum of the weak boson connected with the upper line
	 q2_up = abs( -2*dotp(kn_pborn(0,1),kn_pborn(0,4)) )
c     squared momentum of the weak boson connected with the lower line
	 q2_down = abs( -2*dotp(kn_pborn(0,2),kn_pborn(0,5)) )
         fact = q2_up/(q2_up + pt2supp) * q2_down/(q2_down + pt2supp)
c      if (pt2.gt.10) then
c         fact = 1d0
c      else
c         fact = 0d0
