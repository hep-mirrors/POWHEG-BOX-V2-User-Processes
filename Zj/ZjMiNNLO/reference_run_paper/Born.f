...
c     Z->bbar
      ...
      if(firsttime) then
         write(*,*) 'Gamma(Z->ll) [computed]= ', 1d0/(246.22**2*dsqrt(2d0))*ph_zmass**3*4d0/(24d0*dsqrt(2d0)*pi)*(v_l**2+a_l**2)
         write(*,*) 'Gamma_tot(Z) [input]= ',ph_Zwidth
         write(*,*) 'BR(Z->ll)= ',1d0/(246.22**2*dsqrt(2d0))*ph_zmass**3*4d0/(24d0*dsqrt(2d0)*pi)*(v_l**2+a_l**2)/ph_Zwidth
         firsttime = .false.
      endif
