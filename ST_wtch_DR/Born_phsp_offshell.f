...
c     from POWHEG-st_dec code
      ...
      integer ixx_tau,ixx_ycm,ixx_cth1,ixx_mtop,ixx_mw
      parameter(
     #ixx_tau  =1,
     #ixx_ycm  =2,
     #ixx_cth1 =3,
...
c     other local functions
      real *8 virt2,gam,mass,r,tvirt2,wvirt2,rr
      virt2(mass,gam,r)=gam*mass*tan(pi/2d0*(1d0-2d0*r))+mass**2
...
c     !: May2020: generate full offshellness. First check if ndim is
c     correct (we need 2 dimensions more in the unit hypercube)

      if(ndiminteg.ne.5) then
         write(*,*) "================================================"
         write(*,*) "================================================"
         write(*,*) "This code is intended only for checks at LO."
         write(*,*) "To run it, increase ndiminteg by +2 in nlegborn.h"
         write(*,*) "================================================"
         write(*,*) "THIS CODE MAKES SENSE ONLY IF YOU RUN IT WITH"
         write(*,*) "bornonly=1 and LOevents=1"
         write(*,*) "================================================"
         write(*,*) "================================================"
         stop
      endif

c     rr=1d-2 + xx(ixx_mtop) * (0.99d0 - 1d-2)
c     m2top=virt2(topmass_pow,topwidth_pow, rr)
c     if(m2top.lt.wmass_pow**2) jacb=0d0 m2top=topmass_pow**2

      m2top = 165d0**2 + xx(ixx_mtop) * (185d0**2 - 165d0**2)
      jacb=jacb*(185d0**2 - 165d0**2)
         
c     rr=1d-2 + xx(ixx_mw) * (0.99d0 - 1d-2)
c     m2w=virt2(wmass_pow,wwidth_pow,        rr)
c     if(m2w.lt.50d0**2) jacb=0d0
      
      m2w = 50d0**2 + xx(ixx_mw) * (150d0**2 - 50d0**2)
      jacb=jacb*(150d0**2 - 50d0**2)

c      print*, "PS: ",jacb,sqrt(m2w),sqrt(m2top),topmass_pow,topwidth_pow,wmass_pow,wwidth_pow,xx(4),xx(5)
