...
A better treatment of offshellness effects was added in May 2020 (from
rev r3409). It's explained in sec. 4.
...
\subsection*{A new option to simulate offshellness effects}
From revision r3409 (May 2020) the procedure used to simulate
offshellness effects of the $W$ bosons was improved ({\bf so far only
  for the DR case}), to take into account that the invariant mass
distribution of the primary $W$ boson from the hard scattering should
deviate slightly from an exact Breit-Wigner shape, due to the fact
that the structure of the $2\to 2$ (and $2\to 3$) amplitudes contains
different propagators for the internal lines, that are not constant
when the virtuality of the $W$ boson is fixed to a given value. In the
NLO computation of the hard scattering, nothing was changed (all is
computed with onshell external final states), therefore the total
cross section remains unchanged. However, the aforementioned effects
are now taken into account when generating full events with decays.

To activate this improved treatment, a new (optional) flag was introduced~\\~\\
%
\tmtexttt{
\#  fulloffshell 1
}
%
~\\~\\~
When the flag is activated, if needed the user can also select in
which interval the virtuality of the primary $W$ boson ($Q_W$) has to
be generated:~\\~\\
%
\tmtexttt{
\#  numwidth\_W 15\\
...
~\\~\\~
%
The first token set a constraint on the lower and upper bounds of the
virtuality of the $W$ boson ($Q_W\in [m_W - \mbox{\tmtexttt numwidth\_W}\ \Gamma_W, m_W + \mbox{\tmtexttt numwidth\_W}\ \Gamma_W]$).
%
The second token allows to put an additional lower bound on the same
quantity (typically in experimental analysis one is interested to the
tail at large values of $Q_W$).
