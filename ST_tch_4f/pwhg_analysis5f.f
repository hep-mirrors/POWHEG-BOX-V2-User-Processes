...
c      spin-correlation (theta asymmetry)
      cth=-1d0 !fake value, just to fill the plot somehow
c$$$      if(specjet.gt.0.and.nt.gt.0) then
c$$$         pspecj(0)=pjet(4,specjet)
c$$$         do mu=1,3
c$$$            beta(mu)=-p_top(mu,1)/p_top(0,1)
c$$$            pspecj(mu)=pjet(mu,specjet)
c$$$         enddo
c$$$         call boost_pow(beta,pspecj,ptemp)
c$$$         call boost_pow(beta,p_l(0,1),ptemp2)
c$$$         cth=(ptemp(1)*ptemp2(1)+ptemp(2)*ptemp2(2)+
c$$$     #ptemp(3)*ptemp2(3))
c$$$         cth=cth/sqrt(ptemp(1)**2+ptemp(2)**2+ptemp(3)**2)
c$$$         cth=cth/sqrt(ptemp2(1)**2+ptemp2(2)**2+ptemp2(3)**2)
c$$$      endif
