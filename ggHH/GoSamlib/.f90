program test
   use p1_dg_hhd_config, only: ki, logfile, nlo_prefactors
   use p1_dg_hhd_kinematics, only: dotproduct, boost_to_cms
   use p1_dg_hhd_model, only: parse
   use p1_dg_hhd_matrix, only: samplitude, &
     & initgolem, exitgolem, ir_subtraction
   use p1_dg_hhd_color, only: numcs, CA
   use p1_dg_hhd_rambo, only: ramb
...
subroutine  print_parameters(scale2)
   use p1_dg_hhd_config, only: renormalisation, &
