module     p1_dg_hhd_matrix
   use p1_dg_hhd_util, only: square
   ...
   use p1_dg_hhd_kinematics, only: &
       in_helicities, symmetry_factor, num_legs, &
       lo_qcd_couplings, corrections_are_qcd, num_light_quarks, num_gluons
   use p1_dg_hhd_model, only: Nf, NC, sqrt2, init_functions
   use p1_dg_hhd_color, only: TR, CA, CF, numcs, &
     & incolors, init_color
   use p1_dg_hhd_amplitudeh0, only: samplitudeh0l1 => samplitude, &
        &   finite_renormalisation0 => finite_renormalisation
   use p1_dg_hhd_amplitudeh1, only: samplitudeh1l1 => samplitude, &
        &   finite_renormalisation1 => finite_renormalisation
   use p1_dg_hhd_amplitudeh2, only: samplitudeh2l1 => samplitude, &
        &   finite_renormalisation2 => finite_renormalisation
   use p1_dg_hhd_amplitudeh3, only: samplitudeh3l1 => samplitude, &
        &   finite_renormalisation3 => finite_renormalisation
   use p1_dg_hhd_dipoles, only: insertion_operator, insertion_operator_qed
...
contains
   ...
   subroutine     samplitude(vecs, scale2, amp, prec, ok, h)
      ...
      if(PSP_check) then
         ...
         if(spprec1.lt.PSP_chk_li1.and.spprec1.ge.PSP_chk_li2) then
            call increasecnt('qq->HHg: do rotation with Ninja')
            icheck=2 ! ROTATION
         end if
         ...
         if(icheck.eq.2) then
            ...
            if(fpprec1.lt.PSP_chk_li3) then                                ! RESCUE
               icheck=3
               call increasecnt('qq->HHg: rotation unsuccesfull, switch to golem95')
            endif
         endif
         ...
         if(icheck.eq.3.and.PSP_rescue) then
            icheck=1
            call increasecnt('qq->HHg: switch to golem95')
   ...
   function     samplitudel0(vecs, h) result(amp)
      use p1_dg_hhd_config, only: logfile
      use p1_dg_hhd_kinematics, only: init_event
      implicit none
      real(ki), dimension(5, 4), intent(in) :: vecs
   ...
   pure function square_0l_0l_sc(color_vector1, color_vector2) result(amp)
      use p1_dg_hhd_color, only: cmat => CC
      implicit none
   ...
   pure function square_2_1_sc(color_vector1, color_vector2) result(amp)
      use p1_dg_hhd_color, only: cmat => T1T2
   ...
   pure function square_2_5_sc(color_vector1, color_vector2) result(amp)
      use p1_dg_hhd_color, only: cmat => T2T5
