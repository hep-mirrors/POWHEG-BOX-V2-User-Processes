module pb_gg_hh_amplitudeh1_qp
   use pb_gg_hh_config_qp, only: ki, &
       & reduction_interoperation
   use pb_gg_hh_color_qp, only: numcs
   use pb_gg_hh_groups_qp
   use golem_precision_golem_qp, only: ki_gol => ki
   use pb_gg_hh_golem95h1_qp
   use ninjago_qp_module, only: ki_nin
   use pb_gg_hh_ninjah1_qp
...
!---#[ function finite_renormalisation:
   function     finite_renormalisation(scale2) result(amp)
      use pb_gg_hh_util_qp, only: square
      use pb_gg_hh_color_qp, only: CF, CA
      use pb_gg_hh_kinematics_qp, only: &
   ...
   function     samplitude(scale2,ok,rational2,the_col0,opt_perm)
      use pb_gg_hh_config_qp, only: include_eps_terms, include_eps2_terms, &
      & logfile, debug_nlo_diagrams
      use pb_gg_hh_globalsl1_qp, only: col0,perm, use_perm, epspow
      use pb_gg_hh_globalsh1_qp, &
     & only: init_lo, rat2
      use pb_gg_hh_abbrevd10h1_qp, only: init_abbrevd10 => init_abbrev
      use pb_gg_hh_abbrevd4h1_qp, only: init_abbrevd4 => init_abbrev
      use pb_gg_hh_abbrevd8h1_qp, only: init_abbrevd8 => init_abbrev
      use pb_gg_hh_abbrevd6h1_qp, only: init_abbrevd6 => init_abbrev
      use pb_gg_hh_groups_qp
      implicit none
      real(ki), intent(in) :: scale2
...
subroutine     evaluate_group0(scale2,samplitude,ok)
   ...
   use pb_gg_hh_golem95h1_qp, only: reconstruct_golem95 => reconstruct_group
   use pb_gg_hh_groups_qp, only: contract_golem95
   use pb_gg_hh_ninjah1_qp, only: ninja_reduce => ninja_reduce_group0
   implicit none
   real(ki), intent(in) :: scale2
   logical, intent(out) :: ok
...
subroutine     evaluate_group1(scale2,samplitude,ok)
   ...
   use pb_gg_hh_golem95h1_qp, only: reconstruct_golem95 => reconstruct_group
   use pb_gg_hh_groups_qp, only: contract_golem95
   use pb_gg_hh_ninjah1_qp, only: ninja_reduce => ninja_reduce_group1
   implicit none
   real(ki), intent(in) :: scale2
   logical, intent(out) :: ok
   complex(ki), dimension(-2:0), intent(out) :: samplitude
...
subroutine     evaluate_group2(scale2,samplitude,ok)
   ...
   use pb_gg_hh_golem95h1_qp, only: reconstruct_golem95 => reconstruct_group
   use pb_gg_hh_groups_qp, only: contract_golem95
   use pb_gg_hh_ninjah1_qp, only: ninja_reduce => ninja_reduce_group2
   implicit none
   real(ki), intent(in) :: scale2
   logical, intent(out) :: ok
   complex(ki), dimension(-2:0), intent(out) :: samplitude
...
!---#] subroutine evaluate_group2:
end module pb_gg_hh_amplitudeh1_qp
