...
// This is exported in the ninjavholo_qp_module defined in
// ninjavholo_qp.F90.  They may also be called directly from fortran-77
// when using either gfortran or ifort.  (note: Since it uses the
// global instance ninja_qp::avh_olo, it is not thread-safe).
...
extern "C" {
  ...
  void ninjavholo_qp_set_ir_threshold_(const Real & threshold);

  void ninjavholo_qp_clear_cache_();
  void ninjavholo_qp_free_cache_();

  void ninjavholo_qp_init_(const Real & mur2);

  void ninjavholo_qp_get_mi4_rm_(Complex rslt[3],
  ...
  void ninjavholo_qp_get_mi4_cm_(Complex rslt[3],
                              const Real & s21, const Real & s32,
                              const Real & s43, const Real & s14,
                              const Real & s31, const Real & s42,
  ...
  void ninjavholo_qp_get_mi3_rm_(Complex rslt[3],
                              const Real & s21, const Real & s32,
                              const Real & s13,
                              const Real & m1, const Real & m2,
                              const Real & m3);
  void ninjavholo_qp_get_mi3_cm_(Complex rslt[3],
                              const Real & s21, const Real & s32,
                              const Real & s13,
                              const Complex & m1, const Complex & m2,
                              const Complex & m3);

  void ninjavholo_qp_get_mi2_rank2_rm_(Complex b11[3],
                                    Complex b1[3], Complex b0[3],
                                    const Real & s21, const Real & m1,
                                    const Real & m2);
  void ninjavholo_qp_get_mi2_rank2_cm_(Complex b11[3],
                                    Complex b1[3], Complex b0[3],
                                    const Real & s21,
                                    const Complex & m1,
                                    const Complex & m2);

  void ninjavholo_qp_get_mi1_rm_(Complex rslt[3], const Real & m0);
  void ninjavholo_qp_get_mi1_cm_(Complex rslt[3], const Complex & m0);
