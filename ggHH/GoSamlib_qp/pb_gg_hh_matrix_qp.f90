module pb_gg_hh_matrix_qp
   ...
   use pb_gg_hh_model_qp, only: Nf, NC, sqrt2, init_functions
   use pb_gg_hh_color_qp, only: TR, CA, CF, numcs, &
     & incolors, init_color
   use pb_gg_hh_amplitudeh0_qp, only: samplitudeh0l1 => samplitude, &
        &   finite_renormalisation0 => finite_renormalisation
   use pb_gg_hh_amplitudeh1_qp, only: samplitudeh1l1 => samplitude, &
        &   finite_renormalisation1 => finite_renormalisation
   use pb_gg_hh_amplitudeh3_qp, only: samplitudeh3l1 => samplitude, &
        &   finite_renormalisation3 => finite_renormalisation
   use pb_gg_hh_dipoles_qp, only: insertion_operator, insertion_operator_qed
...
contains
   ...
   subroutine     samplitudel01(vecs, scale2, amp, rat2, ok, h)
      ...
      use pb_gg_hh_dipoles_qp, only: pi
      implicit none
      real(ki), dimension(4, 4), intent(in) :: vecs
      real(ki), intent(in) :: scale2
   ...
   function     samplitudel0(vecs, h) result(amp)
      use pb_gg_hh_config_qp, only: logfile
      use pb_gg_hh_kinematics_qp, only: init_event
      implicit none
      real(ki), dimension(4, 4), intent(in) :: vecs
   ...
   pure subroutine color_correlated_lo(color_vector,res)
      use pb_gg_hh_color_qp, only: T1T1, &
      & T1T2, &
      & T2T2
      implicit none
   ...
   pure function square_0l_0l_sc(color_vector1, color_vector2) result(amp)
      use pb_gg_hh_color_qp, only: cmat => CC
      implicit none
   ...
   pure function square_1_2_sc(color_vector1, color_vector2) result(amp)
      use pb_gg_hh_color_qp, only: cmat => T1T2
