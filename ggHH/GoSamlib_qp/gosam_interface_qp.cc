...
namespace {
  ...
  public:
    ...
    Complex evaluate(const ninja_qp::ComplexMomentum & q,
    ...
    void muExpansion(const ninja_qp::ComplexMomentum Vort[],
    ...
    void t3Expansion(const ninja_qp::ComplexMomentum & a,
                     const ninja_qp::ComplexMomentum & e3,
                     const ninja_qp::ComplexMomentum & e4,
                     const ninja_qp::Complex & param,
                     int mindeg, int cut, const ninja_qp::PartitionInt part[],
                     ninja_qp::Complex c[])
    {
      // Format cut-number (not used by gosam)
      // int ncut = 0;
    ...
    void t2Expansion(const ninja_qp::ComplexMomentum & a0,
  ...
  std::ofstream ninjago_qp_out;

  // other flags
  bool ninjago_qp_first = true;
  inline void ninjago_qp_init_impl()
  {
    if (ninjago_qp_first == true) {
      setDefaultFloatingPointThreshold(0);
      ninjago_qp_first = false;
    }
  }
...
extern "C" {  
  ...
  void ninjago_qp_init()
  {
    ninjago_qp_init_impl();
  }

  void ninjago_qp_diag_rm(gosam_interface::Numerator numerator,
                       gosam_interface::Numerator_t3 numerator_t3,
                       gosam_interface::Numerator_t2 numerator_t2,
                       gosam_interface::Numerator_d numerator_d,
  ...
  void ninjago_qp_diag_cm(gosam_interface::Numerator numerator,
                       gosam_interface::Numerator_t3 numerator_t3,
                       gosam_interface::Numerator_t2 numerator_t2,
                       gosam_interface::Numerator_d numerator_d,
                       const int & nprops_group,
      ...
      for (int i =0; i<nprops; ++i) {
        ...
        for (int j =i+1; j<nprops; ++j) {
          ...
          s_mat(i,j) = g_mat(im,jm)
            + ninja_qp::real(msq[im]) + ninja_qp::real(msq[jm]);
          s_mat(j,i) = s_mat(i,j);
        }
      }
    }

    // create a ninja_qp::amplitude object and pass the parameters
  ...
  void ninjago_qp_diag_nm(gosam_interface::Numerator numerator,
                       gosam_interface::Numerator_t3 numerator_t3,
                       gosam_interface::Numerator_t2 numerator_t2,
                       gosam_interface::Numerator_d numerator_d,
                       const int & nprops_group,
  ...
  void ninjago_qp_clear_integral_cache()
  {
    ...
    if (ninja_qp::getIntegralLibrary() == (& ninja_qp::avh_olo))
      avh_olo.clearIntegralCache();
#endif
#ifdef NINJA_USE_LOOPTOOLS
    if (ninja_qp::getIntegralLibrary() == (& ninja_qp::loop_tools))
  ...
  {
    setVerbosity(val);
    if (val && !ninjago_qp_out.is_open()) {
      ninjago_qp_out.open("ninja_gosam_qp.out", std::ofstream::app);
      if (ninjago_qp_out.fail()) {
        std::cerr << "ERROR IN NINJA: "
                  << "ninja_gosam_qp.out could not be opened for appending."
                  << std::endl;
        std::terminate();
      }
      using namespace std;
    ...
    setOutputStream(ninjago_qp_out);
  }
  ...
  void ninjago_qp_set_test_tolerance(const Real & val)
  {
    ninja_qp::setTestTolerance(val);
  }

  void ninjago_qp_set_output_precision(int val)
  {
    ninjago_qp_out.precision(val);
  }
...
#ifdef NINJA_USE_ONELOOP
    case 1:
      ninja_qp::setDefaultIntegralLibrary(ninja_qp::avh_olo);
      break;
#endif
#ifdef NINJA_USE_LOOPTOOLS
    case 2:
      ninja_qp::setDefaultIntegralLibrary(ninja_qp::loop_tools);
      break;
#endif
    default:
      std::cerr << "ERROR IN NINJA: "
                << "The flag specified in ninjago_qp_set_integral_library"
  ...
  void ninjago_qp_fp_check_default_threshold()
  {
    ninjago_qp_init_impl();
    ninja_qp::setDefaultFloatingPointThreshold(REAL_MIN);
  }

  void ninjago_qp_fp_check_threshold(const Real & thr)
  {
    ninjago_qp_init_impl();
    ninja_qp::setDefaultFloatingPointThreshold(thr);
  }
