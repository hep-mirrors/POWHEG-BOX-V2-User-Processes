program test
   use p1_dg_hhd_config_qp, only: ki, logfile, nlo_prefactors
   use p1_dg_hhd_kinematics_qp, only: dotproduct, boost_to_cms
   use p1_dg_hhd_model_qp, only: parse
   use p1_dg_hhd_matrix_qp, only: samplitude, &
     & initgolem, exitgolem, ir_subtraction
   use p1_dg_hhd_color_qp, only: numcs, CA
   use p1_dg_hhd_rambo_qp, only: ramb
...
subroutine  print_parameters(scale2)
   use p1_dg_hhd_config_qp, only: renormalisation, &
