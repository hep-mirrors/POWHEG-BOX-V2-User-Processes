module p1_dg_hhd_matrix_qp
   ...
   use p1_dg_hhd_model_qp, only: Nf, NC, sqrt2, init_functions
   use p1_dg_hhd_color_qp, only: TR, CA, CF, numcs, &
     & incolors, init_color
   use p1_dg_hhd_amplitudeh0_qp, only: samplitudeh0l1 => samplitude, &
        &   finite_renormalisation0 => finite_renormalisation
   use p1_dg_hhd_amplitudeh1_qp, only: samplitudeh1l1 => samplitude, &
        &   finite_renormalisation1 => finite_renormalisation
   use p1_dg_hhd_amplitudeh2_qp, only: samplitudeh2l1 => samplitude, &
        &   finite_renormalisation2 => finite_renormalisation
   use p1_dg_hhd_amplitudeh3_qp, only: samplitudeh3l1 => samplitude, &
        &   finite_renormalisation3 => finite_renormalisation
   use p1_dg_hhd_dipoles_qp, only: insertion_operator, insertion_operator_qed
...
contains
   ...
   subroutine     samplitude(vecs, scale2, amp, prec, ok, h)
      ...
      if(PSP_check) then
         ...
         if(spprec1.lt.PSP_chk_li1.and.spprec1.ge.PSP_chk_li2) then
            call increasecnt('qq->HHg qp: do rotation with Ninja')
            icheck=2 ! ROTATION
         end if
         if(spprec1.lt.PSP_chk_li2) then                                       ! RESCUE
            call increasecnt('qq->HHg qp: switch to golem95')
            icheck=3
         ...
         if(icheck.eq.2) then
            ...
            if(fpprec1.lt.PSP_chk_li3) then                                ! RESCUE
               icheck=3
               call increasecnt('qq->HHg qp: rotation unsuccesfull, switch to golem95')
            endif
         endif
   ...
   subroutine     samplitudel01(vecs, scale2, amp, rat2, ok, h)
      ...
      use p1_dg_hhd_dipoles_qp, only: pi
      implicit none
      real(ki), dimension(5, 4), intent(in) :: vecs
      real(ki), intent(in) :: scale2
   ...
   function     samplitudel0(vecs, h) result(amp)
      use p1_dg_hhd_config_qp, only: logfile
      use p1_dg_hhd_kinematics_qp, only: init_event
      implicit none
      real(ki), dimension(5, 4), intent(in) :: vecs
   ...
   pure subroutine color_correlated_lo(color_vector,res)
      use p1_dg_hhd_color_qp, only: T1T1, &
      & T1T2, &
      & T1T5, &
      & T2T2, &
      & T2T5, &
   ...
   pure function square_0l_0l_sc(color_vector1, color_vector2) result(amp)
      use p1_dg_hhd_color_qp, only: cmat => CC
      implicit none
   ...
   pure function square_2_1_sc(color_vector1, color_vector2) result(amp)
      use p1_dg_hhd_color_qp, only: cmat => T1T2
   ...
   pure function square_2_5_sc(color_vector1, color_vector2) result(amp)
      use p1_dg_hhd_color_qp, only: cmat => T2T5
...
end module p1_dg_hhd_matrix_qp
