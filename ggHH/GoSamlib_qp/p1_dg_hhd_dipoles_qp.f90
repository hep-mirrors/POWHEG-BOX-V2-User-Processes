module p1_dg_hhd_dipoles_qp
   use p1_dg_hhd_config_qp, only: ki
   use p1_dg_hhd_color_qp, only: gammaF, gammaA, &
       & CF, CA, numcs, KF, KA, &
       & T1T2, &
       & T1T5, &
       & T2T5
   use p1_dg_hhd_kinematics_qp, only: num_legs, dotproduct, &
       lambda
   implicit none
...
contains
   ...
   function I_ff_qed(mu_sq, vec, I, J)
      ...
      if (flag) then
              ...
              term = V_SING_QED(s_jk, real(mH, ki), real(0, ki))
   ...
   function I_if_qed(mu_sq, vec, I, J)
      ...
      if (flag) then
              ncs=1.0_ki
              term = V_SING_QED(s_ja, 0.0_ki, 0.0_ki)
              term=term*ncs*(0.0_ki)*(1.0_ki/9.0_ki)
              term(1) = term(1) + log_s * term(2)
   ...
   function I_ii_qed(mu_sq, vec, I, J)
      ...
      if (flag) then
              ...
              term(1) = ncs*(-1.0_ki)*(0.0_ki)/9.0_ki
              term(2) = ncs*(-1.0_ki)*(0.0_ki)/9.0_ki
...
end module p1_dg_hhd_dipoles_qp
