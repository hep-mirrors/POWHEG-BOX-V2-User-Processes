...
namespace ninja_qp {
  ...
  // ninja_qp::Amplitude is the main class of the ninja library.  The
  ...
  public:
    ...
    Amplitude & reset()
    {
      result[0] = result[1] = result[2] = ninja_qp::Complex();
      cut_constr = ninja_qp::Complex();
      return *this;
    }
  ...
  private:
      ...
      if (return_val & UNSTABLE_KINEMATICS) {
        if (Options::verb) {
          (*Options::out) << std::endl
                          << "ninja_qp::Amplitude is returning "
    ...
    // data
    ninja_qp::details::Array3D<Complex> result;
    SMatrix s_mat;
