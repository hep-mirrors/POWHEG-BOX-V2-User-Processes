...
namespace ninja_qp {
  ...
  private:
    ninja_qp::details::Array4D<Real> data;
  ...
  private:
    ninja_qp::details::Array4D<Complex> data;
  ...
  {
    return RealMomentum(ninja_qp::real(p[0]), ninja_qp::real(p[1]),
                        ninja_qp::real(p[2]), ninja_qp::real(p[3]));
  }
  
  // Immaginary part of a complex momentum
  inline RealMomentum imag(const ComplexMomentum & p)
  {
    return RealMomentum(ninja_qp::imag(p[0]), ninja_qp::imag(p[1]),
                        ninja_qp::imag(p[2]), ninja_qp::imag(p[3]));
  }
  
  // Complex conjugate of a complex momentum
  inline ComplexMomentum conj(const ComplexMomentum & p)
  {
    return ComplexMomentum(ninja_qp::conj(p[0]), ninja_qp::conj(p[1]),
                           ninja_qp::conj(p[2]), ninja_qp::conj(p[3]));
  }
