...
!---#[ function finite_renormalisation:
   ...
   function     samplitude(scale2,ok,rational2,the_col0,opt_perm)
      use p1_dg_hhd_config_qp, only: include_eps_terms, include_eps2_terms, &
      & logfile, debug_nlo_diagrams
      use p1_dg_hhd_globalsl1_qp, only: col0,perm, use_perm, epspow
      use p1_dg_hhd_globalsh0_qp, &
     & only: init_lo, rat2
      use p1_dg_hhd_abbrevd4h0_qp, only: init_abbrevd4 => init_abbrev
      use p1_dg_hhd_abbrevd1h0_qp, only: init_abbrevd1 => init_abbrev
      use p1_dg_hhd_abbrevd6h0_qp, only: init_abbrevd6 => init_abbrev
      use p1_dg_hhd_abbrevd8h0_qp, only: init_abbrevd8 => init_abbrev
      use p1_dg_hhd_groups_qp
      implicit none
      real(ki), intent(in) :: scale2
...
subroutine     evaluate_group0(scale2,samplitude,ok)
   ...
   use p1_dg_hhd_golem95h0_qp, only: reconstruct_golem95 => reconstruct_group
   use p1_dg_hhd_groups_qp, only: contract_golem95
   use p1_dg_hhd_ninjah0_qp, only: ninja_reduce => ninja_reduce_group0
   implicit none
   real(ki), intent(in) :: scale2
   logical, intent(out) :: ok
...
subroutine     evaluate_group1(scale2,samplitude,ok)
   ...
   use p1_dg_hhd_golem95h0_qp, only: reconstruct_golem95 => reconstruct_group
   use p1_dg_hhd_groups_qp, only: contract_golem95
   use p1_dg_hhd_ninjah0_qp, only: ninja_reduce => ninja_reduce_group1
   implicit none
   real(ki), intent(in) :: scale2
   logical, intent(out) :: ok
   complex(ki), dimension(-2:0), intent(out) :: samplitude
...
subroutine     evaluate_group2(scale2,samplitude,ok)
   ...
   use p1_dg_hhd_golem95h0_qp, only: reconstruct_golem95 => reconstruct_group
   use p1_dg_hhd_groups_qp, only: contract_golem95
   use p1_dg_hhd_ninjah0_qp, only: ninja_reduce => ninja_reduce_group2
   implicit none
   real(ki), intent(in) :: scale2
   logical, intent(out) :: ok
   complex(ki), dimension(-2:0), intent(out) :: samplitude
...
!---#] subroutine evaluate_group2:
end module p1_dg_hhd_amplitudeh0_qp
