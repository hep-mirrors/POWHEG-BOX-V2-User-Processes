module p0_gg_hhg_matrix_qp
   ...
   use p0_gg_hhg_model_qp, only: Nf, NC, sqrt2, init_functions
   use p0_gg_hhg_color_qp, only: TR, CA, CF, numcs, &
     & incolors, init_color
   use p0_gg_hhg_amplitudeh0_qp, only: samplitudeh0l1 => samplitude, &
        &   finite_renormalisation0 => finite_renormalisation
   use p0_gg_hhg_amplitudeh1_qp, only: samplitudeh1l1 => samplitude, &
        &   finite_renormalisation1 => finite_renormalisation
   use p0_gg_hhg_amplitudeh3_qp, only: samplitudeh3l1 => samplitude, &
        &   finite_renormalisation3 => finite_renormalisation
   use p0_gg_hhg_amplitudeh4_qp, only: samplitudeh4l1 => samplitude, &
        &   finite_renormalisation4 => finite_renormalisation
   use p0_gg_hhg_dipoles_qp, only: insertion_operator, insertion_operator_qed
...
contains
   ...
   subroutine     samplitude(vecs, scale2, amp, prec, ok, h)
      ...
      if(PSP_check) then
         ...
         if(spprec1.lt.PSP_chk_li1.and.spprec1.ge.PSP_chk_li2) then
            call increasecnt('gg->HHg qp: do rotation with Ninja')
            icheck=2 ! ROTATION
         end if
         if(spprec1.lt.PSP_chk_li2) then                                       ! RESCUE
            call increasecnt('gg->HHg qp: switch to golem95')
            icheck=3
         ...
         if(icheck.eq.2) then
            ...
            if(fpprec1.lt.PSP_chk_li3) then                                ! RESCUE
               icheck=3
               call increasecnt('gg->HHg qp: rotation unsuccessful, switch to golem95')
            endif
         endif
         ...
         if(icheck.eq.3.and.PSP_verbosity) then
            ...
            write(42,'(4x,A15,A9,A3)') &
                 &  "<process name='","p0_gg_hhg","'/>"
   ...
   function     samplitudel0(vecs, h) result(amp)
      use p0_gg_hhg_config_qp, only: logfile
   ...
   pure subroutine color_correlated_lo(color_vector,res)
      use p0_gg_hhg_color_qp, only: T1T1, &
      & T1T2, &
      & T1T5, &
      & T2T2, &
      & T2T5, &
   ...
   pure function square_0l_0l_sc(color_vector1, color_vector2) result(amp)
      use p0_gg_hhg_color_qp, only: cmat => CC
      implicit none
   ...
   pure function square_1_2_sc(color_vector1, color_vector2) result(amp)
      use p0_gg_hhg_color_qp, only: cmat => T1T2
   ...
   pure function square_1_5_sc(color_vector1, color_vector2) result(amp)
      use p0_gg_hhg_color_qp, only: cmat => T1T5
   ...
   pure function square_2_5_sc(color_vector1, color_vector2) result(amp)
      use p0_gg_hhg_color_qp, only: cmat => T2T5
   ...
   pure function square_5_1_sc(color_vector1, color_vector2) result(amp)
      use p0_gg_hhg_color_qp, only: cmat => T1T5
      ...
      v2 = conjg(color_vector1)
      amp = sum(v1(:) * v2(:))
   end function  square_5_1_sc
      
   pure function square_5_2_sc(color_vector1, color_vector2) result(amp)
      use p0_gg_hhg_color_qp, only: cmat => T2T5
...
end module p0_gg_hhg_matrix_qp
