...
! USAGE
!
!  use golem_function_3p_finite_qp
!
! DESCRIPTION
!
...
!
!*****
module golem_function_3p_finite_qp
  !
  ...
  use golem_generic_function_2p_qp
  use golem_multiply_div_qp
  use golem_s_matrix_type_qp
  use golem_matrice_s_qp, only : prepare_s_matrix_local, find_plus_grand
  implicit none
  !
  private :: ki
...
!AC!    else 
    ...
    ! on-shell cutoff decreased 26.4.2013 GH
         if (.not. olo) then
              call avh_olo_onshell_qp(100._ki*del)
       ...
       !
       !
        call avh_olo_c0m_qp(C0olo,real(s1r,ki_avh),real(s2r,ki_avh),real(s3r,ki_avh), &
...
!AC!    else 
   ...
   !       call avh_olo_onshell_qp(1.e-10_ki)
       ...
       !
       call avh_olo_c0c_qp(C0olo,cp1,cp2,cp3,cm3,cm1,cm2)
       !
       C0_carg=C0olo(0)
       !
!AC!       end if !end if withlt
  ...
  ! *************************************************
  !
end module golem_function_3p_finite_qp
