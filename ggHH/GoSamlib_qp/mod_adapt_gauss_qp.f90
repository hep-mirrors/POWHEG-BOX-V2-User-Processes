  ...
  contains
    ...
    subroutine creation(new,b_inf,b_sup)
      ...
      if (res /= 0) then
        !
        tab_erreur_par(1)%a_imprimer = .true.
        tab_erreur_par(1)%chaine = 'In subroutine creation (module golem_numerical_evaluation_qp)'
        tab_erreur_par(2)%a_imprimer = .true.
      ...
      if (associated(new%suivant)) then ! if this is not the last cell
        ...
        if (new%divise) then ! if one has to split
          ...
          if (res /= 0) then
            !
            tab_erreur_par(1)%a_imprimer = .true.
            tab_erreur_par(1)%chaine = 'In subroutine decoupe (module golem_numerical_evaluation_qp)'
            tab_erreur_par(2)%a_imprimer = .true.
      ...
      if (associated(new%suivant)) then
        ...
        if (.not.new%divise) then
          ...
          if (res /= 0) then
            !
            tab_erreur_par(1)%a_imprimer = .true.
            tab_erreur_par(1)%chaine = 'In subroutine recupere_partiel (module golem_numerical_evaluation_qp)'
            tab_erreur_par(2)%a_imprimer = .true.
    ...
    recursive subroutine libere(new)
      ...
      if (res /= 0) then
        !
        tab_erreur_par(1)%a_imprimer = .true.
        tab_erreur_par(1)%chaine = 'In subroutine libere (module golem_numerical_evaluation_qp)'
        tab_erreur_par(2)%a_imprimer = .true.
        tab_erreur_par(2)%chaine = 'the deallocation runs into trouble %d0'
        tab_erreur_par(2)%arg_int = res
        call catch_exception(0)
    ...
    !
end module golem_adapt_gauss_qp
