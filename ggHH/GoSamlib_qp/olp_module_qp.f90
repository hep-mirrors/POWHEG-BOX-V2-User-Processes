module olp_module_qp
   use, intrinsic :: iso_c_binding;
   implicit none
   private
   public :: OLP_Start_qp, OLP_EvalSubProcess_qp, OLP_Finalize_qp, OLP_Option_qp
   public :: OLP_SetParameter_qp, OLP_Info_qp

contains

   subroutine     OLP_Start_qp(contract_file_name,ierr,stage,rndseed) &
   & bind(C,name="olp_start_qp_")
      use, intrinsic :: iso_c_binding
      use pb_gg_hh_matrix_qp, only: pb_gg_hh_initgolem => initgolem
      use pb_gg_hh_config_qp, only: pb_gg_hh_PSP_rescue => PSP_rescue, &
           & pb_gg_hh_PSP_verbosity => PSP_verbosity, &
           & pb_gg_hh_PSP_chk_th1 => PSP_chk_th1, &
           & pb_gg_hh_PSP_chk_th2 => PSP_chk_th2, &
           & pb_gg_hh_PSP_chk_th3 => PSP_chk_th3, &
           & pb_gg_hh_PSP_chk_kfactor => PSP_chk_kfactor
      use p0_gg_hhg_matrix_qp, only: p0_gg_hhg_initgolem => initgolem
      use p0_gg_hhg_config_qp, only: p0_gg_hhg_PSP_rescue => PSP_rescue, &
      ...
      use p1_dg_hhd_matrix_qp, only: p1_dg_hhd_initgolem => initgolem
      use p1_dg_hhd_config_qp, only: p1_dg_hhd_PSP_rescue => PSP_rescue, &
      ...
      ! pb_gg_hh_PSP_rescue = PSP_rescue
      ! pb_gg_hh_PSP_verbosity =  PSP_verbosity
      ! pb_gg_hh_PSP_chk_th1 = PSP_chk_th1
      ! pb_gg_hh_PSP_chk_th2 = PSP_chk_th2
      ! pb_gg_hh_PSP_chk_th3 = PSP_chk_th3
      ...
      ! p1_dg_hhd_PSP_chk_kfactor = PSP_chk_kfactor
      if(stage.lt.0) then
         call pb_gg_hh_initgolem(.false.)
         call p0_gg_hhg_initgolem(.false.)
         call p1_dg_hhd_initgolem(.false.)
      else
         call pb_gg_hh_initgolem(.false.,stage,rndseed)
         call p0_gg_hhg_initgolem(.false.,stage,rndseed)
         call p1_dg_hhd_initgolem(.false.,stage,rndseed)
      end if

   end subroutine OLP_Start_qp

   subroutine OLP_Info_qp(olp_name, olp_version, message)&
   & bind(C,name="olp_info_qp_")
   use, intrinsic :: iso_c_binding, only: C_CHAR, C_NULL_CHAR
   use p0_gg_hhg_version_qp, only: gosamversion, gosamrevision
   ...
   end subroutine OLP_Info_qp


   subroutine OLP_SetParameter_qp(variable_name, real_part, imag_part, success)&
   & bind(C,name="olp_setparameter_qp_")
      use, intrinsic :: iso_c_binding
      use pb_gg_hh_model_qp, only: pb_gg_hh_set_parameter => set_parameter
      use p0_gg_hhg_model_qp, only: p0_gg_hhg_set_parameter => set_parameter
      use p1_dg_hhd_model_qp, only: p1_dg_hhd_set_parameter => set_parameter
      implicit none
      character(kind=c_char,len=1), intent(in) :: variable_name
      real(kind=c_float128), intent(in) :: real_part, imag_part
      ...
      call pb_gg_hh_set_parameter(variable_name(1:l),real_part,imag_part,success)
      if(success==0) then ! return immediately on error
          return
      end if
      call p0_gg_hhg_set_parameter(variable_name(1:l),real_part,imag_part,success)
      if(success==0) then ! return immediately on error
          return
      end if
      call p1_dg_hhd_set_parameter(variable_name(1:l),real_part,imag_part,success)
      if(success==0) then ! return immediately on error
          return
      end if
   end subroutine
   ...
   ! BLHA1 interface
   subroutine     OLP_EvalSubProcess_qp(label, momenta, mu,  parameters, res) &
   & bind(C,name="olp_evalsubprocess_qp_")
      use, intrinsic :: iso_c_binding
      implicit none
      integer(kind=c_int), intent(in) :: label
      real(kind=c_float128), intent(in) :: mu
      real(kind=c_float128), dimension(50), intent(in) :: momenta
      real(kind=c_float128), dimension(10), intent(in) :: parameters
      real(kind=c_float128), dimension(60), intent(out) :: res
      integer(kind=c_int) :: succ
      real(kind=c_float128) :: alpha_s
      real(kind=c_float128), parameter :: one_over_2pi = 0.15915494309189533577d0
      logical inquadprec
      common/cquad/inquadprec

      alpha_s = parameters(1)
      call increasecnt('real in quad prec')
      inquadprec = .true.
      select case(label)
      case(0)
   ...
   end subroutine OLP_EvalSubProcess_qp

   subroutine     OLP_Finalize_qp() &
   & bind(C,name="olp_finalize_qp_")
      use, intrinsic :: iso_c_binding
      use pb_gg_hh_matrix_qp, only: pb_gg_hh_exitgolem => exitgolem
      use p0_gg_hhg_matrix_qp, only: p0_gg_hhg_exitgolem => exitgolem
      use p1_dg_hhd_matrix_qp, only: p1_dg_hhd_exitgolem => exitgolem
      implicit none
      call pb_gg_hh_exitgolem(.false.)
      call p0_gg_hhg_exitgolem(.true.)
      call p1_dg_hhd_exitgolem(.true.)
   end subroutine OLP_Finalize_qp

   subroutine     OLP_Option_qp(line,stat) &
   & bind(C,name="olp_option_qp_")
      use, intrinsic :: iso_c_binding
      use pb_gg_hh_model_qp, only: pb_gg_hh_parseline => parseline
      use p0_gg_hhg_model_qp, only: p0_gg_hhg_parseline => parseline
      use p1_dg_hhd_model_qp, only: p1_dg_hhd_parseline => parseline
      implicit none
      ...
      l = strlen(line)
      call pb_gg_hh_parseline(line(1:l),ios)
      if (ios .ne. 0) then
         stat = 0
         return
      end if
   ...
   subroutine     eval0(momenta, mu, parameters, res, acc, blha1_mode)
      use, intrinsic :: iso_c_binding
      use p0_gg_hhg_config_qp, only: ki, PSP_chk_li3, nlo_prefactors, PSP_check
      use p0_gg_hhg_model_qp, only: parseline
      use p0_gg_hhg_kinematics_qp, only: boost_to_cms
      use p0_gg_hhg_matrix_qp, only: samplitude, OLP_spin_correlated_lo2, OLP_color_correlated
      use p0_gg_hhg_groups_qp, only: tear_down_golem95
      use p0_gg_hhg_groups_qp, only: ninja_exit

      implicit none
      real(kind=c_float128), dimension(25), intent(in) :: momenta
      real(kind=c_float128), intent(in) :: mu
      real(kind=c_float128), dimension(10), intent(in) :: parameters
      real(kind=c_float128), dimension(60), intent(out) :: res
      ...
      real(kind=c_float128), optional :: acc
      ...
      if(present(acc)) then
         ...
         if(prec.lt.PSP_chk_li3 .and. PSP_check) then
            call increasecnt('WARN: real in quad prec unstable')
      ...
      res(1) = real(amp(4), c_float128)
      res(2) = real(amp(3), c_float128)
      res(3) = real(amp(2), c_float128)
      res(4) = real(amp(1), c_float128)
   ...
   subroutine     eval1(momenta, mu, parameters, res, acc, blha1_mode)
      use, intrinsic :: iso_c_binding
      use p1_dg_hhd_config_qp, only: ki, PSP_chk_li3, nlo_prefactors, PSP_check
      use p1_dg_hhd_model_qp, only: parseline
      use p1_dg_hhd_kinematics_qp, only: boost_to_cms
      use p1_dg_hhd_matrix_qp, only: samplitude, OLP_spin_correlated_lo2, OLP_color_correlated
      use p1_dg_hhd_groups_qp, only: tear_down_golem95
      use p1_dg_hhd_groups_qp, only: ninja_exit
   ...
   subroutine     eval2(momenta, mu, parameters, res, acc, blha1_mode)
      ...
      use p1_dg_hhd_model_qp, only: parseline
      use p1_dg_hhd_kinematics_qp, only: boost_to_cms
      use p2_gd_hhd_matrix_qp, only: samplitude, OLP_spin_correlated_lo2, OLP_color_correlated
      use p1_dg_hhd_groups_qp, only: tear_down_golem95
      use p1_dg_hhd_groups_qp, only: ninja_exit
   ...
   subroutine     eval3(momenta, mu, parameters, res, acc, blha1_mode)
      ...
      use p1_dg_hhd_model_qp, only: parseline
      use p1_dg_hhd_kinematics_qp, only: boost_to_cms
      use p3_dbarg_hhdbar_matrix_qp, only: samplitude, OLP_spin_correlated_lo2, OLP_color_correlated
      use p1_dg_hhd_groups_qp, only: tear_down_golem95
      use p1_dg_hhd_groups_qp, only: ninja_exit
   ...
   subroutine     eval4(momenta, mu, parameters, res, acc, blha1_mode)
      ...
      use p1_dg_hhd_model_qp, only: parseline
      use p1_dg_hhd_kinematics_qp, only: boost_to_cms
      use p4_gdbar_hhdbar_matrix_qp, only: samplitude, OLP_spin_correlated_lo2, OLP_color_correlated
      use p1_dg_hhd_groups_qp, only: tear_down_golem95
      use p1_dg_hhd_groups_qp, only: ninja_exit
   ...
   subroutine     eval5(momenta, mu, parameters, res, acc, blha1_mode)
      ...
      use p1_dg_hhd_model_qp, only: parseline
      use p1_dg_hhd_kinematics_qp, only: boost_to_cms
      use p5_ddbar_hhg_matrix_qp, only: samplitude, OLP_spin_correlated_lo2, OLP_color_correlated
      use p1_dg_hhd_groups_qp, only: tear_down_golem95
      use p1_dg_hhd_groups_qp, only: ninja_exit
   ...
   subroutine     eval6(momenta, mu, parameters, res, acc, blha1_mode)
      ...
      real(kind=ki), dimension(5,4) :: vecs,tmpvecs
      real(kind=ki), dimension(4) :: amp
      real(kind=c_float128), optional :: acc
      logical, optional :: blha1_mode
      real(kind=ki) :: zero
      ...
      tmpvecs(:,1) = real(momenta(1::5),ki)
      tmpvecs(:,2) = real(momenta(2::5),ki)
      tmpvecs(:,3) = real(momenta(3::5),ki)
      tmpvecs(:,4) = real(momenta(4::5),ki)
   ...
   subroutine     eval7(momenta, mu, parameters, res, acc, blha1_mode)
      use, intrinsic :: iso_c_binding
      use pb_gg_hh_config_qp, only: ki, PSP_chk_li3, nlo_prefactors, PSP_check
      use pb_gg_hh_model_qp, only: parseline
      use pb_gg_hh_kinematics_qp, only: boost_to_cms
      use pb_gg_hh_matrix_qp, only: samplitude, OLP_spin_correlated_lo2, OLP_color_correlated
      use pb_gg_hh_groups_qp, only: tear_down_golem95
      use pb_gg_hh_groups_qp, only: ninja_exit
      ...
      real(kind=ki), dimension(5,4) :: vecsr
      ...
      vecsr(:,1) = real(momenta(1::5),ki)
      vecsr(:,2) = real(momenta(2::5),ki)
      vecsr(:,3) = real(momenta(3::5),ki)
      vecsr(:,4) = real(momenta(4::5),ki)
   ...
   !    use p0_gg_hhg_kinematics_qp, only: Spab3, Spaa
   !    implicit none
   !    real(kind=c_float128), dimension(0:3), intent(in) :: p,q
   !    real(kind=c_float128), dimension(0:7), intent(out) :: eps
   ...
   !    eps(0)=real(eps_complex(1),c_float128)
   !    eps(1)=real(aimag(eps_complex(1)),c_float128)
   !    eps(2)=real(eps_complex(2),c_float128)
   !    eps(3)=real(aimag(eps_complex(2)),c_float128)
   !    eps(4)=real(eps_complex(3),c_float128)
   !    eps(5)=real(aimag(eps_complex(3)),c_float128)
   !    eps(6)=real(eps_complex(4),c_float128)
   !    eps(7)=real(aimag(eps_complex(4)),c_float128)
   ...
   !    use, intrinsic :: iso_c_binding
   !    use p0_gg_hhg_model_qp, only: p0_gg_hhg_print_parameter => print_parameter
   !    use p1_dg_hhd_model_qp, only: p1_dg_hhd_print_parameter => print_parameter
   !    implicit none
   ...
   subroutine     read_slha_file(line)
      use pb_gg_hh_model_qp, only: pb_gg_hh_read_slha => read_slha
      use p0_gg_hhg_model_qp, only: p0_gg_hhg_read_slha => read_slha
      use p1_dg_hhd_model_qp, only: p1_dg_hhd_read_slha => read_slha
      implicit none
