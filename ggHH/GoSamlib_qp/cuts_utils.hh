...
namespace ninja_qp {
  namespace cuts_utils {
    ...
    void complementaryPartition (const ninja_qp::PartitionInt p [],
                                 const int dimp, const int n,
                                 ninja_qp::PartitionInt * cp);

    // This function checks whether a Partition 'a' of dimension 4 is a
    // subset of another Partition 'b' of dimension 5
    bool isSubPartition4of5(const ninja_qp::PartitionInt a[],
                            const ninja_qp::PartitionInt b[]);
    ...
    bool isSubPartition2of3(const ninja_qp::PartitionInt a[],
                            const ninja_qp::PartitionInt b[],
                            ninja_qp::PartitionInt c[]);

    // This function checks whether a Partition 'a' of dimension 1 is a
    // subset of another Partition 'b' of dimension 3 and in that case
    // returns b-a in c
    bool isSubPartition1of3(const ninja_qp::PartitionInt a[],
                            const ninja_qp::PartitionInt b[],
                            ninja_qp::PartitionInt c[]);
    ...
    // returns b-a in (the first entry of) c
    bool isSubPartition1of2(const ninja_qp::PartitionInt a[],
                            const ninja_qp::PartitionInt b[],
                            ninja_qp::PartitionInt c[]);
    ...
    class CutTadpole {
    public:
      CutTadpole(const ninja_qp::Basis & ein,
                 const MassType & mqin)
        : e(ein), mq(mqin) {}
