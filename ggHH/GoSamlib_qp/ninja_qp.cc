// Definition of the public method ninja_qp::Amplitude::evaluate() and
...
using namespace std;
using namespace ninja_qp_integer_math;
...
namespace quadninja_qp {
  void printBanner(std::ostream & banner_out, bool force_print)
  {
    ninja_qp::printBanner(banner_out, force_print);
  }
}
#endif
  ...
  int Amplitude<MassType>::evaluate(Numerator & num)
  {
    ...
    if (Options::verb) {
      ...
      if (return_val == Amplitude::SUCCESS) {
        (*Options::out) << "ninja_qp::Amplitude is returning SUCCESS" << endl;
      } else {
        (*Options::out) << "ninja_qp::Amplitude is returning TEST_FAILED" << endl;
      }
      (*Options::out) << "\n\n----------------------------"
                      << "----------------------------" << endl;
    }
