...
cc (no check of abs(id) in V2)
      subroutine old_from_number_to_madgraph(n,flav,emitter,string)
      implicit none
...
c     write(unit=iun,fmt=*) ' index= ',index
      do j=1,flst_nalr
cc use conversion adapted to sleptons start
         call old_from_number_to_madgraph
     #         (nlegreal,flst_alr(1,j),flst_emitter(j),string)
         call old_from_number_to_madgraph
     #         (nlegborn,flst_uborn(1,j),-1,stringb)
cc use conversion adapted to sleptons end
