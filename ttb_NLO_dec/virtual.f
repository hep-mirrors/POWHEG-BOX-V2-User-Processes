...
c     The as/(2pi) factor is attached at a later point
      subroutine setvirtual(ppp,vflav,vres)
      implicit none
      include 'nlegborn.h'
      include 'pwhg_flst.h'
...
c mxpart is in constants.f
     ...
     & msqsl(-nf:nf,-nf:nf),msqh(-nf:nf,-nf:nf),msq(-nf:nf,-nf:nf)
      ...
      do mu=0,3
         do j=1,nlegs
            if(ppp(mu,j).ne.psav(mu,j)) goto 11
         enddo
      enddo
      goto 22
 ...
 11   continue
      psav=ppp
c get momenta from on shell projected phase space
c      if(offshelltop.and..not.flg_in_smartsig) then
c         p = cmpborn_os
c      else
         p = ppp
c      endif
...
c Take away the alpha/pi piece
C-----Setup Born matrix element
         call qqb_QQbdk(mcfmp,msq)
         if(fltdec.lt.5) then
c one for t decay
            msqh = msqh - msq*2*ason2pi
         endif
c one for antitop decay
         if(fladec.lt.5) then
            msqh = msqh - msq*2*ason2pi
         endif
      endif
...
c Supply strong correction to branching ratios, if needed
      ...
      if(.not.flg_in_smartsig.and.reweightbymg) then
         iborn = flst_cur_iborn
         do while(equivto(iborn).ne.-1)
            iborn = equivto(iborn)
         enddo
         vres = vres * resc_array_os(iborn)
      endif
