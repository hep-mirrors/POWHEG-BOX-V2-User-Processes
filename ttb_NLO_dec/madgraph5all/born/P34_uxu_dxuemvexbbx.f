      SUBROUTINE SP34_uxu_dxuemvexbbx1(P,ANS)
C     
...
C     
C     Process: u~ u > t t~ > d~ u e- ve~ b b~ QCD=2 QED=4 @34
C     
      IMPLICIT NONE
C     
C     CONSTANTS
C     
      ...
      INCLUDE 'maxamps_P34_uxu_dxuemvexbbx.inc'
      INTEGER                 NCOMB
      PARAMETER (             NCOMB=256)
      INTEGER    NGRAPHS
      PARAMETER (NGRAPHS=1)
...
C     
      ...
      COMMON/TO_AMPS_P34_uxu_dxuemvexbbx/  AMP2,       JAMP2
      ...
      IF (ISHEL(IMIRROR) .EQ. 0 .OR. NTRY(IMIRROR) .LE. MAXTRIES) THEN
        DO I=1,NCOMB
          IF (GOODHEL(I,IMIRROR) .OR. NTRY(IMIRROR).LE.MAXTRIES) THEN
            T=P34_uxu_dxuemvexbbx1(P ,NHEL(1,I),JC(1))
            DO JJ=1,NINCOMING
...
C           Set right sign for ANS, based on sign of chosen helicity
 ...
 10     CONTINUE
      ...
      REAL*8 FUNCTION P34_uxu_dxuemvexbbx1(P,NHEL,IC)
C     
C     Generated by MadGraph 5 v. 1.5.13, 2013-11-04
C     By the MadGraph Development Team
C     Please visit us at https://launchpad.net/madgraph5
...
C     Amplitude(s) for diagram number 1
      ...
      JAMP(2)=+1./2.*(-AMP(1))
      P34_uxu_dxuemvexbbx1 = 0.D0
      DO I = 1, NCOLOR
        ZTEMP = (0.D0,0.D0)
        DO J = 1, NCOLOR
          ZTEMP = ZTEMP + CF(J,I)*JAMP(J)
        ENDDO
        P34_uxu_dxuemvexbbx1=P34_uxu_dxuemvexbbx1+ZTEMP*DCONJG(JAMP(I))/DENOM(I)
      ENDDO
