      SUBROUTINE SP50_gg_dxuscxbbx1(P,ANS)
C     
...
C     
C     Process: g g > t t~ > d~ u s c~ b b~ QCD=2 QED=4 @50
C     
      IMPLICIT NONE
C     
C     CONSTANTS
C     
      ...
      INCLUDE 'maxamps_P50_gg_dxuscxbbx.inc'
      INTEGER                 NCOMB
      PARAMETER (             NCOMB=256)
      INTEGER    NGRAPHS
      PARAMETER (NGRAPHS=3)
...
C     
      ...
      COMMON/TO_AMPS_P50_gg_dxuscxbbx/  AMP2,       JAMP2
      ...
      IF (ISHEL(IMIRROR) .EQ. 0 .OR. NTRY(IMIRROR) .LE. MAXTRIES) THEN
        DO I=1,NCOMB
          IF (GOODHEL(I,IMIRROR) .OR. NTRY(IMIRROR).LE.MAXTRIES) THEN
            T=P50_gg_dxuscxbbx1(P ,NHEL(1,I),JC(1))
            DO JJ=1,NINCOMING
...
C           Set right sign for ANS, based on sign of chosen helicity
 ...
 10     CONTINUE
      ...
      REAL*8 FUNCTION P50_gg_dxuscxbbx1(P,NHEL,IC)
C     
C     Generated by MadGraph 5 v. 1.5.13, 2013-11-04
C     By the MadGraph Development Team
C     Please visit us at https://launchpad.net/madgraph5
...
C     
      DATA DENOM(1)/1/
      DATA (CF(I,  1),I=  1,  2) /   48,   -6/
C     1 T(1,2,7,8) T(4,3) T(5,6)
      DATA DENOM(2)/1/
      DATA (CF(I,  2),I=  1,  2) /   -6,   48/
...
C     ----------
      ...
      CALL FFV2_1(W(1,7),W(1,10),GC_100,MT,WT,W(1,6))
      CALL FFV2_2(W(1,8),W(1,4),GC_100,MT,WT,W(1,10))
C     Amplitude(s) for diagram number 1
      CALL FFV1_0(W(1,10),W(1,6),W(1,9),GC_11,AMP(1))
      CALL FFV1_1(W(1,6),W(1,1),GC_11,MT,WT,W(1,9))
C     Amplitude(s) for diagram number 2
      CALL FFV1_0(W(1,10),W(1,9),W(1,2),GC_11,AMP(2))
      CALL FFV1_2(W(1,10),W(1,1),GC_11,MT,WT,W(1,9))
C     Amplitude(s) for diagram number 3
      CALL FFV1_0(W(1,9),W(1,6),W(1,2),GC_11,AMP(3))
      JAMP(1)=-IMAG1*AMP(1)+AMP(2)
      JAMP(2)=+IMAG1*AMP(1)+AMP(3)
      P50_gg_dxuscxbbx1 = 0.D0
      DO I = 1, NCOLOR
        ZTEMP = (0.D0,0.D0)
        DO J = 1, NCOLOR
          ZTEMP = ZTEMP + CF(J,I)*JAMP(J)
        ENDDO
        P50_gg_dxuscxbbx1=P50_gg_dxuscxbbx1+ZTEMP*DCONJG(JAMP(I))/DENOM(I)
      ENDDO
