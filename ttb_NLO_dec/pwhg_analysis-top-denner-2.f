c     Denner cuts, only LO,NLO,LHE analysis
      ...
      call bookupeqbins('pt_wp_b' ,40d0,0d0,800d0)
...
c      call bookupeqbins('costh_e+_mu-' ,0.1d0,-1d0,1d0)
      ...
      call bookupeqbins('pt_wp_b_wide',40d0,0d0,800d0)
      call bookupeqbins('m_wp_b_wide' ,50d0,0d0,2000d0)
      call bookupeqbins('m_wwbb_wide',200d0,0d0,2000d0)
      call bookupeqbins('pt_top_wide',40d0,0d0,800d0)
      call bookupeqbins('m_top_wide' ,50d0,0d0,2000d0)
      call bookupeqbins('m_tt_wide',200d0,0d0,2000d0)

      call bookupeqbins('pt_wp_b_wide_0_100',40d0,0d0,800d0)
      call bookupeqbins('m_wp_b_wide_0_100' ,50d0,0d0,2000d0)
      call bookupeqbins('m_wwbb_wide_0_100',200d0,0d0,2000d0)
      call bookupeqbins('pt_top_wide_0_100',40d0,0d0,800d0)
      call bookupeqbins('m_top_wide_0_100' ,50d0,0d0,2000d0)
      call bookupeqbins('m_tt_wide_0_100',200d0,0d0,2000d0)

      call bookupeqbins('pt_wp_b_wide_100_300',40d0,0d0,800d0)
      call bookupeqbins('m_wp_b_wide_100_300' ,50d0,0d0,2000d0)
      call bookupeqbins('m_wwbb_wide_100_300',200d0,0d0,2000d0)
      call bookupeqbins('pt_top_wide_100_300',40d0,0d0,800d0)
      call bookupeqbins('m_top_wide_100_300' ,50d0,0d0,2000d0)
      call bookupeqbins('m_tt_wide_100_300',200d0,0d0,2000d0)

      call bookupeqbins('pt_wp_b_wide_300_1000',40d0,0d0,800d0)
      call bookupeqbins('m_wp_b_wide_300_1000' ,50d0,0d0,2000d0)
      call bookupeqbins('m_wwbb_wide_300_1000',200d0,0d0,2000d0)
      call bookupeqbins('pt_top_wide_300_1000',40d0,0d0,800d0)
      call bookupeqbins('m_top_wide_300_1000' ,50d0,0d0,2000d0)
      call bookupeqbins('m_tt_wide_300_1000',200d0,0d0,2000d0)

      call bookupeqbins('pt_wp_b_wide_1000_inf',40d0,0d0,800d0)
      call bookupeqbins('m_wp_b_wide_1000_inf' ,50d0,0d0,2000d0)
      call bookupeqbins('m_wwbb_wide_1000_inf',200d0,0d0,2000d0)
      call bookupeqbins('pt_top_wide_1000_inf',40d0,0d0,800d0)
      call bookupeqbins('m_top_wide_1000_inf' ,50d0,0d0,2000d0)
      call bookupeqbins('m_tt_wide_1000_inf',200d0,0d0,2000d0)
...
c Massa del sistema W+bjet
     ...
     2         ptzmf(4),plzmf(4),p_top_rec(4),p_tb_rec(4),p_tt_rec(4)
     ...
     $     ,xab,ewp,pwp,ewm,pwm,xw, dy,deta,delphi,dr,cth1,cth2,ptmp(4)
     $     ,ht,m_top,m_tb,m_tt,m_top_rec,m_tb_rec,m_tt_rec
      integer jcth1
      real * 8 w(4),pb(4),ptb
...
c$$$c old piece of code, used to double-check our LO result with standalone MG5
c$$$      if(powheginput("#madevent").eq.1) then
c$$$c     !: MG5 doesn't seem to be compliant with LHE file 
c$$$c     conventions for value of xwgtup when idwtup is 3
c$$$c     (MG5 events don't have weight = 1, but tot_x_sec/nevents)
c$$$c     So  
c$$$c     if(abs(idwtup).eq.3) xwgtup=xwgtup*xsecup(1)
c$$$c     had to be commented away in lhef_analysis.f
c$$$         dsig=dsig*nevents
c$$$         
c$$$c     In madgraph record, not necessarily we have
c$$$c     the intermediate resonances, so don't look
c$$$c     for them, and just set the momenta after we
c$$$c     have extracted the final-state particles....
c$$$
c$$$         do j=1,nhep
c$$$c            if(idhep(j).eq.6) i_top = j
c$$$c            if(idhep(j).eq.-6) i_atop = j
c$$$c            if(idhep(j).eq.24) i_wp = j
c$$$c            if(idhep(j).eq.-24) i_wm = j
c$$$            if(idhep(j).eq.-11) i_lwp = j
c$$$            if(idhep(j).eq.13) i_lwm = j
c$$$            if(idhep(j).eq.12) i_nuwp = j
c$$$            if(idhep(j).eq.-14) i_nuwm = j
c$$$            if(idhep(j).eq.5) i_bfromtop = j
c$$$            if(idhep(j).eq.-5) i_abfromatop = j
c$$$         enddo
c$$$
c$$$         p_lwp=phep(1:4,i_lwp)
c$$$         p_lwm=phep(1:4,i_lwm)
c$$$         p_nuwp=phep(1:4,i_nuwp)
c$$$         p_nuwm=phep(1:4,i_nuwm)
c$$$         p_b=phep(1:4,i_bfromtop)
c$$$         p_ab=phep(1:4,i_abfromatop)
c$$$
c$$$         p_wp=p_lwp+p_nuwp
c$$$         p_wm=p_lwm+p_nuwm
c$$$
...
c$$$         IsForClustering(13) = .true.
c$$$         IsForClustering(i_bfromtop) = .true.
c$$$         IsForClustering(i_abfromatop) = .true.
c$$$         
c$$$      endif
...
c     reconstructed tops (using b-jets)
      p_top_rec=p_wp+p_b
      p_tb_rec=p_wm+p_ab
      p_tt_rec=p_top_rec+p_tb_rec

      m_top_rec=sqrt(p_top_rec(4)**2-p_top_rec(1)**2-p_top_rec(2)**2
     $     -p_top_rec(3)**2)
      m_tb_rec=sqrt(p_tb_rec(4)**2-p_tb_rec(1)**2-p_tb_rec(2)**2
     $     -p_tb_rec(3)**2)
      m_tt_rec=sqrt(p_tt_rec(4)**2-p_tt_rec(1)**2-p_tt_rec(2)**2
     $     -p_tt_rec(3)**2)

c     'MC' truth tops
      m_top=sqrt(p_top(4)**2-p_top(1)**2-p_top(2)**2 -p_top(3)**2)
      m_tb=sqrt(p_tb(4)**2-p_tb(1)**2-p_tb(2)**2 -p_tb(3)**2)
      ptmp = p_top+p_tb
      m_tt=sqrt(ptmp(4)**2-ptmp(1)**2-ptmp(2)**2 -ptmp(3)**2)
...
c     plots
      ...
      call filld('pt_wp_b' ,sqrt(p_top_rec(1)**2+p_top_rec(2)**2),dsig)
     ...
     1  sqrt((p_nuwp(1)+p_nuwm(1))**2+(p_nuwp(2)+p_nuwm(2))**2),dsig)
      ...
      call filld('m_wp_b',m_top_rec,dsig)
     ...
     1     sqrt(ptmp(4)**2-ptmp(1)**2-ptmp(2)**2-ptmp(3)**2),dsig)
      ...
      call filld('m_wwbb' ,m_tt_rec,dsig)
...
c      call filld('costh_e+_mu-' ,0.1d0,-1d0,1d0)
      ...
      call filld('m_top' ,m_top,dsig)
      call filld('m_tt' ,m_tt,dsig)
      call filld('m_tt_zoom',m_tt,dsig)


      call filld('pt_wp_b_wide',sqrt(p_top_rec(1)**2+p_top_rec(2)**2)
     $     ,dsig)
      call filld('m_wp_b_wide',m_top_rec,dsig)
      call filld('m_wwbb_wide',m_tt_rec,dsig)
      call filld('pt_top_wide',sqrt(p_top(1)**2+p_top(2)**2)
     $     ,dsig)
      call filld('m_top_wide',m_top,dsig)
      call filld('m_tt_wide',m_tt,dsig)

      if((m_top.lt.100).and.(m_tb.lt.100)) then
         call filld('pt_wp_b_wide_0_100',sqrt(p_top_rec(1)**2
     $        +p_top_rec(2)**2),dsig)
         call filld('m_wp_b_wide_0_100',m_top_rec,dsig)
         call filld('m_wwbb_wide_0_100',m_tt_rec,dsig)
         call filld('pt_top_wide_0_100',sqrt(p_top(1)**2+p_top(2)**2)
     $        ,dsig)
         call filld('m_top_wide_0_100',m_top,dsig)
         call filld('m_tt_wide_0_100',m_tt,dsig)
      endif

      if((m_top.gt.100.and.m_tb.gt.100).and.(m_top.lt.300.and.m_tb.lt.300)
     $     ) then
         call filld('pt_wp_b_wide_100_300',sqrt(p_top_rec(1)**2
     $        +p_top_rec(2)**2),dsig)
         call filld('m_wp_b_wide_100_300',m_top_rec,dsig)
         call filld('m_wwbb_wide_100_300',m_tt_rec,dsig)
         call filld('pt_top_wide_100_300',sqrt(p_top(1)**2+p_top(2)**2)
     $        ,dsig)
         call filld('m_top_wide_100_300',m_top,dsig)
         call filld('m_tt_wide_100_300',m_tt,dsig)
      endif

      if((m_top.gt.300.and.m_tb.gt.300).and.(m_top.lt.1000.and.m_tb.lt.1000)
     $     ) then
         call filld('pt_wp_b_wide_300_1000',sqrt(p_top_rec(1)**2
     $        +p_top_rec(2)**2),dsig)
         call filld('m_wp_b_wide_300_1000',m_top_rec,dsig)
         call filld('m_wwbb_wide_300_1000',m_tt_rec,dsig)
         call filld('pt_top_wide_300_1000',sqrt(p_top(1)**2+p_top(2)**2)
     $        ,dsig)
         call filld('m_top_wide_300_1000',m_top,dsig)
         call filld('m_tt_wide_300_1000',m_tt,dsig)
      endif

      if((m_top.gt.1000).and.(m_tb.gt.1000)) then
         call filld('pt_wp_b_wide_1000_inf',sqrt(p_top_rec(1)**2
     $        +p_top_rec(2)**2),dsig)
         call filld('m_wp_b_wide_1000_inf',m_top_rec,dsig)
         call filld('m_wwbb_wide_1000_inf',m_tt_rec,dsig)
         call filld('pt_top_wide_1000_inf',sqrt(p_top(1)**2+p_top(2)**2)
     $        ,dsig)
         call filld('m_top_wide_1000_inf',m_top,dsig)
         call filld('m_tt_wide_1000_inf',m_tt,dsig)
      endif
