...
C - undecayed.
        do ixx=1,nhep
         ...
         enddo
C - If the particle is a Z label it as such and store the momentum
...
C - We get to here for all non-Higgs particles
         if(WHCPRG.eq.'NLO   '.or.WHCPRG.eq.'LHE   ') then
            if(abs(idhep(ixx)).ne.11
     $           .and.abs(idhep(ixx)).ne.12) then
               plabel(ixx)='pp' !  N.B. This means EVERYTHING but W and leptons goes
                                !       into the genclust_kt jet finder.
                                !       This is what DYNNLO does.
            endif
         endif
      enddo
...
C     Maximum number of particles to send to jet finder: 
      ...
      if(jets.eq.2) then
         ptj1=pt(6,p)  !  This is pt(5,p) in HNNLO as there H decays to YY. 
         ptj2=pt(7,p)  !  This is pt(6,p) in HNNLO as there H decays to YY. 
