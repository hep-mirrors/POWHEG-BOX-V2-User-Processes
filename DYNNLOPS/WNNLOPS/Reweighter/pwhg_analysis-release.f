...
C - Set bin widths
      ...
      call pwhgbookup3(1,'yw-theta-mw','lin',0.4d0,-5d0,5d0
     $     ,pi/25d0,0d0,pi
     $     ,pi/25d0,-pi/2d0,pi/2d0)
      call pwhgbookup3(2,'yw-theta-mw-ptj1-3','lin',0.4d0,-5d0,5d0
     $     ,pi/25d0,0d0,pi
     $     ,pi/25d0,-pi/2d0,pi/2d0)
      call pwhgbookup3(3,'yw-theta_l-mw','lin',0.4d0,-5d0,5d0
     $     ,pi/25d0,0d0,pi
     $     ,pi/25d0,-pi/2d0,pi/2d0)
      call pwhgbookup3(4,'yw-theta_l-mw-ptj1-3','lin',0.4d0,-5d0,5d0
     $     ,pi/25d0,0d0,pi
     $     ,pi/25d0,-pi/2d0,pi/2d0)
      call pwhgbookup3(5,'yw-theta_t-mw','lin',0.4d0,-5d0,5d0
     $     ,pi/25d0,0d0,pi
     $     ,pi/25d0,-pi/2d0,pi/2d0)
      call pwhgbookup3(6,'yw-theta_t-mw-ptj1-3','lin',0.4d0,-5d0,5d0
     $     ,pi/25d0,0d0,pi
     $     ,pi/25d0,-pi/2d0,pi/2d0)
      end
...
C - Maximum number of particles to send to jet finder: 
      include  'mxpart.f'
C - Higgs kinematics
      real*8  wmass,theta,theta_t,theta_l,wwidth
      common/wmass/wmass,wwidth
      real*8  pw(4),yw,etaw,ptw,mw,p_ele(4),p_boosted(4),boost(4)
C - Jet radius: 
      real*8  Rmin !:
      common/Rmin/Rmin
C - HNNLO jet finding:
...
ccccccccccccccccccccccccccc
     ...
     1    phij(maxjet),pj(4,maxjet),ptrel(4)
      ...
      if (ini) then
         if(wmass.le.0d0) then
            write(*,*) 'pwhg_analysis-release.f:'
            write(*,*) 'wmass = ',wmass
            write(*,*) 'Have not got acceptable W mass'
            stop
         endif
         write(*,*) '*****************************'
         ...
         write(*,'(a,f10.5)') 
     C        ' pwhg_analysis: W mass (wmass) value: ', wmass
         write(*,'(a,f6.4)') 
...
C     find the w
      do ixx=1,nhep
         if(abs(idhep(ixx)).eq.24) then
            pw=phep(1:4,ixx)
            cycle
         endif
      enddo
      do ixx=1,nhep
         if(abs(idhep(ixx)).eq.11
     $        .or.abs(idhep(ixx)).eq.13 ) then
            p_ele=phep(1:4,ixx)
            cycle
         endif
      enddo
...
C     compute damping and fill histograms 
      ...
      call pwhgfill3(1,yw,theta,x,dsig)
      call pwhgfill3(2,yw,theta,x,damping_vec*dsig)
      call pwhgfill3(3,yw,theta_l,x,dsig)
      call pwhgfill3(4,yw,theta_l,x,damping_vec*dsig)
      call pwhgfill3(5,yw,theta_t,x,dsig)
      call pwhgfill3(6,yw,theta_t,x,damping_vec*dsig)
      call pwhgaccumup3
...
C     - Extract final state particles to feed to jet finder
         do j=1,nhep
c all but the W and leptons
            if (isthep(j).eq.1.and.(
     $           .not.abs(idhep(j)).eq.24.and..not.abs(idhep(j)).eq.12
     $           .and..not.abs(idhep(j)).eq.11
     $           .and..not.abs(idhep(j)).eq.13)) then
...
c      print*, 'njets', njets, 'momentum', pjet(:,:2)
c check consistency
c      do k=1,ntracks
c         print*, 'jetvec(k)', k , jetvec(k)
