...
C     maxbins: max number of bins per distribution 
      ...
      real * 8 yZ,ptz,HT,ptj1,ptj2,hdamp,theta,theta_l,theta_t
      real * 8 etaz,pz(4),mz,histo(10),p_ele(4)
      real * 8 damping_vec  !:
...
c     Find the value of zmass in the header of the event
...
c     
C     Read in all lines until you find the first occurrence of zmass
      ios=0
      zmass=-1000d0
      zwidth=-1000d0
      do while(ios.eq.0)
         read(nlf,fmt='(a)',iostat=ios) tmpLine
         if(index(tmpLine,'Zmass').ne.0) then
            tmpLine2 = tmpLine(index(trim(tmpLine),' ',back=.true.)
     $                        :len(tmpLine))
            read(tmpLine2,*,iostat=ios) zmass
         endif
         if(index(tmpLine,'Zwidth').ne.0) then
            tmpLine2 = tmpLine(index(trim(tmpLine),' ',back=.true.)
     $                        :len(tmpLine))
            read(tmpLine2,*,iostat=ios) zwidth
         endif
      enddo
      if(zmass.lt.0) then
         write(*,*) 'zmass token not found in LH input file'
         zmass=91.1876d0
         write(*,*) 'zmass set to ', zmass
      else
         write(*,*) ' In LH input file header, found '//
     $        'zmass = ',zmass
      endif
      if(zwidth.lt.0) then
         write(*,*) 'zwidth token not found in LH input file'
         zwidth=2.49595d0
         write(*,*) 'zwidth set to ', zwidth
      else
         write(*,*) ' In LH input file header, found '//
     $        'zwidth = ',zwidth
      endif
...
C     Find bin associated to current value of Z rapidity, mass and angle
         xbin = 0
         ybin = 0
         zbin = 0
         ibin = 0

         tmp=((yz-HMIN3(1,dataindex))/HDEL3(1,dataindex))+1d0
         xbin=int(tmp)
     ...
     $        .or.zbin.gt.nbin3(3,dataindex).or.zbin.lt.0) then 
            write(*,*) 'event out of range', yz, theta, mz
            cycle
         endif
