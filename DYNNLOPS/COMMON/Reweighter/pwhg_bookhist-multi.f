...
C-- projection on the x-axis
 ...
 10   BOOK3(N)=' NO'
    ...
    1 HIST3(:,I,J,K,N)=HIST3(:,I,J,K,N)/HINT3(:,N)*X
      ...
      CHARACTER CTIME*8
      character*64 num_str
      character*64 outputpre
      character*64 outputname 
...
c         print*, i, 'book3(n)', book3(i)
      enddo
c      print*, 'Entered print3', BOOK3(N)
      IF(BOOK3(N).NE.'YES') RETURN
      ...
      do m=1,nmulti
         write(num_str,'(I1)') m
         call strcat(trim(outputpre),trim(num_str),outputname)
         call strcat(trim(outputname),'-denom.top',outputname)
         open(unit=iun,file=outputname,status='unknown')
