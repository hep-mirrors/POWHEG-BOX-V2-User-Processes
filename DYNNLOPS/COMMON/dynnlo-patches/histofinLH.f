...
c--- For itno>0, this is an intermediate result only
      ...
      character*72 runname,outfiledat,outfiletop,outfileerr,outfiletop3D
      character*3 oldbook
...
C     C    Rescale by itmx/itno and put in j+120
            
            call mopera(j+20,'F',2,j+120,dfloat(itmx)/dfloat(itno),1d0)
            
C     C    If needed, combine with final result for virtual
...
c         call strcat(runstrin,'3D.top',outfiletop3D)
c         open(unit=399,file=outfiletop3D,status='unknown')
c         call pwhgaddout 
         
C     C If we are at the last step
...
C     now 3D plot 
      ...
      call strcat(runstring,'3D.top',outfiletop3D)
      open(unit=399,file=outfiletop3D,status='unknown')
