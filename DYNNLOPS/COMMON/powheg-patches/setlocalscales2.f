...
c coupling rescaling, for Born (imode=1) and NLO corrections (imode=2)
      ...
      if(ini) then
         ...
         if(factsc2min.lt.0) factsc2min = 1
         if(frensc2min.lt.4) frensc2min = 4 ! : see comments below
         write(*,*) ' **** factsc2min:',factsc2min
         write(*,*) ' **** frensc2min:',frensc2min
...
c     We need to set to 0 the weight of the current point if ptb is extremely small.

c     When as is run using the powheg routine pwhg_alphas, the following condition
c     can be used, with frensc2min>=2 (if it is set to 1, integration grids are unstable)
      if(st_renfact**2*ptb2.lt.frensc2min*st_lambda5MSB**2) then
         rescfac  =0d0
         orescfac =0d0
         return
      endif
...
c     When as is run from mb to ptb using as/(1-y) + ..., the condition y.ge.1 should be used.
...
c     alpha_s reweighting
      ...
      st_mufact2=max(ptb2*st_facfact**2,factsc2min*q2min)
      omuf2=st_mufact2

c      alphas=as/(1-y)-as**2*b1*log(1-y)/(b0*(1-y)**2) 
