module p2_udbar_epnebbbar_matrix
...
contains
   ...
   pure function prefactor()
      ...
      use p0_dbaru_epnebbbar_model, only: NC
      use p0_dbaru_epnebbbar_kinematics, only: &
      & in_helicities, symmetry_factor
      use p0_dbaru_epnebbbar_color, only: incolors
      implicit none
   ...
   subroutine     initgolem(is_first)
      use p0_dbaru_epnebbbar_matrix, only: orig_initgolem => initgolem
      implicit none
   ...
   subroutine     exitgolem(is_last)
      use p0_dbaru_epnebbbar_matrix, only: orig_exitgolem => exitgolem
      implicit none
   ...
   subroutine     samplitude(vecs, scale2, amp, ok, h)
      use p0_dbaru_epnebbbar_matrix, only: orig_sub => samplitude
      implicit none
   ...
   function     samplitudel0(vecs, h) result(amp)
      use p0_dbaru_epnebbbar_matrix, only: orig_func => samplitudel0
      implicit none
      real(ki), dimension(6, 4), intent(in) :: vecs
   ...
   function     samplitudel1(vecs,scale2,ok,rat2,h) result(amp)
      use p0_dbaru_epnebbbar_matrix, only: orig_func => samplitudel1
      implicit none
      real(ki), dimension(6, 4), intent(in) :: vecs
...
end module p2_udbar_epnebbbar_matrix
