module     p1_dbarc_epnebbbar_samuraih3
...
function     numeval_group0(icut, Q, mu2) result(num)
   ...
   use p1_dbarc_epnebbbar_d14h3l1, only: numerator_d14 => numerator_samurai
   use p1_dbarc_epnebbbar_d33h3l1, only: numerator_d33 => numerator_samurai
   implicit none
...
function     numeval_group1(icut, Q, mu2) result(num)
   use p1_dbarc_epnebbbar_kinematics, only: k1, k2, k3, k4, k5, k6
   use p1_dbarc_epnebbbar_model
   use p1_dbarc_epnebbbar_d3h3l1, only: numerator_d3 => numerator_samurai
   implicit none
   integer, intent(in) :: icut
   complex(ki_sam), dimension(4), intent(in) :: Q
   complex(ki_sam), intent(in) :: mu2
...
function     numeval_group2(icut, Q, mu2) result(num)
   use p1_dbarc_epnebbbar_kinematics, only: k1, k2, k3, k4, k5, k6
   use p1_dbarc_epnebbbar_model
   use p1_dbarc_epnebbbar_d2h3l1, only: numerator_d2 => numerator_samurai
   implicit none
   integer, intent(in) :: icut
   complex(ki_sam), dimension(4), intent(in) :: Q
   complex(ki_sam), intent(in) :: mu2
...
function     numeval_group3(icut, Q, mu2) result(num)
   use p1_dbarc_epnebbbar_kinematics, only: k1, k2, k3, k4, k5, k6
   use p1_dbarc_epnebbbar_model
   use p1_dbarc_epnebbbar_d27h3l1, only: numerator_d27 => numerator_samurai
   use p1_dbarc_epnebbbar_d28h3l1, only: numerator_d28 => numerator_samurai
   use p1_dbarc_epnebbbar_d31h3l1, only: numerator_d31 => numerator_samurai
   implicit none
   integer, intent(in) :: icut
   complex(ki_sam), dimension(4), intent(in) :: Q
   complex(ki_sam), intent(in) :: mu2
...
function     numeval_group4(icut, Q, mu2) result(num)
   use p1_dbarc_epnebbbar_kinematics, only: k1, k2, k3, k4, k5, k6
   use p1_dbarc_epnebbbar_model
   use p1_dbarc_epnebbbar_d1h3l1, only: numerator_d1 => numerator_samurai
   use p1_dbarc_epnebbbar_d4h3l1, only: numerator_d4 => numerator_samurai
   use p1_dbarc_epnebbbar_d6h3l1, only: numerator_d6 => numerator_samurai
   use p1_dbarc_epnebbbar_d8h3l1, only: numerator_d8 => numerator_samurai
   use p1_dbarc_epnebbbar_d9h3l1, only: numerator_d9 => numerator_samurai
   use p1_dbarc_epnebbbar_d10h3l1, only: numerator_d10 => numerator_samurai
   use p1_dbarc_epnebbbar_d24h3l1, only: numerator_d24 => numerator_samurai
   use p1_dbarc_epnebbbar_d25h3l1, only: numerator_d25 => numerator_samurai
   use p1_dbarc_epnebbbar_d26h3l1, only: numerator_d26 => numerator_samurai
   use p1_dbarc_epnebbbar_d29h3l1, only: numerator_d29 => numerator_samurai
   use p1_dbarc_epnebbbar_d30h3l1, only: numerator_d30 => numerator_samurai
   use p1_dbarc_epnebbbar_d32h3l1, only: numerator_d32 => numerator_samurai
   implicit none
   integer, intent(in) :: icut
   complex(ki_sam), dimension(4), intent(in) :: Q
   complex(ki_sam), intent(in) :: mu2
...
subroutine     reduce_group0(scale2,tot,totr,ok)
   ...
   use p1_dbarc_epnebbbar_model
   use p1_dbarc_epnebbbar_d14h3l1, only: numerator_diagram14 => numerator_samur&
   &ai
   use p1_dbarc_epnebbbar_d33h3l1, only: numerator_diagram33 => numerator_samur&
   &ai
...
subroutine     reduce_group1(scale2,tot,totr,ok)
   ...
   use p1_dbarc_epnebbbar_kinematics
   use p1_dbarc_epnebbbar_model
   use p1_dbarc_epnebbbar_d3h3l1, only: numerator_diagram3 => numerator_samurai
   use p1_dbarc_epnebbbar_globalsl1, only: epspow
...
subroutine     reduce_group2(scale2,tot,totr,ok)
   ...
   use p1_dbarc_epnebbbar_kinematics
   use p1_dbarc_epnebbbar_model
   use p1_dbarc_epnebbbar_d2h3l1, only: numerator_diagram2 => numerator_samurai
   use p1_dbarc_epnebbbar_globalsl1, only: epspow
...
subroutine     reduce_group3(scale2,tot,totr,ok)
   ...
   use p1_dbarc_epnebbbar_kinematics
   use p1_dbarc_epnebbbar_model
   use p1_dbarc_epnebbbar_d27h3l1, only: numerator_diagram27 => numerator_samur&
   &ai
   use p1_dbarc_epnebbbar_d28h3l1, only: numerator_diagram28 => numerator_samur&
   &ai
   use p1_dbarc_epnebbbar_d31h3l1, only: numerator_diagram31 => numerator_samur&
   &ai
   use p1_dbarc_epnebbbar_globalsl1, only: epspow
...
subroutine     reduce_group4(scale2,tot,totr,ok)
   ...
   use p1_dbarc_epnebbbar_kinematics
   use p1_dbarc_epnebbbar_model
   use p1_dbarc_epnebbbar_d1h3l1, only: numerator_diagram1 => numerator_samurai
   use p1_dbarc_epnebbbar_d4h3l1, only: numerator_diagram4 => numerator_samurai
   use p1_dbarc_epnebbbar_d6h3l1, only: numerator_diagram6 => numerator_samurai
   use p1_dbarc_epnebbbar_d8h3l1, only: numerator_diagram8 => numerator_samurai
   use p1_dbarc_epnebbbar_d9h3l1, only: numerator_diagram9 => numerator_samurai
   use p1_dbarc_epnebbbar_d10h3l1, only: numerator_diagram10 => numerator_samur&
   &ai
   use p1_dbarc_epnebbbar_d24h3l1, only: numerator_diagram24 => numerator_samur&
   &ai
   use p1_dbarc_epnebbbar_d25h3l1, only: numerator_diagram25 => numerator_samur&
   &ai
   use p1_dbarc_epnebbbar_d26h3l1, only: numerator_diagram26 => numerator_samur&
   &ai
   use p1_dbarc_epnebbbar_d29h3l1, only: numerator_diagram29 => numerator_samur&
   &ai
   use p1_dbarc_epnebbbar_d30h3l1, only: numerator_diagram30 => numerator_samur&
   &ai
   use p1_dbarc_epnebbbar_d32h3l1, only: numerator_diagram32 => numerator_samur&
   &ai
   use p1_dbarc_epnebbbar_globalsl1, only: epspow
...
!-----#] subroutine reduce_group4:
!---#] reduce groups with samurai:
end module p1_dbarc_epnebbbar_samuraih3
