module     p3_usbar_epnebbbar_groups
   use precision, only: ki_sam => ki
   use madds
   use precision_golem, only: ki_gol => ki
   use tens_rec
   use p3_usbar_epnebbbar_config, only: ki , reduction_interoperation, samurai_scalar 
   implicit none
   save
...
function     contract_tensor_coefficients_group_0(coeffs) result(amp)
   ...
   use p3_usbar_epnebbbar_config, only: debug_nlo_diagrams, logfile
   use p3_usbar_epnebbbar_kinematics, only: k1, k2, k3, k4, k5, k6, es234, es345, es12, es56, es23, es61, es123, &
   & es6, es5, es45, es34
...
!---#] subroutine tear_down_golem95:
end module p3_usbar_epnebbbar_groups
