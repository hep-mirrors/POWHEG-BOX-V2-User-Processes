...
c calls to pdfset (you never know)
     ...
     ,         alphasPDF,asmz,mz,lam5
      ...
      if(ndns.ne.ndns0) then
         ...
         asmz=alphasPDF(mz)
c     The pdf returns alphas(mz) with nf flavors. If nf is different
c     from 5, we would end up computing a wrong lam5, if we use this value 
c     of alphas 
c     Since a function to get the number of flavors from the pdf has not
c     been standardized, we HARDWIRE here the value of asmz for the 
c     commonly used pdfs with nf=4
c     MSTW2008nlo68cl_nf4  : 23300
c     CT10nlo_nf4          : 11082
         if (ndns.eq.11082.or.ndns.eq.23300) then
            write(*,*) "*******************************"
            write(*,*) "Using the value of alpha strong at the Z mass"
